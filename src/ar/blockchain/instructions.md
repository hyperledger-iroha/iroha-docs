...
| [التسجيل/التخصيص / التحديث ](#other-instructions) |السجلات، الحملات المفيدة الخاصة بالمؤسسات التنفيذية، تحديثات المواصلات |                      |
...
كل ما يمكن تسجيله هو `Registrable` و `Identifiable`، لكن ليس كل ما هو `Identifiable` هو `Registrable`. يتم تسجيل معظم الأشياء مباشرة، ولكن في بعض الحالات تمثيل في بلوكتشين لديه الكثير من البيانات. لأسباب أمنية وأداء، نحن نستخدم البنّاء لهذه هيكلات البيانات (مثل `NewAccount`) ، وتحتوي تسجيل الأقران على تعليمة مخصصة لإثبات الملكية.
...
راجعوا إحدى الدليلات الخاصة باللغة لتشيركم إلى عملية تسجيل الأشياء في بلوكتشين:
...
راجعوا إحدى الدليلات الخاصة باللغة لتشيركم إلى عملية صياغة الأصول في بلوكتشين:
