...
FastPQ булып тора Iroha Ул - STARK Һайланған башҡарыу эффекттары өсөн иҫбатлау юлы. Ул ғәҙәти транзакция үтәлеше йәки консенсус алмаштырмай. ISI, IVM, һәм Sumeragi ғәҙәттәгесә; FastPQ детерминистик үтәү шаһиты ҡулланыу һәм тәьмин ителгән эффекттар иҫбатлау партияларына әйләндереү.

Хәҙерге ҡабул итеүсе интеграцияла өс төп юл бар:

- Блоктарҙы үтәү ваҡытында теркәлгән һанлы активтар күсерелеүе
- Nexus проверенные релеев полосы которых AXT иҫбатлау конверты менән FastPQ бәйләүсе
...
Бында `Hash` мәғәнәһе Iroha Ул - `iroha_crypto::Hash::new`, 32 байтлыҡ Blake2bVar дигесты, әгәр формула менән асыҡтан-асыҡ Poseidon2 йәки SHA-256.
...
ҡайҙа `h_i` транзакция һәм ваҡыт trigger инеү нөктәһе хэштегтар. асыҡ иҫбатлау IO, әгәр `perm_root` йәки `tx_set_hash` бөтәһе лә нуль булһа, проверка кире ҡайтыу ҡиммәттәрен тултыра:
...
ҡайҙа `P` - 7 байтлы пакетта, `E` булып тора Norito кодлау, `D_o` булып тора `fastpq:v1:ordering`, һәм `T*` тип иҫәпләнгән күсеү исемлеге.
...
Эҙләүсе битте яҙҙыра . `b_l`, ҡустыһы `s_l`, инеү узелы `x_l`, һәм сығарыу узелы `x_{l+1}` һәр кимәлдә. Кодтың тармаҡ конвенцияһы:
...
Разрешение эҙләү аккумуляторы Fiat-Shamir проблемаһын ҡуллана . `gamma`. Өсөн түбән дәрәжәләге киңәйтеү баһалары `s_perm` һәм `perm_hash`, хәрәкәт итеүсе продукт:
...
Килһен `omega_T` эҙләнеү-домен генераторы булырға, `omega_E` баһалау домены генераторы, һәм `g` конфигурированный косет оффсет. `v_i`, интерполяция коэффициенттар тыуҙыра `a_j` шул тиклем:
...
Һуңынан LDE, FastPQ һәр рәт буйынса бөтә hashes LDE колонналар. өсөн `m` бағаналар:
...
Индекста япраҡ өсөн `i`, юл `(s_0,\ldots,s_{d-1})` тамырға ҡаршы тикшерелә `R` ҡабатланыуы буйынса:
...
FRI бурысын үтәй AIR Төҙөлөш баһалары. Һәр тур өсөн `l`, транскрипты өлгөһө һынау `beta_l`. Склад һуңғы ҡиммәтен ҡабатлап, аритлыҡтың ҡабатланыусыһына тиклем ҡаплана.
...
Каноник параметрҙар каталогы транскрипты хэш тип билдәләй SHA3-256. Хәҙерге провер һәм верификатор ғәмәлгә ашырыу менән проблема байт алып килә: `iroha_crypto::Hash::new`, Был 32-байт Blake2bVar һеңдереү, һуңынан беренсе һигеҙ бәләкәй эндиан байттарҙы кәметә `F`:
...
3. `gamma`
4. AIR составындағы проблемалар `alpha_0`, `alpha_1`
...
Ҡоролтай AIR композиция асыу подлинность `R_air_composition`. Ҡоролтай FRI сылбыр шул уҡ баштан башлана `A_q` һәм раҫланған финал менән тамамланырға тейеш FRI Терминал аҫтындағы япраҡ FRI тамыр.
...
`auto` проверканың арттағы вариантты һайлап алыуына мөмкинлек бирә. `cpu` фишкалар үтәлеше CPU. `gpu` өҫтөнлөк бирә GPU үтәү, CPU кире ҡайтыу, әгәр backend талап ителгән ядроларҙы ҡуллана алмай.

## Тикшереү {#verification}

FastPQ иҫбатлау тикшереүе каноник партия йөкләмәһен ҡайтанан төҙөй һәм йәмәғәт транскриптын ҡабатлай. Тикшереүсе протокол версияһын, параметрҙар ҡуйылған версияны, ҡабатлау сиктәрен, эҙләү йөкләмәһен, йәмәғәт инештәрен, өлгө буйынса Merkle асыҡлыҡтарын тикшерә. AIR асҡыстар, һәм FRI һорауҙар сылбыры.
...
Әгәр тикшереү уңышлы булһа, Iroha һаҡлана `VerifiedLaneRelayRecord` эстафетаһы, оригиналь конверт, иҫбатлау йөкләмәһе хэшиғы, тикшереү бейеклеге, манифст тамырҙары һәм FastPQ бәйләүсе.
...
Ҡоролтай AXT партия манифест һеңдереү - SHA-256 өҫтөндә Norito Каноник бәйләнеш кодлау:
...
Ҡоролтай SCCP ярҙамсы һандыҡ шулай уҡ ҡулланыла FastPQ үтә күренмәле арҡыры сылбыр хәбәрҙәр иҫбатлау өсөн. был юлы айырым `irohad` фоновый провер лента. Ул төҙөй FastPQ туранан-тура бер партия SCCP хәбәр иҫбатлау бәйләме һәм манифест, һуңынан асыҡ тикшереү өсөн һөҙөмтәле иҫбатлау төрөү.
...
Ҡоролтай OpenVerify тикшереүсе йөкләмәһе SHA-256 өҫтөндә SCCP хәбәрҙең һуңғы исеме һәм каноникаль FastPQ раҫлаусы һүрәтләүсе:
...
Улар икеһе лә 128-бит хәүефһеҙлек маҡсат һәм ҡулланыу эҙҙәр домен күләме `2^16`. Ҡоролтай Rust V1 транскрипт ҡабатлау коды әлеге ваҡытта Fiat-Shamir проблемаһы байттар менән сығарыла `iroha_crypto::Hash::new` туранан-тура мөрәжәғәт итеү урынына SHA3-256.
