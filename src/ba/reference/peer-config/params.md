...
Консенсус раҫлаусылар ҡулланырға тейеш BLS-Һәр validator өсөн, шулай уҡ беркетеү менән тәьмин итеү [`trusted_peers_pop`](#param-trusted-peers-pop) инеү урыны.

<param-table env="TRUSTED_PEERS">
<template #type>

Яҡындары менән бәйләнгән ҡылдар. `PUBLIC_KEY@ADDRESS` ҡасан P2P адресы билдәле; `PUBLIC_KEY` шулай уҡ ҡабул ителә һәм тиҫтерҙәрҙең адресын ғәйбәттән асыҡларға мөмкинлек бирә.
...
Файл юлы менән ҡул ҡуйылған генезис блогы файҙалы йөкләмәһе `kagami genesis sign`. Булдырылған профилдәрҙә ғәҙәттә был тип яҙырға Norito `.nrt` Файл.
...
Стринг, бер йәки бер нисә комета менән айырылған директиванан тора.Һәр директиваға тейешле максималь вербоситет кимәле булырға мөмкин, был мөмкинлек бирә (мәҫәлән, һайлай) арауыҡ һәм ваҡиғалар тап килә. Iroha түбән эксклюзив кимәлдәр (мәҫәлән, `trace` йәки `info`) һынлыраҡ тип әйтеп була (мәҫәлән: `error` йәки `warn`).
...
`logger.filter` менән берлектә эшләй [`logger.level`](#param-logger-level) Уларҙың береһе лә икенсеһенән өҫтөнлөк бирмәй.
...
- `json`: Сығарылыштар newline-delimited JSON Был системалар менән етештереү өсөн тәғәйенләнгән, унда структуралы бүрәнәләр ҡулланыла JSON Анализ һәм күҙәтеү ҡоралдары менән. JSON сығанаҡ кеше өсөн уҡырға яраҡлылыҡ өсөн оптималләштерелмәгән.
...
- `read_write`: Iroha слайд менән билдәләнгән ваҡытты индерә. [`snapshot.create_every_ms`](#param-snapshot-create-every-ms). Башланғанда, Iroha бар булған фотоһүрәтте уҡып сығара (әгәр булһа) һәм уның блоктарҙы һаҡлап ҡалыу менән яңыртылғанын тикшерә.
- `readonly`: Үҙенсәлекле `read_write` әммә Iroha был фотоһүрәттәрҙе төшөрмәй.
