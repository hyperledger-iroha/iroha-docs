...
Түбәндәге уҡырға ғына бирелгән миҫалдар асыҡтан-асыҡ менән тикшерелде Taira на сайте `https://taira.sora.org`. Мутацион миҫалдар - транзакция өлгөләре: улар реаль кәрәк Taira Хакимиәт, шәхси асҡыс, газ метамәғлүмәттәре һәм улар тапшырылыр алдынан маҡсатлы маршрутҡа кәрәкле оператор билдәләре.
...
Пакеттағы метамәғлүмәттәр исеме `iroha-python`. Яҡшы тип уйламағыҙ PyPI ҡуйыу тере көйгә тура килә Taira селтәр. Интеграция маҡсаттары менән бер үк өҫкө ағымы ревизиянан төҙөлгән тәгәрмәсле йәки сығанаҡ контроле ҡуйыу:

```bash
python -m pip install /path/to/iroha_python-*.whl
```

Әгәр һеҙҙең проект туранан-тура өҫтәрәк эш урыны ҡулланыу, ҡуйырға Python бәйлелектәре һәм ҡулланыу өлгөләрен эшләгәнгә тиклем урындағы киңәйтеү төҙөү `Instruction`, `TransactionDraft`, ҡултамғалау, крипто, SoraFS урындағы ярҙамсылары, GPU ярҙамсылары, йәки тоташтырыу рамка кодэктары. Үҫеш командаһын ҡулланып `python/iroha_python/README.md`, һуңынан тыуған экспорт йөкләмәһенең:
...
Әгәр `create_torii_client` Импорт, әммә `Instruction` йәки `generate_ed25519_keypair` уңышһыҙлыҡтарға дусар, саф Python пакеты бар, әммә тыуған киңәйтеү юҡ.
...
`authority` транзакцияға ҡул ҡуйған иҫәп. `private_key` шул иҫәпкә тап килә, `CHAIN_ID` маҡсатлы селтәр менән тура килергә тейеш, һәм `TX_METADATA` түбәндәге урынды биләүселәр белә тоноп юҡҡа сыға, шуға күрә улар осраҡлы рәүештә тапшырылмай.
...
`Instruction.*` саҡырыуҙар бары тик конструкция инструкцияһы файҙалы йөкләмәләр. `submit()` был нөктәлә SDK килешеүгә ҡул ҡуя, уны Torii, һәм ул кешенең хәле тураһында хыяллана.

## Һалымдар һәм газ {#fees-and-gas}

Яҙыу транзакциялар өсөн түләү метамәғлүмәттәре һәм финансланған түләү актив балансы кәрәк. Taira, түләү активы дәүләт фондынан финанслана һәм транзакция метамәғлүмәттәре `gas_asset_id`. Артабан Minamoto, түләүҙәр реаль аҡса менән түләнә XOR һәм актив ID был селтәрҙең конфигурацияһынан килә.
...
Маршруттар: `/v1/status`, йәмәғәт тиңдәшлек инвентаризацияһы, Sumeragi RBC пробка алыу, узел администраторы мгновенные снимки , һәм Connect ҡушымталар реестры идара итеү асыҡтан-асыҡ булмай Taira тикшереү ваҡытында ҡулланыу. `request_json("GET", "/status")` йәмәғәт узел статусы файҙалы йөкләмәһе өсөн Taira.
...
|Тыуған активтарҙы бикләү | `open_asset_lock`, `drawdown_asset_lock`, `cancel_asset_lock`, `expire_asset_lock`, өҫтәп клиент `*_and_wait` ярҙамсылары |
|Grant/Revocate, SetParameter, Log, Custom, Upgrade, һәм ҡағиҙә булараҡ теркәлгән / теркәлмәгән варианттар | `Instruction.from_json` йәки `TransactionBuilder.add_instruction_json` каноник менән `InstructionBox` JSON |

Шартлы түләүҙәр өсөн, ҡарағыҙ: [Туған активтар иҫәбенә кредит](/ba/blockchain/escrow.md#python-asset-locks). Python хәҙерге ваҡытта беренсе класлы ярҙамсыларҙы дөйөм активтарҙы ябыу өсөн асыҡлай; баҙар һәм аноним конфискация ярҙамы биреүселәр беренсе класлы түгел Python методтар әлегә.
...
Метамәғлүмәттәрҙең мәғәнәһе: JSON- серияға сығарыла. `TransactionDraft`, власть в `TransactionConfig` билдәләнгән маҡсатлы иҫәпкә әйләнә.
...
RWA ярҙамсылары ҡуллана JSON- актив-специфик метамәғлүмәттәр, килеп сығыу һәм контроллер сәйәсәте өсөн сериялаштырыла торған файҙалы йөкләмәләр. `register_rwa` ҡабул итмәй `id` йәки `owner`: йүгереү ваҡыты барлыҡҡа килтерә `RwaId`, һәм транзакция власы башланғыс хужа булып китә.
...
Теркәлеү операцияһы йөкләмәләрен үтәгәндән һуң, ҡулланыу `FindRwas`, `/v1/rwas`, Информация RWA ваҡиға, йәки эҙләүсе маршруты булдырылған табыу өсөн ID:
...
Тулы күсереүҙәр ғәмәлдәге партияла `owned_by` үҙгәрештәр яһай ала.
...
Теркәү, башҡарыу, ҡабатлау үҙгәрештәрен һәм теркәүҙе бөтөрөү - мутациялы операциялар.
...
Ҡулланыу `TransactionDraft` ҡул ҡуйыу алдынан бер нисә инструкция төҙөй торған ҡушымталар өсөн. проект һеҙгә транзакция кимәлендәге параметрҙарҙы һаҡларға мөмкинлек бирә: `ttl_ms`, `nonce`, һәм бер урында метаданмалар, һуңынан бер тапҡыр ҡултамға:
...
Стриминг ярҙамсылары декодлау JSON дефолт буйынса файҙалы йөкләмәләр. `with_metadata=True` әгәр һеҙ кәрәк SSE ваҡиға исеме, идентификаторы, ҡабаттан һынау һылтауы һәм сей файҙалы йөкләмә. `EventCursor` был миҫалдар тере ваҡиғалар көтә, шуға күрә уларҙы тейешле ваҡиға ағым булдырылған һәм әүҙем булған узел менән эшләй.
...
Ҡулланыу `supported_crypto_algorithms()` Дөйөм ярҙамсылары каноник алгоритм билдәләрен ҡуллана һәм Ed25519, secp256k1 өсөн эшләй. ML-DSA, GOST, BLS, һәм SM2 әгәр был алгоритмдар түбәндәгеләр буйынса төҙөлә:
...
`crypto.sm.enabled` тип хәбәр итә, узел ҡабул итәме SM- ғаилә алгоритмдар уның хәҙерге сәйәсәте. шул уҡ реклама SM hash сәйәсәте һәм тиҙләтеү статусы, был файҙалы ҡарар ҡабул итеү өсөн SM2-ҡайһы бер ағымдар:
...
Әгәр узел кәрәкле алгоритмды иғлан итмәй икән, был асҡысты урындағы йәки офлайн эш ағымдары өсөн генә ҡулланығыҙ. Taira иҫәбенә, GOST һәм ML-DSA булғандар SDK криптовалюта ярҙамсылары өҫкө ағымында Python китапханаһы, әммә транзакцияға ҡул ҡуйыу өсөн узел тарафынан иғлан ителмәгән.
...
Ҡоролтай Python SDK ағымды һорарға мөмкин JSON әҙерлек маршруты уның дөйөм Torii ярҙамсы үтенесе:
...
Python типталған Kagemusha тулыландырыу йәки ҡайтарыу архив төҙөүселәр асҡармай. типталған ҡулланырға Swift йәки JVM ҡануниәт төҙөү өсөн аҡса янсығы V4 архивтар, һуңынан уларҙы ярҙам иткән Кагемуша аша тапшырып һорау алыу үткәрә Torii клиент.
...
## SoraFS, UAID, һәм Kaigi Ярҙамсылар {#sorafs-uaid-and-kaigi-helpers}
...
## Norito RPC һәм GPU Ярҙамсылар {#norito-rpc-and-gpu-helpers}

Ҡулланыу `NoritoRpcClient` ҡасан һеҙ инде Norito байттар һәм бинар саҡырырға кәрәк Torii тамамлау пункты. миҫалға элекке транзакция өлгөһөнән ҡул ҡуйылған конверт кәрәк:
...
- аккаунт адресы, бөтә алгоритм ҡултамғалау ярҙамсылары, күп һанлы хаш буйынса ҡайтыуҙар, SM2, GOST, ML-DSA, BLS, һәм серле асҡыстар менән эш итеү
- URIs, сессиялар, кадрҙар, шифрлау ярҙамсылары һәм реестр администраторы тоташтырыу
- идара итеү, эшләү ваҡытын камиллаштырыу, Sumeragi, node-admin, SoraFS, UAID, һәм Kaigi Һөйәктең был үҙенсәлектәрҙе асып биреүсе һуңғы нөктәләрҙе ҡаплауҙар
