...
- թվային ակտիվը հաշվառման կողմից պահվող ֆունգիբալ մնացորդ է
...
Օգտագործեք RWAs այն դեպքում, երբ գլխավոր գրքում պետք է ներկայացվի միայն ֆունգիբալ մնացորդայինության փոխարեն որոշակի ոչ շղթայական խմբաքանակ:
...
`RegisterRwa` չի ընդունում զանգահարողի կողմից ընտրված `id` դաշտը եւ չի ընդունում `owner` դաշտը: Գործարքի լիազոր հաշիվը դառնում է սկզբնական `owned_by` հաշիվը, իսկ վազման ժամանակը առաջացնում է նպատակային տիրույթում գտնվող `RwaId` դոմենը:
...
Դիմումները պետք է պահեն իրենց բիզնեսի նույնականացողը `primary_reference` կամ `metadata` հասցեներում, այնուհետեւ հայտնաբերեն `RwaId`-ը, որը ստեղծվել է `RwaEvent::Created`, `FindRwas`, `/v1/rwas` կամ գործարքի commit հետո սահմանված հետազոտողի երթուղինից:
...
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
|`RegisterRwa` |Ստեղծեք ID ստեղծված բողոքը տիրույթում, գործարքի լիազոր հաշիվը դառնում է `owned_by`: |
|`TransferRwa` |Տեղափոխեք քանակությունը մեկ այլ հաշիվ: Ամբողջական փոխանցումը կարող է փոխել `owned_by`. Կիսական փոխանցում ստեղծում է առանձին երեխա խմբաքանակ, որը ստեղծվում է ID. |
|`HoldRwa` |Պահեստային քանակություն: Պահանջվում է կոնֆիգուրացված վերահսկողություն եւ `hold_enabled`. |
...
|`RedeemRwa` |Հաշվարկից մշտապես հանել քանակությունը: Գնորդը կամ վերահսկիչը կարող է այն ներկայացնել, երբ `redeem_enabled` ճիշտ է: |
|`MergeRwas` |Միացրեք նույն տիրույթով ծնողական խմբերի քանակները եւ կազմեք արտադրված երեխայի խմբի: |
|`ForceTransferRwa` |Փոխեք քանակությունը վերահսկողության հոսքի միջոցով: Պահանջվում է կոնֆիգուրացված վերահսկողություն եւ `force_transfer_enabled`. |
|`SetRwaControls` |Փոխանակեք խմբաքանակի վերահսկողության քաղաքականությունը: Պահանջում է սեփականատեր կամ վերահսկող: |
|`SetKeyValue<Rwa>` / `RemoveKeyValue<Rwa>` |Թարմացնում է խմբաքանակի մետատվյալները։ Պահանջվում է սեփականատեր կամ վերահսկիչ, իսկ սառեցված խմբաքանակների համար՝ վերահսկիչ։ |
...
- վավերագրերի եւ իրավական փաստաթղթերի բովանդակության հեշեր
...
Կառավարիչների հաշիվներն ու դերերը կարող են կատարել միայն համապատասխան բուլյան դրոշներով հնարավորություն տրված գործողությունները: Ներկայիս վերահսկողության օգնական բեռը պարունակում է կառավարիչի ինքնությունը եւ գործառույթի դրոշները: Տրանսֆերի թույլատրելի ցուցակները եւ տեղադրված `transfers` կանոնները դուրս են այս օգնական բեռնից.

## Հարցումներ, իրադարձություններ եւ APIs {#queries-events-and-apis}

Օգտագործել [`FindRwas`](/hy/reference/queries.md#assets-nfts-and-rwas) գրանցված ցուցակ RWA Հավելվածները, որոնք կարիք ունեն կենդանի թարմացումների, կարող են բաժանորդագրվել [`Rwa` տվյալների իրադարձություններ](/hy/blockchain/filters.md#data-event-filters) ստեղծված, սեփականատեր փոխված, բաժանված, միավորված, վաճառում, սառեցված, չսառեցված պահված, թողարկված, ուժային փոխանցում, վերահսկողության փոփոխություն եւ մետադատա:

Torii-ն տրամադրում է շղթայի վիճակի `/v1/rwas` և `/v1/rwas/query` route-երը, ինչպես նաև `/v1/explorer/rwas` և `/v1/explorer/rwas/{rwa_id}` explorer route-երը, երբ այդ route family-ն միացված է։ Գեներացված client-ները պետք է նախընտրեն կենդանի [`/openapi.json`](/hy/reference/torii-endpoints.md#common-endpoints) փաստաթուղթը՝ հանգույցի տրամադրած պատասխանի ճշգրիտ կառուցվածքի համար։
...
Օգտագործեք նախագիծ, երբ մի բիզնես գործողություն պետք է դառնա մեկ ստորագրված գործարք: Գործարար վոմի թիվը գնում է `primary_reference`; գլխավոր գրասենյակը ID ստեղծվում է այն բանից հետո, երբ գործարքը commit է:
...
Հանձնեք `ReleaseRwa`՝ առանց շղթայի ընթացակարգի հաջողությունից հետո.
...
### Մարման կամ շրջանառությունից հանելու քանակը {#redeem-or-retire-quantity}

Ներկայացրեք `RedeemRwa` այն բանից հետո, երբ ներկայացված արտագաղթային ակտիվը մատակարարվում է, սպառվում, հեռանում կամ այլ կերպ հանվում է շրջանառությունից: Սա մշտապես զրկում է ներկայացված քանակությունը խմբաքանակից. խմբաքանակը պետք է ունենա `redeem_enabled`. ստորագրողը պետք է լինի սեփականատեր կամ վերահսկող:
...
Ներկայացրեք `FreezeRwa` այն դեպքում, երբ օֆ-շղթակից վերանայման արդյունքում պետք է արգելափակվի սովորական սեփականատերերի գործունեությունը: ստորագրողը պետք է լինի վերահսկողություն իրականացնող անձը: Խումբը պետք է ունենա `freeze_enabled`.
...
Վերանայումը անցնելուց հետո ներկայացրեք `UnfreezeRwa`:
...
Ներկայացված գումարը փոխհատուցել՝ առանց շղթայի հաշվետվությունից հետո:
...
### Ածխածնային վարկի չեղարկում {#carbon-credit-retirement}

Ներկայացրեք `RedeemRwa` ՝ պնդվող ածխածնի վարկանիշները շրջանառությունից հանելու համար: Պահպանեք առանց շղթայի վկայականը կամ գրանցման ապացույցը մետադատաներում.
...
Python գործարքի ամբողջական օրինակի համար տես [Իրական աշխարհի ակտիվներ](/hy/guide/tutorials/python.md#real-world-assets):
...
- [Հարցումներ](/hy/reference/queries.md#assets-nfts-and-rwas)
