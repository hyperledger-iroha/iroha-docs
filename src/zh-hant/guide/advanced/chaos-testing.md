...
使用 Izanami進行產前彈性檢查,迴歸複製和共識調整.不要指向生產網絡:該工具旨在擁有它啓動的同行,包括同行重啓,存儲 wipes,臨時可信任同行分區和本地 CPU 或磁盤壓力.
...
在用戶配置目錄中, Izanami 仍然保留 TUI 和 CLI 設置.首次發佈的文件有一個明確的 V1 佈局字節;預發或其他未版本的設置被拒絕,並且應該重建而不是遷移.在重新使用當前的配置文件之前,請查看顯示的設置.
...
|領導人隔離|使用一個錯誤的同行,只有網絡分區錯誤; Izanami 遵循 Sumeragi 的領先遠程測量.|
