...
密碼可保護維運主控臺、機密資訊儲存庫、備份與本機金鑰檔案。密碼只是其中一項控制措施；它必須搭配安全的金鑰保管、存取控制，以及在可用時採用多因素驗證。
...
- 伺服器端密碼驗證資料應使用加鹽且記憶體密集的密碼雜湊函式，引數則應符合部署需求。
...
用來解鎖私鑰的密碼，無法讓已暴露的私鑰副本恢復安全。若懷疑私鑰遭到暴露，請遵循部署的金鑰替換或撤銷程式。
