...
私鑰可以授權其授權主體所允許的每項操作。切勿分享私鑰；種子材料、復原秘密、Bearer 權杖及匯出的金鑰檔案，都必須以同等謹慎的方式保護。

請在生產環境上線前選定保管設計。該設計必須符合風險價值、帳戶控制者政策及部署的復原流程。

## 定義保管邊界 {#define-the-custody-boundary}

- 維護每個授權主體、公開金鑰、演演算法、環境、用途、保管人、儲存位置、備份及取代程式的清冊。
...
在本機開發、受控測試或安全的保管交付中，可將金鑰匯出至許可權受限的檔案。在支援的 Unix 平臺，請使用 `kagami` 產生新的金鑰目錄：
...
私鑰檔是未加密的匯出物。不得將它放入原始碼控制、共用資料夾、日誌、工單、聊天或建置成品。請將生產金鑰匯入經核准的保管邊界，再依部署程式移除匯出物。切勿在生產環境重複使用開發金鑰。
...
所選整合支援不可匯出屬性時，應讓金鑰材料保持不可匯出。確認保管系統支援 Iroha 授權主體所需的演演算法及簽署操作。

靜態加密只能保護已儲存的副本。未經授權的程式或操作員一旦取得解密後的位元組，靜態加密便無法再保護金鑰。請強化主機、限制執行階段存取，並監控簽署活動。

## 保護簽名工作流程 {#protect-signing-workflows}

- 使用具名的操作員身分、強式驗證，以及經稽核的簽署系統存取方式。
- 不得在命令列引數、Shell 歷程、環境傾印、程式清單、當機報告或應用程式日誌中放置原始金鑰。
- 僅為必要操作解鎖簽署器，使用後立即關閉工作階段或讓其到期。
- 核准前顯示授權主體、網路、指令、資產及費用。
- 特權或高價值交易必須明確確認。
- 若自訂使用者端整合可委派簽署，應避免讓原始私鑰進入瀏覽器頁面及一般用途應用程式程式。

純文字使用者端組態僅適合本機開發與受控測試。生產整合應透過經核准的保管邊界取得簽章。標準 Iroha CLI 會從使用者端組態讀取私鑰，且未提供通用外部簽署器介面卡。自訂使用者端可建立交易承載雜湊，並附加由外部簽署器產生的簽章。
...
7. 如何移除舊金鑰的許可權，以及封存或銷毀其副本
8. 事後如何驗證網路與相依應用程式

::: warning

加密或更換密碼都無法讓已遭複製的私鑰重新變得安全。若懷疑金鑰已暴露，請立即停止使用，並遵循經核准的取代或撤銷程式。
