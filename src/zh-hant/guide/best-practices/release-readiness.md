...
在將 Iroha 應用程式或網路變更推進到下一階段前，先在足以暴露相關風險的最小環境中驗證其行為，再審慎透過共享測試網與正式環境的關卡。
...
## 共享測試閘道器卡 {#shared-testnet-gate}

- 使用 Taira 或其他共享測試網，驗證端點行為、費用、帳戶資金、延遲，並演練維運程式。
- 即時測試網寫入應保持為選擇啟用，避免一般測試執行依賴網路可用性或耗用測試網資金。
- 提交每筆即時測試交易前，確認簽署者資金、費用資產中繼資料、許可權與預期狀態。
...
- 在發布時段前審查許可權、費用贊助、速率限制、監控、備份狀態與回滾條件。
- 高影響寫入必須具備書面的交易或移轉計畫。
...
- 監控涵蓋輸送量、延遲、佇列深度、拒絕、檢視變更與相關業務事件。
- 維運人員備有預期失敗模式的操作手冊。
- 安全審查已涵蓋金鑰保管、許可權、網路暴露範圍與自動化許可權。
