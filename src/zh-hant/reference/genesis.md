...
在當前 Iroha 3 工作流程，一個 `genesis.json` 清單描述了第一個
網路啟動時將套用的事務和參數。

分發給同級的簽章工件是 Norito-編碼的 `.nrt` 文件
生產者 `kagami genesis sign`.
...
- `chain` 對於鏈標識符
- `executor` 對於可選的執行程序升級字節碼路徑
- `ivm_dir` 為了 IVM 觸發器和升級使用的函式庫
- `consensus_mode` 對於清單所公佈的初始模式
- `transactions` 用於有序參數更新、指令、觸發器和拓撲
- `crypto` 對於初始加密快照
...
對於公眾 SORA Nexus 資料空間， `npos` 是預期的共識模式。
其他 Iroha 3 根據目標，部署可以使用許可或 NPoS
輪廓。
...
`kagami genesis sign` 從清單中讀取創世公鑰並使用
來自所有者持有的單連結常規文件的私鑰，以生成
可部署的簽名區塊。該文件必須包含一個規範的私鑰
多哈希後跟換行符； Kagami 拒絕符號連結和模式其他
比 `0600`. 命令列不接受原始私鑰。結果
是對等方應從其配置中引用的文件。
