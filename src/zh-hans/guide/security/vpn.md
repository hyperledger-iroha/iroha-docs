...
一个 VPN 不取代 Iroha 对等节点密钥,帐户密钥,权限,防火墙规则,监控或安全密钥存储.将其视为一个层部署界限: VPN 缩小网络可访问性,而 Iroha 配置和治理决定哪些对等节点和账户是值得信赖的.
...
- 数据,日志, SSH 或其他管理端点必须在私营运营商网络中保持
- 测试或阶段化网络应类似于生产访问控制,而不暴露公共端点

每次部署都不需要 VPN.公共网络可能会故意通过公共门户,负载平衡器或反转代理来暴露 Torii.即使在这种情况下,尽可能地将验证器对等流量和管理端点放在一个受限制的网络上.
...
对于私人验证器网格,给每个验证器一个稳定的 VPN 地址或私人 DNS 名称.配置对等节点,使其广告的对等节点对对等节点地址可以从其他验证器通过该网络访问:
...
使用分配给当前对等节点的地址 `network.address` 和 `network.public_address`. 每个对等节点应列出相同的可信的对等节点的身份,但有自家可访问的地址 VPN 路线表.

客户端和 CLI 配置应指向通过 VPN 或通过控制的内部网关可访问的 Torii 端点:

```toml
torii_url = "http://10.20.0.11:8080"
```

如果 Torii 必须在 VPN 外使用,请将其置于提供 TLS,身份验证,速度限制和记录的反转代理或负载平衡器后面.避免直接向公共互联网暴露原始的对等节点端口或管理端点.
...
默认拒绝规则比宽泛的允许规则更易审计。当新的对等节点加入网络时，应将 VPN 成员资格、防火墙允许列表和 Iroha 可信对等节点配置作为一项协调变更一并更新。

## 运营检查列表 {#operational-checklist}

- 选择一个经过审计和积极维护的 VPN 实现,如 WireGuard,IPsec或由组织批准的管理私人网络.
- 用每个主机和运营商的独特 VPN 凭证.不要在验证器之间共享 VPN 密钥.
- 保持 VPN 凭证与 Iroha 私钥和创世签字材料分开.
- 监测 VPN 延迟,数据包丢失,重新连接和路线变化.共识对持续网络不稳定性很敏感.
- 测试有效的 MTU.包碎可以看起来像间歇性对等节点或 Torii 故障.
- VPN 范围允许达到同等, Torii,指标, SSH 和备份端点的文件.
...
- [对等节点管理](/zh-hans/guide/configure/peer-management.md)
- [对等节点配置参考](/zh-hans/reference/peer-config/index.md)
