...
Izanami是上游 Iroha 工作空间中的混沌网管.它启动一次性本地 Iroha 集群,提交可配置的工作负载,并将故障注入选定的对等节点中,以便运营商可以检查网络是否在受控故障下继续取得进展.

使用 Izanami进行产前弹性检查,回归复制和共识调整.不要指向生产网络:该工具旨在拥有它启动的对等节点,包括对等节点重启,存储 wipes,临时可信任对等节点分区和本地 CPU 或磁盘压力.
...
必须明确允许二进制器创建和操纵网络对等节点.通过 `--allow-net` 每次非 TUI 运行,或启用 `allow_net` 在 TUI 中.
...
记录命令,种子, Iroha 提交,对等节点数量,故障对等节点数值,工作负载配置文件,目标 TPS 和延迟门.如果没有这些值,另一个操作员无法重复相同的失败模式.
...
|个人资料|用它来|备忘录|
| -------- | -------------------------------------------------- | -------------------------------------- |
|`stable`|长时间的浸泡运行和可复制性性能检查|喜欢安全的操作指南|
|`chaos`|失败路径覆盖率|包含故意无效的操作指南|
...
|错误|CLI 旗|它所做的一切|
| ------------------------ | ------------------------------------------ | ------------------------------------------ |
|崩和重新启动|`--fault-enable-crash-restart`|对等节点流程损失和恢复|
...
|网络分区|`--fault-enable-network-partition`|暂时的可信对等节点隔离|
...
|停止和恢复|使用大量的故障对等节点群体与崩/重新启动|
|领导人隔离|使用一个错误的对等节点,只有网络分区错误; Izanami 遵循 Sumeragi 的领先远程测量.|

每次应固定一个变量。如果在同一次运行中同时更改对等节点数量、工作负载配置文件、故障窗口和 TPS，结果将难以解释。
...
- 提交,接受,拒绝和截止期的交易
- 排队深度,排队和端点反压力
- 查看变化,恢复路径,缺失的区块和缺失的法定人数证书
- 签署的 RS16 可用性后期记录,待定会议和延迟共识流量.
- CPU,存储器,磁盘和网络度在运行对等节点的主机上
...
每个区块都应发出 `block validation timings` 与 `stateless_ms`, `execution_ms` 和 `total_ms`.在改变共识计时器之前,将这些时间与 p95 区块间隔,视图变换计数器和队列压力进行比较.

## 解释结果 {#interpreting-results}

当所有选择的对等节点继续提交区块时,对运行进行健康处理,后载不增长而无限,并且设置窗口结束后故障停止导致新的恢复活动.
