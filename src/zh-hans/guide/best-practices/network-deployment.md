...
将 Iroha 网络视为一个协调系统. 在网络启动之前,验证者必须同意创世学,拓扑,可信赖的对等节点和共识相关的配置继续完成区块.

## 环境分离 {#environment-separation}

- 为本地开发,共享测试网络,阶段化和生产提供单独的配置捆绑.
- 在生产中不要再使用 localnet 或 Taira 密钥材料.
- 保持对等节点配置,客户端配置,签署的起源,脚本和部署笔记作为一个版本的发布构件.
- 在存储库和部署模板之外保存私钥.

见 [网络部署关键](/zh-hans/guide/configure/keys-for-network-deployment.md).

## 创世纪和拓扑 {#genesis-and-topology}

- 让每个验证者都使用相同的签署创世交易,可靠的对等节点集,拓,在个人资料要求时,验证者拥有权证明.
- 用至少四个验证器来实现最小的拜占庭错误耐受性部署.
- 在能力规划中,与观察者分别进行验证. 观察者不会投票,提出或收集信息,但它们仍然消耗存储,区块同步和网络带宽.
- 把创世,执行器和拓变化视为协调的迁移而不是单对等节点编辑.

参见 [Genesis](/zh-hans/reference/genesis.md), [对等节点管理](/zh-hans/guide/configure/peer-management.md)和 [绩效和指标](/zh-hans/guide/advanced/metrics.md#node-count-and-quorum).

## Torii 和网络访问 {#torii-and-network-access}

- 当它暴露在主机或私人网络之外时,将 Torii 置于反向代理或防火墙后面.
- 在部署需要时,终止 TLS,并在边缘应用基本身份验证,速度限制和要求尺寸控制.
- 只有环境所需的端点才可公布.运营商和远程测量路线应比公共仅阅读路线更为有限.
- 当对等节点不应直接接受远程流量时,将听者地址绑定到主机本地接口.

查看 [Torii 端点](/zh-hans/reference/torii-endpoints.md)和 [虚拟私人网络](/zh-hans/guide/security/vpn.md).
