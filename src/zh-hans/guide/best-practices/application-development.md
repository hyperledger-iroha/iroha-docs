...
- 在正常的网络动荡下,一个非常短的寿命可能会过期,而一个非常长的寿命可以使复制提交变得更加难以推理.
- 仅在重复交易时使用 `nonce = true`.对于无权的业务运营,存储和再利用应用请求 ID,以便重新测试可以追踪.
...
- 对于订阅和触发器,更喜欢狭窄的过器.宽度过器对诊断有用,但可增加不必要的执行和客户端处理.
- 保持仅阅读式烟雾检查与签署的交易测试分开,以便更容易诊断终点可用性.

查看[查询](/zh-hans/blockchain/queries.md), [事件](/zh-hans/blockchain/events.md)和 [过器](/zh-hans/blockchain/filters.md).
...
- 在任何代理提交实时测试网交易之前,需要一个交易计划.该计划应该命名网络,权威,指令,费用资产,航班前阅读,预期结果和重复尝试行为.
