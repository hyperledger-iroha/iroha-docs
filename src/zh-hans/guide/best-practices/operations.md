...
- 在同一事件或基准文物集中保存状态快照,指标剪辑,日志和部署配置.
- 警示随着排队的持续增长,意外的拒绝峰值,块高度停滞不前,视角变动和同行健康改变.

查看 [绩效和指标](/zh-hans/guide/advanced/metrics.md).

## 跑本 {#runbooks}

- 为同行重新启动, Torii 降级,关键妥协,许可错误,费用赞助商耗尽,排队和网络分区症状编写运行簿.
- 在写作操作之前,包括精确的仅阅读检查,特别是对同行注册,授权和参数更改.
...
- 根据部署所需的恢复点,备份同行存储. 在非生产主机上验证恢复.
- 保持签署的起源,释放元数据,同行配置和关键存储记录可恢复,即使没有验证器主机.
- 记录恢复程序是否从生成中重建,从快照中恢复,或者用新身份取代失败的同行.
- 在生产事件中,不要第一次检验恢复程序.

## 变革管理 {#change-management}

- 处理链上配置的变化作为需要审查,飞行前阅读,授权和变更后验证的交易.
- 推出兼容性计划和反弹决定点的同行二进制升级.
- 避免在同一维护窗口中改变同行拓,共识时间和应用工作负载,除非迁移计划要求这样做.
- 记录交易哈希和区块高度,以进行操作变化.
...
- 将接受的吞吐量与承诺的吞吐力和队列深度进行比较. 如果提交的 TPS 超过承诺的 TPS,排队增长,网络已经超越了其可持续范围.
