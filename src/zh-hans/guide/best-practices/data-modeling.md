...
- 在配置和测试中使用规范帐户和域名标识符. Iroha 名字在规范解析后对案例敏感.
...
- 避免仅在元数据中编码承载价值的状态。资产和 NFTs 提供元数据所不具备的生命周期事件、转移语义和权限检查。
- 在将资产暴露于应用程序之前,定义准确性,供应政策,发行人责任和销毁/铸造权限.
...
- 扩展权限令牌到满足工作流程的最小对象.
- 作为高影响权限,铸造、销毁,对等节点管理,执行器更改,触发管理和元数据突变的权限.
- 为临时权限添加明确的撤销和轮换程序。
...
- 页面化广泛的结果集,避免使用者界面需要无限制的账本范围扫描正常操作.
