---
translation_locale: zh-hans
translation_source: /get-started/index.md
translation_source_hash: 0c452291828ed262a2e9094c30def83e07f0b7b70060d47bbd0d32160559ac40
translation_status: machine-validated
translation_engine: nllb-200-ct2
---

# Iroha 3 {#iroha-3}

Iroha 3 是首次发布的 Hyperledger Iroha 支持自主托管的网络和多数据空间 SORA Nexus 基于一个确定性代码基础的部署模型.

在高水平上, Iroha 3 结合:

- 确定性执行和存储
- Iroha 移动智能合同虚拟机 (IVM)
- Norito 作为规范序列化格式
- Torii 用于客户端,运营商和应用程序面向 APIs
- Sumeragi 与强制性的数据可用性和可靠的广播达成一致
- 没有域名的可信账户,具有单独绑定的人类可读的号

## 快速开始 {#quickstart}

如果您从零开始,请按顺序关注以下页面:

1. [安装 Iroha 3](/zh-hans/get-started/install-iroha.md)
2. [启动 Iroha 3](/zh-hans/get-started/launch-iroha.md)
3. [通过 CLI](/zh-hans/get-started/operate-iroha-via-cli.md)运行 Iroha 3
4. [连接到 SORA Nexus 数据空间](/zh-hans/get-started/sora-nexus-dataspaces.md)
5. [赞助商私人数据空间费用](/zh-hans/get-started/private-dataspace-fee-sponsor.md)

## SDKs {#sdks}

目前在本站记录的 SDK 入口点是:

- [Rust](/zh-hans/guide/tutorials/rust.md)
- [Python](/zh-hans/guide/tutorials/python.md)
- [JavaScript / TypeScript](/zh-hans/guide/tutorials/javascript.md)
- [Kotlin,Android 和Java](/zh-hans/guide/tutorials/kotlin-java.md)
- [Swift 和iOS](/zh-hans/guide/tutorials/swift.md)

## 运营商参考 {#operator-references}

在运行网络时,您最经常使用的页面是:

- [与 Iroha 二进制](/zh-hans/reference/binaries.md) 合作
- [创世记引用](/zh-hans/reference/genesis.md)
- [Torii 端点](/zh-hans/reference/torii-endpoints.md)
- [连接到 SORA Nexus 数据空间](/zh-hans/get-started/sora-nexus-dataspaces.md)
- [赞助商私人数据空间费用](/zh-hans/get-started/private-dataspace-fee-sponsor.md)
- [兼容性矩阵](/zh-hans/reference/compatibility-matrix.md)

## 了解更多 {#learn-more}

- [Iroha 源存储库](https://github.com/hyperledger-iroha/iroha)
- [文件来源存储库](https://github.com/hyperledger-iroha/iroha-docs)
