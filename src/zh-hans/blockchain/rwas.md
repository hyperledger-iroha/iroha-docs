...
使用 RWAs 如果账本需要代表特定的链外分数,而不是仅仅是可结的余额.

## RWA 分数 {#rwa-lot}

一批 RWA 含有:

- `id`: 产生的规范 RWA 标识符,显示为 `<hash>$<domain>`
- `owned_by`:目前持有物品的账户
...
- `is_frozen`和`held_quantity`:运行时强制执行的生命周期状态

保持连锁有效载荷紧. 在 WSV 之外存储大型法律文件,检查报告和审计捆绑,然后在 RWA 元数据中放一个摘要, URI, SoraFS 路径或明确引用.

## 标识符 {#identifiers}

`RegisterRwa`不接受调用者选择的 `id`,也不接受`owner`字段.交易授权主体成为初始的 `owned_by`账户,运行时在目标域中产生`RwaId`.
...
应用程序应在 `primary_reference` 或 `metadata` 中存储其业务标识符，然后在交易提交后，通过 `RwaEvent::Created`、`FindRwas`、`/v1/rwas` 或 explorer route set 找到生成的 `RwaId`。
...
|行动|实施行为|
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
|`RegisterRwa`|在一个域内创建生成-ID 批量;交易授权主体成为 `owned_by`. |
...
|`FreezeRwa`|阻止普通所有者操作。需要已配置的控制器，并且 `freeze_enabled` 已启用。 |
|`UnfreezeRwa`|重新启用普通所有者操作。需要已配置的控制器，并且 `freeze_enabled` 已启用。 |
|`RedeemRwa`|永久从流通量中扣除数量。当 `redeem_enabled` 为 true 时，所有者或控制器可以提交。 |
...
当前代码中没有 `UnregisterRwa` instruction。当所代表的数量已交付、消耗、结算或以其他方式退出流通时，请使用 `RedeemRwa` 让链下批次退役。
...
Torii 将链状态路线,如 `/v1/rwas`和`/v1/rwas/query`,以及当该路线家族被启用时的探索者路线加上 `/v1/explorer/rwas`和 `/v1/explorer/rwas/{rwa_id}`.生成的客户端应该更喜欢通过节点暴露的确切响应形状而使用现场 [`/openapi.json`](/zh-hans/reference/torii-endpoints.md#common-endpoints)文档.
...
当一项业务操作应形成一笔已签名交易时，请使用草稿。业务收据编号放入 `primary_reference`；交易提交后才会生成账本 ID。
...
在交易提交后,生成列表 RWA IDs.链状态路线暴露了规范的 IDs;使用事件或探索者详细路线当你需要匹配一个 ID 回到 `primary_reference` 或元数据时:
...
### 赎回或注销数量 {#redeem-or-retire-quantity}

在所代表的链外资产交付、消耗、注销或以其他方式从流通中移除后提交 `RedeemRwa`.这将永久减少批次中所表示的数量.批次必须启用`redeem_enabled`.签署者必须是所有者或控制者.
...
### 碳信用注销 {#carbon-credit-retirement}
...
结合两个离链的位置.父母必须在同一领域,使用相同的数量规格.运行时产生孩子的分数 ID.
...
对于 Python 交易的完整例子,请参见[现实世界资产](/zh-hans/guide/tutorials/python.md#real-world-assets).
