...
სააგენტო Python SDK სამუშაო სივრცეში არის `iroha-python`. პირველი Iroha 3
გათავისუფლება მიზნები მიმდინარე Torii და Norito ზედაპირები. შეფუთვის ვერსია დააჭირეთ
ან თქვენი ინტეგრაციის მიერ გამოყენებული წყარო რევიზიონი, რათა SDK და კვანძები დარჩნენ
იგივე ფორმატის რევიზია.

ქვემოთ მოცემული მხოლოდ წაკითხვის მაგალითები შეამოწმეს საზოგადოებრივ Taira დაწვრილებით
`https://taira.sora.org`. მუტაციური მაგალითებია ტრანზაქციის შაბლონები: ისინი
საჭიროებს რეალურ Taira უფლებამოსილება, კერძო გასაღები, გაზის მეტა მონაცემები და ნებისმიერი ოპერატორი
ტოქენები, რომლებიც მიზნობრივი მარშრუტისთვის საჭიროა მათი წარდგენის წინ.

გამოიყენეთ მაგალითები ამ რიგში:

| სცენა | წასვლა საზოგადოების წინააღმდეგ Taira? | რა გჭირდება |
| --- | --- | --- |
| მხოლოდ წაკითხული კლიენტის ზარები | დიახ. | Python პაკეტი და ქსელის წვდომა |
...
| მუტაციის ტრანზაქციები და მომსახურების ზარები | მხოლოდ საკუთარი დაფინანსებული ანგარიშით | ხელისუფლების ანგარიში, კერძო გასაღები, ჯაჭვი ID, საფასურის მეტა მონაცემები, საფასურის აქტივების ბალანსი და მარშრუტის ტოკენი |
| დაკავშირება ჩარჩო კოდეკები, კრიპტო და GPU დამხმარეები | მხოლოდ ადგილობრივი | ადგილობრივი გაფართოება; GPU დამხმარეებსაც სჭირდებათ CUDA-მძლავრი უკანასკნელი |

## დამონტაჟება {#install}

პაკეტის მეტა მონაცემების სახელწოდება `iroha-python`. არ მიიღოს unpinned PyPI
დამონტაჟება შეესაბამება ცოცხალ Taira ქსელი. დაამონტაჟეთ ბორბალი ან წყარო checkout რომ
აგრეთვე, თქვენი ინტეგრაციის მიზნები:

```bash
python -m pip install /path/to/iroha_python-*.whl
```

თუ თქვენი პროექტი უშუალოდ მოიხმარს სამუშაო სივრცეს, დააინსტალირეთ Python
დამოკიდებულებები და აშენოს მშობლიური გაფართოება სანამ run მაგალითები რომ გამოიყენოს
`Instruction`, `TransactionDraft`, ხელმოწერა, კრიპტო, SoraFS ადგილობრივი დამხმარეები, GPU
დამხმარეები, ან Connect ჩარჩო კოდეკები. გამოიყენეთ ყალიბება ბრძანება upstream
`python/iroha_python/README.md`, შემდეგ დაამოწმეთ, რომ ადგილობრივი ექსპორტის ტვირთები:
...
თუ: `create_torii_client` იმპორტი, მაგრამ `Instruction` ან
`generate_ed25519_keypair` წარუმატებლობა, წმინდა Python პაკეტი ხელმისაწვდომია, მაგრამ
ნეივტური გაფართოება არ არის.

## სწრაფი დასაწყისი {#quickstart}

დაიწყეთ საჯარო, მხოლოდ წაკითხვისთვის Taira საბოლოო წერტილები:
...
## საერთო კონსტრუქცია {#shared-setup}

გამოიყენეთ ეს პარამეტრი მუტაციური შაბლონებისთვის. შეცვალეთ თითოეული ადგილის დამჭერი
Taira უფლებამოსილება, კერძო გასაღები, ტოქენი და აქტივი/ანგარიშა IDs თქვენი განთავსებიდან
წინა წარდგენამდე.

`authority` არის ანგარიში, რომელიც ხელს აწერს ტრანზაქციას. `private_key` უნდა შეესაბამებოდეს
ეს ანგარიში, `CHAIN_ID` უნდა შეესაბამებოდეს მიზნობრივი ქსელს და `TX_METADATA` უნდა
მოიცავს ქსელის მიერ მოსალოდნელი საფასურის ველებს. ქვემოთ მოცემული ადგილების მფლობელები არიან:
მიზანმიმართულად არასწორია, ამიტომ ისინი შემთხვევით არ არის წარმოდგენილი.
...
`Instruction.*` ზარები მხოლოდ კონსტრუქციული ინსტრუქციის სასარგებლო ტვირთების. `submit()` ეს არის
წერტილი, სადაც SDK ხელს აწერს ტრანზაქციას, უგზავნის მას Torii, და ელოდება
სტატუსი.

## გადასახადები და გაზი {#fees-and-gas}

გადასახადების დაფინანსება Taira,
საფასურის აქტივი ფინანსდება საჯარო ნაღდიდან და ტრანზაქციის მეტა მონაცემები უნდა იყოს
მოიცავს `gas_asset_id`. დაწყებული Minamoto, გადასახადები იხდის რეალში XOR და აქტივი
ID მოდის ქსელის კონფიგურაციიდან.

საფასურის მეტატალღები შედის ტრანზაქციაზე და არა ინდივიდუალურ ინსტრუქციებზე.
`submit()` დამხმარე ზემოთ დამაგრებული `TX_METADATA` თითოეულ ტრანზაქციას, რომელსაც ის ქმნის:
...
წერილების გაგზავნამდე, დარწმუნდით, რომ ავტორიტეტის ანგარიშს აქვს საკმარისი საფასური
მფლობელობა. ზუსტი ნაბანები და მფლობელი ID ქსელის სპეციფიკური; ეს არის Taira
ფორმა:
...
ფანქარი ბრუნავს ბეტონს. `asset_id` გამოყენება ბალანსის შემოწმებისთვის.
`gas_asset_id` მეტა მონაცემთა ველში გამოიყენება საფასურის აქტივის განსაზღვრა ID.

შეინახეთ განაცხადის მეტა მონაცემები ცალკე საფასურის მეტა მონაცემებისგან რუკების გაერთიანებით
როდესაც შენახავთ ტრანზაქციას:
...
თუ ფასიანი მეტა მონაცემები გამორიცხავთ, გამოიყენეთ არასწორი საფასურის აქტივი ან ხელი მოაწერეთ არაფინანსებულ
ანგარიში, რეალური ქსელი უნდა უარყოს ოპერაცია მაშინაც კი, თუ ინსტრუქცია
სასარგებლო ტვირთი სხვაგვარად ძალაშია.

## Taira-გამოწმებული მხოლოდ წაკითხვის ზარები {#taira-checked-read-only-calls}

ეს ზარები წარმატებით დაუბრუნდა საზოგადოებას Taira:
...
მარშრუტები, როგორიცაა: `/v1/status`, საჯარო თანატოლთა ინვენტარი, Sumeragi RBC ნიმუშების აღება, კვანძი
ადმინის გადაღებები და Connect აპლიკაციის რეგისტრის ადმინისტრაცია არ იყო საჯაროდ
ხელმისაწვდომია Taira ჩექის დროს. `request_json("GET", "/status")` სამედიცინო
საჯარო კვანძის სტატუსის სასარგებლო ტვირთი Taira.

## ინსტრუქციის მშენებლები {#instruction-builders}

სააგენტო SDK გამოყოფს ტიპირებულ მშენებლებს ყველაზე გავრცელებული ინსტრუქციის ოჯახებისთვის და
JSON გაქცევის კარი პირველი კლასის ვარიანტებისთვის Python ჯერჯერობით მეთოდები.
შემდეგი ნაწყვეტები არის მუტაციური ტრანზაქციის შაბლონები და არ იყო
საჯაროდ წარდგენილი Taira გარეშე ხელმოწერილი ანგარიშის.

უპირატესობას ანიჭებენ ტიპირებულ დამხმარეებს, როდესაც ისინი არსებობენ: ისინი ნორმალიზდებიან Python ღირებულებები და წარუმატებლობა
გამოყენება: `Instruction.from_json` მხოლოდ მაშინ, როდესაც საჭიროა
ინსტრუქციის ვარიანტი, რომელსაც არ აქვს Python ოჲმჲდნთკა ჯერ.

| ინსტრუქციის ოჯახი | Python ზედაპირი |
| --- | --- |
| რეგისტრაცია | `register_account`, `register_asset_definition_numeric`, `register_rwa`, `register_time_trigger`, `register_precommit_trigger`; `register_domain` განკუთვნილია გენეზის/ბოტსტრაპის ინსტრუმენტებისთვის |
| არარეგისტრირება | `unregister_trigger`; გამოყენება `Instruction.from_json` სხვა ვარიანტებისთვის |
| მენტა/ბერნი | `mint_asset_numeric`, `burn_asset_numeric`, `mint_trigger_repetitions`, `burn_trigger_repetitions` |
| გადარიცხვა | `transfer_asset_numeric`, `transfer_domain`, `transfer_asset_definition`, `transfer_nft`, `transfer_rwa`, `force_transfer_rwa` |
...
| რეპოს/დასახლების გაფართოება | `repo_initiate`, `repo_unwind`, `repo_margin_call`, `settlement_dvp`, `settlement_pvp` |
| ადგილობრივი აქტივების ჩაკეტვა | `open_asset_lock`, `drawdown_asset_lock`, `cancel_asset_lock`, `expire_asset_lock`, პლუს კლიენტი `*_and_wait` დამხმარეები |
| გრანტი/რევოკი, SetParameter, სარეგისტრაციო/არარეგისტრირებელი ვარიანტები | `Instruction.from_json` ან `TransactionBuilder.add_instruction_json` კანონიკური `InstructionBox` JSON |

საფინანსო სტილის პირობითი გადახდებისათვის იხილეთ
[ნაციონალური აქტივების გადახდა](/ka/blockchain/escrow.md#python-asset-locks). Python
ამჟამად გამოავლინებს პირველხარისხის დამხმარე პირებს გენერული აქტივების ჩაკეტვისთვის;
ანონიმური საფინანსო დამხმარეები არ არიან პირველი კლასის Python ჯერჯერობით მეთოდები.

### შექმნათ დომენები, შემდეგ დაარეგისტრირეთ ანგარიშები და აქტივები {#set-up-domains-then-register-accounts-and-assets}

რიგითი დომენის შექმნა ხდება დეკლარაციური ალექსანდრე დაგეგმვა, ასე რომ SNS
ქირავდება, მფლობელის შესაძლებლობები, ციტატის დაცვა და დომენის მდგომარეობა შეამოწმება ერთად.
შექმენით საიდუმლოების გარეშე `AliasSetupPlanRequestV1` განზრახვა თქვენი SDK ან
სამგზავრო სერვისი, შემდეგ გამოიყენეთ `iroha app alias setup plan` და
`iroha app alias setup apply`. არ წარუდგინოთ `Instruction.register_domain`
აპლიკაციის ტრანზაქციიდან; ეს მშენებელი რჩება genesis/bootstrap- ისთვის
ინსტრუმენტები.

მას შემდეგ, რაც დომენის შედგენა გეგმა აპირებს, რეგისტრირება დომენის საკუთრებაში არსებული ობიექტები.
ქსელი, როგორიცაა: Taira, გამოიყენეთ თქვენთვის მინიჭებული დომენი და ანგარიშის სახელების სივრცე.
...
`mintable` მიიღებს `Infinitely`, `Once`, `Not`, ან `Limited(n)` მიღებული ღირებულებები
მონაცემთა მოდელის მიხედვით. `scale` უკრძალებელი ციფრული აქტივისთვის.

### მინა, დამწვარი და გადარიცხული ქონებები {#mint-burn-and-transfer-assets}

ეს ზარები იყენებს არსებულ აქტივს ID. დარეგისტრირეთ აქტივის განსაზღვრა ჯერ, შემდეგ
ბეტონის აქტივის შექმნა ID ანგარიშზე, რომელიც ფლობს აქტივს.
...
### საკუთრების გადაცემა {#transfer-ownership}

საკუთრების გადაცემა ცვლილება, ვინ აკონტროლებს დომენს, აქტივების განსაზღვრა ან NFT.
გამოიყენეთ დღევანდელი მფლობელი, როგორც ოპერაციის ავტორი.
...
### მეტა მონაცემების შედგენა და ამოღება {#set-and-remove-metadata}

მეტა მონაცემების ღირებულებები უნდა იყოს: JSON-სერიალიზებადი. როდესაც იყენებთ `TransactionDraft`, დასახელება
უფლებამოსილება `TransactionConfig` ხდება მიზნობრივი ანგარიშის გათვალისწინება.
...
მაღალ დონეზე შემუშავებული დახმარების პროექტის მიზანი ტრანზაქციული ორგანოა:
...
RWA დამხმარეები JSON-სერიალიზებადი სასარგებლო ტვირთები აქტივების სპეციფიკური მეტა მონაცემებისთვის,
წარმოშობისა და კონტროლერის პოლიტიკის შესახებ. `register_rwa` არ მიიღებს `id` ან
`owner`: გაშვების დრო წარმოქმნის `RwaId`, და ოპერაციული ორგანო
ხდება პირველადი მფლობელი.
...
რეგისტრაციის ტრანზაქციის დადების შემდეგ, გამოყენება `FindRwas`, `/v1/rwas`, დასახელება RWA
მოვლენა, ან მკვლევრის მარშრუტი დაყენებული აღმოაჩენად ID:
...
შემდგომი ოპერაციები იყენებენ წარმოქმნილ `hash$domain` ID:
...
მთლიანი გადარიცხვები შეიძლება შეიცვალოს `owned_by` არსებულ პარტზე. ნაწილობრივი გადარიცხვები და
გაერთიანება ქმნის წარმოქმნილ ბავშვთა ნაკრებს.

### გამამოწვევები {#triggers}

გამოიყენეთ trigger რეგისტრაციის დამხმარეები, როდესაც executable არის სხვა ინსტრუქცია
თანმიმდევრობა:
...
Torii აგრეთვე გამოხატავს REST დამხმარეები საგამოძრავებელი ინვენტარიზაციისათვის:
...
ჟაჟრთნარაჲ ოპვკვნთრვ ჟვ ჲბაპთგრა თლთ ოჲჟლვეგაქ ნა ჟაჟთნარაჲ.
აღსრულება, განმეორებითი ცვლილებები და არარეგისტრირება მუტაციური ოპერაციებია.

### რეპორტისა და ანგარიშსწორების ინსტრუქციები {#repo-and-settlement-instructions}

რეპოს და ორმხრივი დასახლების დამხმარეები დამატებენ დომენის სპეციფიკურ ინსტრუქციას
ვარიანტები ხელნაკეთი დამუშავების გარეშე Norito სასარგებლო ტვირთები:
...
### JSON Escape Hatch-ი {#json-escape-hatch}

როდესაც Python დამხმარე ჯერ არ არის ხელმისაწვდომი, მიწოდება კანონიკური მონაცემების მოდელი
`InstructionBox` JSON დაწვრილებით `Instruction.from_json` ან პირდაპირ
`TransactionBuilder.add_instruction_json`. ეს არის რეკომენდებული გზა
`Grant`, `Revoke`, `SetParameter`, `Log`, `Custom`, `Upgrade`, პარტნიორი/ როლი/NFT
რეგისტრაცია, და non-trigger unregister ვარიანტები სანამ ეს დამხმარეები
ტაიპით.
...
გათვლილი ან გამჭვირვალე ინსტრუქციებისათვის, გადაადგილება-გადახურვა JSON შენახვის წინ
მობილურები:
...
## ტრანზაქციების სამუშაო პროცესები {#transaction-workflows}

გამოყენება `TransactionDraft` აპლიკაციებისთვის, რომლებიც ქმნიან მრავალჯერადი ინსტრუქციებს
ხელმოწერა. პროექტის საშუალებას გაძლევთ შეინარჩუნოთ ტრანზაქციის დონეზე პარამეტრები, როგორიცაა `ttl_ms`,
`nonce`, და მეტა მონაცემები ერთ ადგილას, შემდეგ ხელმოწერა ერთხელ:
...
ექსპორტი დეტერმინისტური მანიფესტის განხილვის, აუდიტის ან საფულის გადაცემის მიზნით:
...
გაფორმებამდე დამაგრეთ ბილიკის კონფიდენციალურობის დამტკიცება, როდესაც მიზნობრივი ბილიკი ამას მოითხოვს:
...
ტიპირებული შეკითხვის დამხმარეები ბრუნდება მონაცემთა კლასების ნაცვლად ნედლეული JSON ლექსიკონები.
არის ყველაზე მარტივი გზა დაიწყოს, რადგან SDK parses pagination და საერთო
შენთვის ჩანაწერის ველები:
...
გამოიყენეთ ზოგადი მოთხოვნის დამხმარეები, როდესაც Torii ბოლო წერტილი ჯერ არ აქვს დატაიპებული
შეფუთვა:
...
ანგარიშის ინვენტარიზაციის დამხმარე პირებს სჭირდებათ ანგარიშის იდენტიფიკატორი, რომელიც SDK ეს არის
ნორმალიზატორი. გამოიყენეთ კანონიკური I105 ანგარიში IDs ან ჯაჭვზე არსებული საიდუმლოები; თუ ბლოკი
მკვლევარი ან ნედლეული საბოლოო წერტილი ბრუნდება ID რომ SDK უარყოფს, გადაწყვიტოს იგი
კანონიკური ანგარიში ID სანამ ამ დამხმარეებს დაუძახებ:
...
გადაცემის დამხმარეების დეკოდირება JSON საგამოყენო ტვირთები დეფოლტად. `with_metadata=True`
როდესაც თქვენ გჭირდებათ SSE ღონისძიების სახელწოდება, იდენტიფიკაცია, განმეორებითი მცდელობის მინიშნება და ნედლეული სასარგებლო ტვირთები. წყვილი ნაკადები
მქონე `EventCursor` ამ მაგალითებს ელოდებიან ცოცხალი
მოვლენები, ასე რომ გაუშვით ისინი კვანძზე სადაც შესაბამისი მოვლენების ნაკადი არის
ჩართული და აქტიური.
...
სააგენტო SDK ამჟღავნებს ადგილობრივ ხელმოწერის დამხმარეებს თითოეული ხელმოწერი ალგორითმისათვის, რომელიც შედგება
ეს დამხმარეები არ იძახებენ Taira, მაგრამ ისინი საჭიროებენ
ადგილობრივი გაგრძელება:
...
გამოყენება `supported_crypto_algorithms()` ეა გთეთქ რა დგომა ოპვრთგა ჟრანთ.
ზოგადი დამხმარეები იყენებენ კანონიკური ალგორითმის ეტიკეტებს და მუშაობენ Ed25519
secp256k1, ML-DSA, GOST, BLS, და SM2 როდესაც ეს ალგორითმები შედგება:
...
სააგენტო Python SDK გამოყოფს ორივე ზოგადი SM2 დამხმარეები და SM2- სპეციფიკური კომფორტი
დამხმარეები. გამოიყენეთ კვანძის შესაძლებლობების რეკლამა, რათა აირჩიოთ SM2 განასხვავებელი
მიზნობრივი ქსელის მიერ მოსალოდნელი იდენტიფიკატორი:
...
`crypto.sm.enabled` გეტყვით, იღებს თუ არა კვანძი SM- ოჯახური ალგორითმები
ამავე რეკლამაში შედის SM ჰაშის პოლიტიკა და აჩქარება
სტატუსი, რომელიც სასარგებლოა გადაწყვეტილების მიღებისას SM2-კერძო ნაკადები:
...
საზოგადოება Taira გამოავლინეს SM შესაძლებლობების რეკლამა შემოწმების დროს, მაგრამ SM ხელმოწერა
მისი რეკლამირებული ხელმოწერის ალგორითმები იყო `ed25519`,
`secp256k1`, და `bls_normal`, ასე რომ, არ დააბრუნოთ SM2- ხელმოწერილი ოპერაციები
განთავსება, თუ შესაძლებლობების სასარგებლო ტვირთის ცვლილება არ მოხდება.

### GOST და პოსტკვანტური გასაღები {#gost-and-post-quantum-keys}

გამოიყენეთ გენერული კრიპტო API სამედიცინო GOST R 34.10-2012 პარამეტრების ნაკრები და ML-DSA
(`ml-dsa`) პოსტ-კვანტური ხელმოწერები. იგივე საკვანძო წყვილის ობიექტი ხელს უჭერს ხელმოწერას,
შემოწმება და მრავალფეხურიანი ექსპორტი:
...
კარი GOST და პოსტ-კვანტური ნაკადები კვანტის რეკლამირებული ხელმოწერის ალგორითმებზე.
გამოიყენეთ ნედლეული შესაძლებლობების სასარგებლო ტვირთი წინასწარი თავსებადობის ალგორითმების სახელებისთვის:
...
თუ კვანძი არ რეკლამირებს საჭირო ალგორითმს, გამოიყენეთ გასაღები მხოლოდ ადგილობრივი
არ წარუდგინოთ ამ ალგორითმით ხელმოწერილი ტრანზაქციები
ამ კვანძში. Taira ჩექი, GOST და ML-DSA ხელმისაწვდომი იყო, როგორც SDK
კრიპტო დამხმარეები ზემო ნაკადში Python ბიბლიოთეკა, მაგრამ არ იყო რეკლამირებული
ტრანზაქციის ხელმოწერის კვანძი.

## კონფიგურირებული კლიენტის შექმნა {#config-aware-client-creation}

გამოყენება `resolve_torii_client_config` როდესაც აპლიკაცია კითხულობს ნოდის პარამეტრებს
ფაილიდან, მაგრამ მაინც საჭიროებს გარემოს ან ტესტის სპეციფიკური გადამწყვეტი მონაცემები:
...
სააგენტო Python SDK შეუძლია შეკითხვა მიმდინარე JSON მზაობის გზა მისი გენერული
Torii მოთხოვნის დამხმარე:
...
Python არ გამოყოფს გატაიპებული Kagemusha დამატების ან გადახდის არქივის შემქმნელები.
გამოიყენეთ ტიპირებული Swift ან JVM საფულის კონსტრუქცია კანონიკური V4 არქივები, მაშინ
წარადგინეთ და გამოკითხეთ ისინი მხარდაჭერილი კაგემუშის მეშვეობით Torii კლიენტი.

## აბონენტები {#subscriptions}

აბონენტების დამხმარეები მუტაციას უწყობენ სერვისის ზარებს, რომლებიც მიემკვიდრეობაა საერთოდან. Torii
მომხმარებლის მიერ გამოყენებული `iroha_python.ToriiClient`. გამოყენება IDs და აქტივები, რომლებიც არსებობენ
ქსელი, რომლის სამიზნეც ხართ.
...
შექმნა და გაანალიზება Connect URIs, და წაიკითხეთ საზოგადოებრივი Connect სტატუსი გამოფენილი
Taira:
...
ჩარჩო კოდეკები, სესიის გასაღების გამომდინარეობა და სესიის შექმნა მოითხოვს მშობლიურ
გაგრძელება და ჩართული Connect სესიის მარშრუტი:
...
დაშიფრეთ შეტყობინებები დამტკიცების შემდეგ სტატისტიური სესიით:
...
## მმართველობა, დრო და ადმინის ზედაპირები {#governance-runtime-and-admin-surfaces}

ეს მხოლოდ წაკითხული ზარები წარმატებით დაბრუნდა საჯარო Taira:
...
Runtime გაუმჯობესების დამხმარეები იღებენ manifest ფორმა გამოყენებული runtime გაუმჯობესება
API. ეს არის ოპერატორის ქმედებები, ასე რომ გამოიყენეთ ისინი მხოლოდ კვანძის წინააღმდეგ სადაც თქვენი
ანგარიში და ტოკი ნებადართულია:
...
## სტატუსი, კონსენსუსი და ქსელის ტელემეტრი {#status-consensus-and-network-telemetry}
...
ეს დამხმარეები ხელმისაწვდომია მაშინ, როდესაც სამიზნე კვანძში გამოჩნდება შესაბამისი
Nexus/SORA საბოლოო წერტილები. განიხილეთ ცარიელი სიები როგორც ვალიდური პასუხი: საჯარო Taira მაისი
გააქტიურებული აქვს მარშრუტი ნიმუშის მანიფესტისთვის მონაცემების გარეშე; ან UAID.
...
გამოყენება `NoritoRpcClient` როცა უკვე გაქვს Norito ბაიტები და საჭიროება მოუწოდოს a
ბინარული Torii საბოლოო წერტილი. მაგალითისთვის საჭიროა ხელმოწერილი კონვერტი
ტრანზაქციის შაბლონი:
...
CUDA დამხმარეები დაბრუნდნენ `None` როდესაც backend არ არის ხელმისაწვდომი, ასე რომ პროგრამები
შეიძლება დაბრუნდეს სკალარულ დანერგვაზე:
...
სააგენტო Python SDK უკვე მოიცავს დამხმარეებს:

- Torii წარდგენა, სტატუსი, შეკითხვა და ადმინის ნაკადები
- წერილიანი ინსტრუქციების შემქმნელები საერთო ISI და დომენის სპეციფიკური გაფართოებები
- ტრანზაქციის პროექტები, მანიფესტები, ხელმოწერა და ხელმოწერილი ტრანსაქციის კონვერტი
  სამუშაო პროცესები
- სტრიმინგის მოვლენები, ფილტრები და განახლებადი კურსორები
- გენერული კაგემუშას მზაობის ხელმისაწვდომობა და Torii აბონენტების დამხმარეები; დატაიფიცირებული
  აღდგენისა და გადახდის მშენებლები არ არიან გამოფენილი
- ანგარიშის მისამართი, სრული ალგორითმის ხელმოწერის დამხმარეები, მრავალფუნქციური ბრუნვის გზები, SM2,
  GOST, ML-DSA, BLS, და კონფიდენციალური გასაღების მართვა
- შეხება URIs, სესიები, ჩარჩოები, კოდირების დამხმარეები და რეესტრის ადმინისტრატორი
- მმართველობა, გამართული დროის განახლება, Sumeragi, ბმულის ადმინისტრატორი, SoraFS, UAID, და Kaigi
  ბოლო წერტილის ჩაკეტვა, სადაც კვანძი ამ მახასიათებლებს ასახავს
...
ეს ფაილები არის სიმართლის წყარო Python ზედაპირი ჩაკეტილი
სამუშაო სივრცის რევიზია.
