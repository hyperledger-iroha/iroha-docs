...
Aitai არის SORA აპლიკაციის კორიდორი ბაზრის ტიპის ანგარიშსწორებისთვის, სადაც მყიდველი და გამყიდველი კოორდინირებენ გადახდას ქსელის გარეთ, ხოლო Iroha აკონტროლებს ქსელზე არსებულ აქტივების დაცვა. ეს უნდა გამოიყენოს ადგილობრივი საფინანსო ინსტრუქციის ოჯახი, ნაცვლად ხელშეკრულების საკუთრებაში არსებული საბანკო ანგარიშის ახალი ციფრული აქტივების დაცვის ნაკადებისთვის.

Native escrow ინახავს მფლობელობას მთავარ წიგნში. გამყიდველი გახსნის შეთავაზებას `OpenAssetEscrow`, მყიდველი იღებს და ნიშანს off-chain გადახდას `AcceptAssetEscrow` და `MarkEscrowPaymentSent`, ხოლო გამყიდველი გათავისუფლებს `ReleaseAssetEscrow` ან გააუქმებს სანამ გადახდა არის აღნიშნული. თუ მყიდველი და გამყიდველი არ ეთანხმებიან, ორივე მხარეს შეუძლია დავა გახსნას და `CanResolveEscrowDispute`-ის მფლობელმა გადაწყვიტოს გაყოფილი თანხა.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `OpenAssetEscrow`, `AcceptAssetEscrow`, `MarkEscrowPaymentSent`, `ReleaseAssetEscrow`, `CancelAssetEscrow`                                                    |გამჭვირვალე ციფრული აქტივების შეთავაზებები, მათ შორის XOR ნომინირებული ანგარიშსწორების ნაკადები. |
|`OpenAnonymousAssetEscrow`, `AcceptAnonymousAssetEscrow`, `MarkAnonymousEscrowPaymentSent`, `ReleaseAnonymousAssetEscrow`, `CancelAnonymousAssetEscrow` |დაცული შეთავაზებები, როდესაც დაფინანსებისა და დახურვის მოძრაობების განხორციელება ხდება მტკიცებულებების თანდართვით |
...
საჯარო Taira საბოლოო წერტილი სასარგებლოა წაკითხვის გვერდის შემოწმებისთვის, მაგრამ არ გამოიყენოთ მას მუტაციური მაგალითებისათვის, თუ თქვენ არ იყენებთ ავტორიზებულ ანგარიშს და აპირებთ ცოცხალ მდგომარეობის შეცვლას.
...
კონტროლის თვითმფრინავი ავტორიტეტულია. განლაგება, განახლება, უკან დაბრუნება, კონფიგურაცია, საიდუმლოება, მოდელი და სტატუსის ბრძანებები გზავნილია Torii მეშვეობით და კითხულობს ვალდებულ მსოფლიო მდგომარეობას; ისინი არ ეყრდნობა ცალკე CLI- ადგილობრივ სარკის. საჯარო მარშრუტი არის ყველაზე გრძელი პრეფისზე დაფუძნებული, ასე რომ ერთი რეგისტრირებულ მასპინძელს შეუძლია გადანაწილოს ტრაფიკი მასპინძლულ HTTP მარშრუტებსა და დეტერმინისტურ API მარშრუსებს შორის.
...
Inrou არის ჰოსტირებული HTTP შესრულების დრო, რომელიც გამოიყენება Soracloud. Iroha კვანძში ჩასმული Soracloud შესრულების დროის პროექტები შეყვანილია Soracloud სახელმწიფო ადგილობრივ მატერიალიზაციის გეგმაში, იწყებს დანიშნულ ჰოსტებულ მომსახურების რეპლიკებს როგორც loopback სერვისებს. და ანგარიშები რეპლიკა runtime მდგომარეობა უკან ავტორიტეტული მოდელი.
...
გამოიყენეთ DA, როდესაც აპლიკაციისთვის ან Nexus მარშრუტისათვის საჭიროა მთავარ წიგნში ხილული დაპირება, რომ რიგგარეშე მონაცემები კვლავაც ამოღებადია. გავრცელებული მაგალითები მოიცავს მარშრუტების სასარგებლო ტვირთის ვალდებულებებს ანგარიშსწორების ნაკადებისთვის, SoraFS პინ განზრახვებს გამოქვეყნებული შინაარსისთვის; მტკიცებულებების ბუნდები, რომლებიც უნდა შენარჩუნდეს შემდგომ შემოწმებისთვის და აპლიკაციის არტეფაქტები, რომელთა საჯარო მდგომარეობა უნდა იყოს დეგესტი და არა სრული სასარგებლო ტვირთი.
...
|ვალდებულება |აღჭურვილი მასალა, რომელიც აკავშირებს მანიფესს, ბილიკის სასარგებლო დატვირთვას, დამტკიცების ბუნდელს ან შინაარსის ფესვს მთავარ წიგნში ხილულ ჩანაწერთან. |
...
1. აღდგენა ან მიღება სასარგებლო ტვირთის WSV გარეთ, მაგალითად SoraFS CAR ფაილი ან Nexus მარშრუტის სარგებელი ტვირთი.
...
1. განახორციელეთ წარდგენილი სასარგებლო ტვირთის კანონიზაცია. Torii იღებს მიღების მოთხოვნას `(lane_id, epoch, sequence)`, სასარგებლო ბაიტების, შეკვამვის მეტა მონაცემების, ნაწილის ზომის, წაშლის პროფილის, შენახვის პოლიტიკისა და გამგზავნის ხელმოწერის საშუალებით. კვანძი გზიპის, დეფლატის ან Zstandard- ის სასარგებლო ტვირთებს მოითხოვს და შემდეგ ადასტურებს, რომ კანონიკური ბაიტის სიგრძე შეესაბამება `total_size`.
2. შეამოწმეთ ბილიკისა და ნაწილის პარამეტრები. ბილიკი უნდა არსებობდეს Nexus ბილიკის კატალოგში. `chunk_size` უნდა იყოს ნულოვანი სიმძლავრე ორი, მინიმუმ ორი ბიტი და არ უნდა აღემატებოდეს კონფიგურირებულ მაქსიმუმს. წაშლის პროფილი უნდა შეიცავდეს მონაცემთა ნაჭრებს და მინიმუმ ორ პარიტეტის ნაჭერს. ბილიკების კატალოგში შეირჩევა დამტკიცების სქემა, ან `merkle_sha256`, ან `kzg_bls12_381`.
...
5. დაამატეთ წაშლის ვალდებულებები. ჭრილობები დაჯგუფებულია `data_shards` სტრიპებად. საბოლოო სტრიპში დაკარგული უჯრედები ნულოვანია პარიტეტის გაანგარიშებისათვის. RS(16) პარიტეტი ქმნის რიგის / გლობალური პარიტეტის ნაჭრებს; ვარიანტული `row_parity_stripes` დაამატეთ კოლონური ტიპის სტრიპის პარიტეტი მატრიცის მასშტაბით. საპარიტეტო ნაჭრის ვალდებულებებია BLAKE3 მცირე ენდიანული `u16` სიმბოლოების დიგესტები.
...
8. გამოშვება ხელმოწერილი არტეფაქტები. Torii ითვლის PDP ვალდებულებას, ხელს აწერს `DaIngestReceipt`, აშენებს `DaCommitmentRecord` და წერს სროლის არტეფაკტებს მანიფესტისთვის, PDP ვალდებულების ჩანაწერზე, ვალდებულებების გრაფიკზე, პინის განზრახვაზე, მიღების ფაილზე და მიღების დღიურზე ქვითრის კურსორი წინსვლის მონოტონურად თითო `(lane_id, epoch)`.
...
ბლოკის სათაური ინახავს ჰეშებს DA მტკიცებულების პოლიტიკის, ვალდებულებების და pin განზრახვისთვის. წევრობის მტკიცებულებებისათვის, ვალდებულება ბუნდი ასევე გამოხატავს Merkle ფესვას, რომლის ფოთლებია კანონიკური Norito კოდირებული `DaCommitmentRecord` ღირებულებების ჰეშები. მშობლიური კვანძები ჰაშის მარცხენა და მარჯვენა ბავშვების კონკოტენირება; უცნაური ფოთლი არ იცვლება მომდევნო ფენის.
...
Iroha 3 კონსენსუსის პროფილები მოიცავს სასარგებლო ტვირთის გავრცელებას, რომელიც მხარდაჭერილია RBC-ით, მანიფესტის დაცვას, DA ბუნდის ვალიდენციზაციას და აღდგენის ტელემეტრიას. პარტნიორული შაბლონი გამოყოფს `[sumeragi.da]` ვალდებულებების ლიმიტებს და დასამტკიცებელი ღია ადგილებს ერთ ბლოკზე; პლუს `[sumeragi.advanced.da]` დროის გამრავლებლები კვორუმისა და ხელმისაწვდომობის ქცევისთვის. შეინარჩუნეთ ეს პარამეტრები თანმიმდევრულად ერთი ქსელის პროფილის ვალიდატორებში.
...
გამოიყენეთ შეკითხვის რეფერენცია [](/ka/reference/queries.md#nexus-data-availability-and-packages) მიმდინარე შეკითხვების სახელებისთვის DA და კონფიგურაციის შაბლონი [ peer](/ka/reference/peer-config/) თქვენი აგების მიერ გამოქვეყნებული ადგილობრივი `[sumeragi.da]` ღილაკებისთვის.

## SoraFS {#sorafs}

SoraFS არის დეცენტრალიზებული შინაარსის ადრესირებული შენახვის ქსოვილი. ის შეავსებს ბაიტებს დეტერმინისტულ ნაწილებად, CAR არქივებს და Norito მანიფესტებს, რომლებიც აკავშირებენ შინაარსის ფესვებს, ნაჭრის პროფილებს, პინ პოლიტიკას და მართვის ატესტატაციას. შენახვის პროვაიდერები რეკლამირებენ ტევადობასა და შინაარსის ხელმისაწვდომობაზე, ხოლო გეტვეიზები კონტენტების მიწოდებამდე ადასტურებენ მანიფესებსა და ნაწილობრივ ვალდებულებებს.
...
SoraDNS არის დეტერმინისტური დასახელების ფენა SORA სერვისებისა და შინაარსისთვის. ის ნორმალიზებს სახელებს, ამაგრებს რეზოლუციის დირექტორის განახლებებს Iroha-ში და განაწილებს ხელმოწერილ ზონას ან რეზოლუსორ ბუნდებს SoraFS -ის მეშვეობით . რეზოლუტორები და გეითეიები რეზოლუსორის ატესტაციის დოკუმენტებს ამოწმებენ, სანამ აღმოჩენების მეტა მონაცემებს ენდობიან.
...
`/soradns/<alias>/...` fallback არ არის სასურველი საჯარო URL. ინსტრუმენტები, აპლიკაციის მანიფესტები და ფრონტენდის კონფიგურაცია უნდა უპირატესობა თავად vanity მასპინძელი. თუ Taira-ზე საიდუმლო სახელი არ არის აქტიური, ბრაუზერის კარიბჭე ან უკან დაბრუნების გზა შეიძლება დაუბრუნდეს `404` ან შეცვალოს TLS აპლიკაციის მარშრუტირების დაწყებამდე.
...
BFV იდენტიფიკატორის გზა გამოიყენება კონფიდენციალურობის შენარჩუნებისათვის. კლიენტს შეუძლია წარადგინოს დაშიფვრილი იდენტიფექცია Torii რეზულუერზე. რეზულირებელი შეაფასებს მას აქტიური იდენტიფრაციის პოლიტიკის მიხედვით, გამოიყოფა `OpaqueAccountId` და გაცემა ქვითარი. `ClaimIdentifier` შემდეგ ამ ქვითარს მიზნულ ანგარიშზე მიმაგრებულ UAID ანგარიშზე აკავშირებს.
...
`FheJobSpecV1::validate_for_execution` აამოწმებს, რომ სამუშაო, განხორციელების პოლიტიკა და პარამეტრების ნაკრებები შეთანხმებულია მიღებამდე. იგი ასევე ახორციელებს ოპერაციის სპეციფიკურ წესებს: დამატება და გამრავლება საჭიროებს მინიმუმ ორ შესასვლელს, ბრუნვა და bootstrap ზუსტად ერთი შესასვლელია საჭირო და მოთხოვნადი სიღრმე, ბრუნვის რაოდენობა, bootstrap რიცხვი, შესასვლელის რაოდენობა, სასარგებლო ტვირთის ბაიტები და დეტერმინისტური გამომუშავების ზომა უნდა დარჩეს პოლიტიკის საზღვრებში. ციფრული ტექსტის გამოკითხვის შედეგები არ უნდა დაბრუნდეს წვრილი ტექსტის რიგები.
