...
ხელმისაწვდომობა დამოკიდებულია კვანძის მშენებლობაზე და ქსელის პროფილზე. გამოიყენეთ [`/openapi`](/ka/reference/torii-endpoints.md#app-and-sora-route-families) მიზნობრივი კვანძიზე გენერირებული აპლიკაციის-API მარშრუტების აღმოჩენისათვის . საჯარო ადგილობრივი SoraFS CID და ცნობილი მარშრუტები ამომუშავებული დოკუმენტის გარეთ არის დამონტაჟებული, ამიტომ განთავსების შემოწმებისას უშუალოდ შეისწავლეთ ეს მარშრუტები.
...
|SoraNet |კონფიდენციალურობა და ტრანსპორტის გადაფარვა ცირკულეტებისთვის, რელე ტრაფიკი, VPN, Connect სესიები და სტრიმინგის მარშრუტები. |`/v1/connect/`, `/v1/vpn/`, SoraNet მარშრუტის მეტა მონაცემები |
...
3. Soracloud მარშრუტები `/api/v1/search` ან `/api/v1/stream` Inrou-ს HTTP სერვისზე.
...
| ----------------- | --------------------- | ------------------------------------------------- |
| `/`               |SoraFS სტატიკური შემცველობა |რეპროდუქციული შინაარსის root და gateway caching |
|`/assets/*` |SoraFS სტატიკური შემცველობა |შინაარსის მიხედვით განსაზღვრული აქტივები და მტკიცებულებები |
...
1. შექმენით სასარგებლო ტვირთები ან დირექტორი.
...
თუ `/openapi.json` არ არის გამოფენილი პროფილის მიერ, სცადეთ `/openapi`. ზუსტი მარშრუტის ხელმისაწვდომობა დამოკიდებულია მშენებლობის მახასიათებლებსა და ქსელის კონფიგურაციაზე. დოკუმენტში არ არის ჩამოთვლილი საჯარო ადგილობრივი SoraFS CID და ცნობილი მარშრუტები; შეამოწმეთ ეს საბოლოო წერტილები პირდაპირ როგორც აღწერილია ქვემოთ.

### Taira მხოლოდ წაკითხვის სიგარეტის შემოწმებები {#taira-read-only-smoke-checks}

საჯარო Taira საბოლოო წერტილი სასარგებლოა წაკითხვის გვერდის შემოწმებისთვის, მაგრამ არ გამოიყენოთ მას მუტაციური მაგალითებისათვის, თუ თქვენ არ ფუნქციონირებთ ავტორიზებულ ანგარიშს და აპირებთ შეცვალოთ საჯარო ტესტნეტის მდგომარეობა.
...
Taira შეიძლება გამოავლინოს განთავსების სპეციფიკური მართვის თვითმფრინავის მარშრუტები, რომლებიც არ არის ჩამოთვლილი OpenAPI გზაზე რუკაზე. განიხილეთ `/openapi` როგორც გაფორმებული ხელშეკრულება მის შემცველ მარშრუქებზე, შემდეგ დაუყოვნებლივ დაადასტურეთ განთავსებისთვის სპეციფიკური და საჯარო ადგილობრივი SoraFS მარშრუნტები, სანამ ისინი ხელმისაწვდომი იქნება დოკუმენტაციის მიხედვით.
...
| ---------------------- | ------- | -------------------------------------------------------------------------------------------- |
|`DeterministicService` |`Ivm` |ავტორი, საცავის მდგომარეობა, სერტიფიცირებული წაკითხვა, დალაგებული ფოსტის ყუთების მენეჯერები, მმართველობის მიმართ მგრძნობიარე მუტაციები |
...
Soracloud კონფიგურაცია და საიდუმლო ჩანაწერები წარმოადგენს ავტორიტეტული განთავსების მდგომარეობის ნაწილს. განლაგება, განახლება და უკან დაბრუნება არ იხურება, როდესაც საჭირო კონფიგურირება ან საიდუმლო კავშირი აკლია ან არ შეესაბამება აქტიურ მანიფესებს.
...
გამოიყენეთ `access_kind = "read-only"` შინაარსის მარშრუტებისთვის, რომლებიც არ საჭიროებს მაყურებლის ავთენტიფიცირებას. გამოიყენეთ `authenticated` მაშინ, როდესაც გასასვლელმა რელიემ უნდა მოახდინოს ბილეთების ან მაყურებლის იდენტობის ამოქმედება Torii-ზე ან მასპინძლე სერვისზე გადასვლის წინ

### SoraNet-ცნობიარე SoraFS მოიტანეთ {#soranet-aware-sorafs-fetch}

SoraFS მოძიებამ CLI შეუძლია გამოიყოს ადგილობრივი პროქსის მანიფესტი და მიწოდოს SoraNet მარშრუტის მეტა მონაცემები ბრაუზერის გაფართოებების ან SDK ადაპტერებისათვის. ორკესტრატორმა JSON უნდა განსაზღვროს `local_proxy` `"emit_browser_manifest": true`, ხოლო CLI უნდა აშენდეს მხარდაჭერით `local-quic-proxy`. Taira -ზე, შეამოწმეთ დაშვებული პროვაიდერის კატალოგი საჯარო ტესტნეტის ფესვში, შემდეგ შეავსეთ ამ პროვაიდერისთვის გამოცემული დაცული პროვაიდერი tuple:
...
### რელიე სტიმულირებელი შემოწმების სია {#relay-incentive-verifier-roster}

რელეის სტიმულირების მიღება უარყოფს მტკიცებულებებს, თუ ყველა საჭირო შემოწმება წარმატებით არ დასრულდება. როდესაც `incentives.enable` მართალია, `incentives.trusted_verifier_ids` უნდა შეიცავდეს მინიმუმ ერთ კანონიკურ ანგარიშს ID. სია არასოდეს უნდა აღემატებოდეს 64 ჩანაწერს, მაშინაც კი, როდესაც წახალისებები გამორთულია. runtime ინახავს მას როგორც დეტერმინისტური დალაგებული ნაკრები და უარყოფს არასწორი სია გეომეტრიას რელეის დაწყების დროს.

თითოეული `RelayBandwidthProofV1` განკუთვნილია ფიქსირებული ჩარჩო/დანაწილების ბიუჯეტის ფარგლებში და უნდა მოიცავდეს მთელ ჩარჩოს. მტკიცებულების შემოწმების ანგარიში უნდა იყოს კონფიგურირებულ სიაში და `RelayBandwidthProofV1::verify_signature()` უნდა გაიმარჯვოს, სანამ რელიე იკეტება ან შეცვლის მისი შესრულების აკუმულატორის. რელიე უგულებელყოფს არასაიმედო ხელმომწერს ან ხელმოწერის არასწორობის/გაბინძურებული მტკიცებულებას. ამგვარი მტკიცებულება არ ემატება გაზომვას და ვერ წარმოქმნის სტიმულირებადი სურათს.
...
|ეტაპი |რა დაფიქსირდა?|
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
|განზრახვა |ბილეთი, მანიფესტური რეფერენცია, alias, ზოლი / ეპოქა / თანმიმდევრობის რეფერენციები, შენახვის პოლიტიკა ან რეპლიკაციის მიზანი. |
|ვალდებულება |აღჭურვილი მასალა, რომელიც აკავშირებს მანიფესს, ბილიკის სასარგებლო დატვირთვას, დამტკიცების ბუნდელს ან შინაარსის ფესვს წიგნის ჩანაწერთან. |
...
1. Torii იღებს მიღების მოთხოვნას `(lane_id, epoch, sequence)`, სასარგებლო ტვირთის ბაიტებით, შეკუმშვის მეტა მონაცემებით, ნაწილების ზომით, წაშლის პროფილით, შენახვის პოლიტიკა და გამგზავრებლის ხელმოწერა. კვანძი გზიპის, დეფლატის ან Zstandard- ის სასარგებლო ტვირთებს მოითხოვს და შემდეგ ადასტურებს, რომ კანონიკური ბაიტის სიგრძე შეადგენს `total_size` .
...
- მანიფესტური ჰაშები უნდა იყოს ნულოვანი და უნიკალური ბუნდის შიგნით.
...
4. სარკე-გამძლეობის პოლიტიკა აცხადებს ვალდებულებას.
5. დაწესებულების ფურცლიდან ძმური გზის გადახრა აღადგენს მიწოდებულ ძირს.
6. რეკონსტრუქციული ფესვი უდრის ბუნდის ფესვას.
...
- RBC აღადგინა სასარგებლო ტვირთის შეესაბამება ბლოკ ჰაშ, სიმაღლე, ნახვა და სასარგებლო სატვირთო ჰაშ.
...
SoraFS ტიპიური გამოყენებები მოიცავს სტატიკურ აპლიკაციების აქტივებს, დოკუმენტაციის ნაგებობებს, ზონის ბუნდლებს, მოდელებსა და არტეფაქტებზე რეფერენციებს და მმართველობის მტკიცებულებების ბუნდებს. Iroha მონაცემთა მოდელი გამოხატავს SoraFS კარიბჭის მოვლენებს და პროვაიდერის საკუთრების გადაწყვეტის მიზნით [`FindSorafsProviderOwner`](/ka/reference/queries.md#nexus-data-availability-and-packages) შეკითხვას .

### Taira ტესტნეტის პროფილი {#taira-testnet-profile}

Taira არის კანონიკური საჯარო ტესტის ქსელი SoraFS. მისი შემოწმებული დამტკიცებლის პროფილი იყენებს ჯაჭვს `fc56984b-2be7-431d-840e-21514d1883f0` და ჯაჭვის დისკრიმინანტს `369`. მისი გამოქვეყნებული SoraFS პარამეტრებია:

- ქსელი ID: `hash:82531CE8EAE8BFF6BEECA4698BFD13A3BC8BEC5F0EE0D23D428C97FC17AB0F3B#3E94`
...
Taira ვალიდატორები ჩაშენებულია SoraFS შენახვა, სარემონტო და ნარჩენების შეგროვება შეზღუდული. მათი კონფიგურირებული სიმძლავრე კვლავ რჩება ვალიდატორის ნაწილი დისკის ბიუჯეტის შემოწმება; ეს არ ნიშნავს, რომ ვალიდატორი არის შენახვის პროვაიდერი. გამოიყენეთ `GET /v1/sorafs/storage/peers?limit=4`, რათა წაიკითხოთ მიმდინარე კონფიგურირებული კარიბჭე და pin მიმართულებები ტესტირების წინ.

`sorafs.sora.org` CID სათადარიგო არის პირდაპირი/პროდუქციული პროფილი და არა Taira. არ შეიყვანოთ იგი Taira მანიფესტებში, წარმოშობის შემოწმებაში ან ბრაუზერის ტესტებში. წარმოების განთავსებამ უნდა გამოიყენოს საკუთარი ქსელის იდენტობა, მმართველობის გასაღები, პროვაიდერის მიღების მასალა, პინ-თავის წერტილები და კapacity/repair პოლიტიკა; არასოდეს გადაწეროს Taira საკრედიტო მონაცემები ან საბოლოო წერტილების ვარაუდები საწარმოო კონფიგურაციაში.
...
ეს მარშრუტები არასდროს იღებენ `x-sorafs-stream-token` ან `x-sorafs-token-id`. ორივე სათაურის არსებობა ცუდი თხოვნაა. კანონიკური მანიფესტი, რომელიც უკვე არის ნოდის ავტორიტეტულ ადგილობრივ მაღაზიაში, არის საჯარო წაკითხვის შესაძლებლობა; კეშის შეცდომა არ აძლევს საშუალებას დისტანციური პროვაიდერის ჰიდრატაცია. დაცული პროვაიდერი CAR და ნაწილობრივი მარშრუტები რჩებიან ცალკე ავთენტიფიცირებული პროტოკოლის ზედაპირები.

ბაიტების წაკითხვის წინ, Torii ადასტურებს ადგილობრივი მანიფესტის კანონიკურ კოდირებას, სემანტიკურ შეზღუდვებს, დიგესტს და ფესვას CID. შემდეგ ეს მოითხოვს ავტორიტეტულ ადგილობრივ პროვაიდერის იდენტობას, მმართველობის აღიარებას და მანიფესი, CID და პროვაიდერისთვის განკუთვნილ შესაბამისობას. კარიბჭის განაკვეთი / აკრძალვის პოლიტიკა იყენებს ეფექტურ კლიენტის მისამართს, რომელიც პატივს სცემს გადაგზავნილ მისამართებს მხოლოდ კონფიგურირებული სანდო პროქსების მეშვეობით. თუ პოლიტიკის, შესაბამისობის, იდენტობის ან მიღების სტატუსი არ არსებობს, Torii უარყოფს მოთხოვნას.

ერთი თხოვნა ფლობს საჯარო კარიბჭის ბოლოდან ბოლომდე ნებართვას; მთელი პროცესის ლიმიტი არის 64 ერთდროული წაკითხვა, ზედმეტი მოთხოვნების დაბრუნებით `503 Service Unavailable` და `Retry-After: 1`. მანიფესტური პასუხები 16 MiB, ფაილების ჩამონათვალები დეფოლტურად 50 ჩანაწერზე და დაბრუნება მაქსიმუმ 500, ხოლო სრული ფაილი ან ერთი ბაიტის დიაპაზონი შეზღუდულია 8 MiB. ოპერატორ-მონტაჟი დამოკიდებულია მშენებლობაზე. გადაზიდვა `app_api` build იღებს დეკოდირებულ და ხელმოწერილ 32-bit-ს `limit`, უგულებელყოფს სხვა შეკითხვის გასაღები, ნებართვა უკანასკნელი გაიმეოროს `limit` გამარჯვება, და კლეამსი ღირებულება `1..=500`. მახასიათებლების მინიმალური მშენებლობა გარეშე `app_api` იღებს მხოლოდ ერთ კანონიკურ `limit=1..500` წყვილი და უარყოფს უცნობი, განმეორებული, პროცენტული ან არაკანონიკური ფორმები. `limit=<1..500>` წყვილი ქცევისთვის, რომელიც გადაადგილებაა მთელ ნაგებობებში. CIDs, მასპინძლები, ბილიკები და მანძილების სათაურები რჩებიან კანონიკური და ერთმნიშვნელოვანი ორივე შენობაში. აქტიური HTML, CSS, JavaScript, SVG, XML, PDF, ან Wasm შინაარსი ემსახურება მხოლოდ კონფიგურირებული CID-დამოყვანილი იზოლირებული წარმოშობა (ან იქ გადამისამართება), რაც ხელს უშლის საერთო ბილიკის კარიბჭის წარმოშობის მიერ არასაიმედო შინაარსის განხორციელებას.

### შეავსე, ააშენე და წარუდგინე {#pack-build-and-submit}

შემდეგი მუტაციის მაგალითი იყენებს შემოწმებულ Taira `NetworkId`, pin endpoint, რეპლიკაციის სართული და მმართველობის პოლიტიკა. გამოიყენეთ ფინანსური ტესტნეტის ანგარიში და ერთჯერადი მფლობელის მხოლოდ საკვანძო ფაილი. Taira უშვებს ნებართვის გარეშე პინებს საბჭოს ხელმოწერების გარეშე, მაგრამ მაინც აგროვებს გათვალისწინებულ საფასურს.
...
`manifest submit` მოითხოვს `/v1/sorafs/pin/register`. თუ სამიზნე კვანძი მას არ მიმართავს, ბრძანება ჩავარდება; პირველი გამოშვების CLI არ დაბრუნდება ზოგად `/transaction` საბოლოო პუნქტზე.

### შეამოწმეთ და მოიტანეთ {#verify-and-fetch}

დაცული მოძიების tuple არის პროვაიდერის სპეციფიკური. მიიღოს მისი პროვაიდერი ID და რეკლამირებული ბაზა URL პროვაიდერების კატალოგიდან Taira, და მიიღოს კარიბჭე საკვანძო და ნაკადი token ამ ნიშნით  პროვაიდერის მიღების ნაკადი. ეს მნიშვნელობები არ არის ვალიდატორის შენახვის პარამეტრები. ჩაკეტილ Taira ვალიდატორებს აქვთ შეზღუდული შენახვა, ამიტომ ნუ ჩაანაცვლებთ ვალიდატორი პინს URL პროვაიდერისთვის URL.
...
ოპერატორებს შეუძლიათ შეამოწმონ, ექსპორტირონ და გამოუცხადონ გამოსაყენებლობის მტკიცებულების შედეგები. გამოწვევები განისაზღვრება ქსელის მტკიცებულებების მილსადენის მიხედვით; CLI ასახავს მათ შედეგებს.
...
|Torii ჩამოვარდნის გზა |`https://taira.sora.org/soradns/<fqdn>/<path>` |Torii დებეგმირება და უკან დაბრუნების მარშრუტი აქტიური alias |
...
SoraDNS მასპინძელი დერეფცია არ შეცვლის რეგულარული ინტერნეტის DNS დელეგაცია. თუ საჯარო DNS სახელი უნდა მიუთითოს SoraDNS კარიბჭეზე:
...
- Soracloud სახელმწიფო და სამუშაო სქემების მოდელი FHE ციფრული ტექსტის სამუშაო დატვირთვები მმართველობის მიერ მართული პარამეტრების კომპლექტებით, განხორციელების პოლიტიკით, ციფრულ ტექსტის ვალდებულებებით, გამოკითხვის კონვერტებით და გამჟღავნების მოთხოვნით.
...
სივრცის დირექტორი აჩვენებს UAID შეუერთების შესაძლებლობებს. `AssetPermissionManifest` დაასახელებს მონაცემთა სივრცეში, აქტივაციის პერიოდსა და ვარიანტის ამოწურვის ეპოქას UAID და შეკვეთა ნებაყოფა / უარყოფა ჩანაწერები, რომლებიც განსაზღვრულია მონაცემთა სიფართოზე, პროგრამაზე, მეთოდიზე, აქტივზე და AMX როლით. შეფასება არის უარყოფითი მოგებები: პირველი შედარების უარყოფა უარყოფს თხოვნას, წინააღმდეგ შემთხვევაში ბოლო შედარების ნებართვის კანდიდატი შემოწმდება ნებისმიერი თანხის ლიმიტის მიმართ. ამ მანიფესტების გამოქვეყნება, ვადა და მოხსნა დაცულია `CanPublishSpaceDirectoryManifest`.
...
UAID არ არის ციფრული ტექსტი და არც თვით FHE პოლიტიკა. ეს არის სტაბილური ანგარიშის შესაძლებლობის ანკერი, რომელიც გამოიყენება ანგარიშის მოსაძებნად, არაგამჭვირვალე იდენტიფიკატორის პრეტენზიები და Space Directory- ის კავშირები, რომლებიც ავტორიზირებენ მომსახურების ან მონაცემთა სივრცის ნაკადს. FHE სქემები კოდირებული სასარგებლო ტვირთების მიღებასა და შესრულებას ცალკე მართავს პარამეტრების ნაკრებების, განხორციელების პოლიტიკის, ციფრული ტექსტის ვალდებულებების და დეკრიფტაციის უფლებამოსილების პოლიტიკის საშუალებით.
...
- დაადასტურეთ გენერირებული მომსახურების ოჯახები `/openapi` სამიზნე კვანძზე Torii, და უშუალოდ შეისწავლეთ საჯარო ადგილობრივი SoraFS CID და ცნობილი მარშრუტები.
- Soracloud განთავსების მანიფესტები, SoraFS მანიფეშტები, SoraDNS გადამწყვეტის დირექტორის ჩანაწერები, SoraNet რელე დირექტორი ჩანაწერების და DA პინების განზრახვა ან ხელმისაწვდომობის ვალდებულებები როგორც მართვისადმი მგრძნობიარე არტეფაქტი.
...
- საჯარო ტესტნეტის გამოყენებისათვის გამოიყენეთ Taira პროფილი და დაიწყეთ [დაკავშირდით SORA Nexus მონაცემთა პაემანებს](/ka/get-started/sora-nexus-dataspaces.md).
...
- [კანონიკური Taira ვალიდატორის კონფიგურაცია ჩაკეტილ commit-ზე](https://github.com/hyperledger-iroha/iroha/blob/0010c5a70039eac101a4846499ba9ceaf43eb65c/configs/soranexus/taira/config.toml)
