...
Aitaiは SORA の市場式決済のためのアプリ廊下で,買い手と売り手はチェーン外での支払いを調整し, Iroha はチェーン上の資産保管を管理する.新しい数値資産保管流のために,契約所有のエスローアカウントではなく,ネイティブ エスロウ指示ファミリーを使用すべきである.

Native escrow は本簿に保管する.売り手は `OpenAssetEscrow` でオファーを開く,買い手は `AcceptAssetEscrow` と `MarkEscrowPaymentSent` でオフチェーンの支払いを受け入れ,マークし,販売者は支払いがマークされる前に `ReleaseAssetEscrow` で解放またはキャンセルします.`CanResolveEscrowDispute`による解決者が,ロックされた金額を分割することができる.
...
| `OpenAnonymousAssetEscrow`, `AcceptAnonymousAssetEscrow`, `MarkAnonymousEscrowPaymentSent`, `ReleaseAnonymousAssetEscrow`, `CancelAnonymousAssetEscrow`       |資金調達と閉算の動きが証明書に添付された場合. |
...
制御平面は権威ある.展開,アップグレード,ロールバック,コンフィギュレーション,シークレット,モデル,およびステータスコマンドは Torii を介して送信され,コミットされた世界状態を読み取ります;彼らは別々の CLI-ローカル鏡に依存しません.パブリックルーティングは,最も長いプレフィックスに基づいているため,登録されたホストがホストされている HTTP ルートと決定的な API ルートの間でトラフィックを分割することができます.
...
Inrou は Soracloud が使用するホストされた HTTP 実行時間である.局所的な物体化計画に組み込まれた Soracloud 実行時間のプロジェクトが Soracloud 状態にある Iroha ノードで,割り当てられたホストサービス複製をループバックサービスとして起動します.信頼性のあるモデルに戻す.
...
- DA は,これらのバイトをスケジュール化,監査し,レジャー状態にリンクすることを可能にするコミットメント,証明方針,証明開示およびピン意図を記録します.

DA を使用する,アプリケーションまたは Nexus レーンは,チェーン外でのデータが取得可能であることを本簿に表示される約束が必要とする場合.一般的な例としては,決済流のためのレーンの有用な負荷のコミットメント,公開されたコンテンツに対する SoraFS ピン意図が含まれます.後に確認するために保存しなければならない証明パネル,および公開状態が完全な使用負荷ではなく消化であるべきアプリケーションアーテファクト.
...
1. Torii は, `(lane_id, epoch, sequence)` の用荷バイト,圧縮メタデータ,パーツサイズ,削除プロフィール,保存ポリシー,送信者の署名を含む使用要求を受け入れます.ノードは要求されたときに gzip, deflate,または Zstandard の役に立たない負荷を解圧し,その後,カノニカルバイト長さが `total_size` に等しいかどうかを確認します.
2. Nexus レーンのカタログに存在しなければならない. `chunk_size` は,ゼロでないパワーで2個,少なくとも2バイトであり,設定された最大限度を超えない必要があります.削除プロフィールにはデータシェードと少なくとも2つのパリティシェードが含まれなければなりません.レーンカタログでは, `merkle_sha256` または `kzg_bls12_381` の証明式を選択します.
...
8. Torii は PDP コミットメントを計算し, `DaIngestReceipt` を署名し, `DaCommitmentRecord` を作成し,マニフェスト, PDP コミットメント,コミットメント記録,コミットメントスケジュール,ピン意図,領収ファイル,領収日記のためのスプリング・アーティファクトを作成します.領収書カーソーは `(lane_id, epoch)` に単調に進みます.
...
ブロックヘッダは DA 証明ポリシー,コミットメント,ピン意図のためのハッシュを保存します.メンバーシップ証明のために,コミットメントバンドルはまたメークルルーツを暴露します.その葉は正規の Norito にエンコードされた `DaCommitmentRecord`値のハッシュです.両親ノードは左と右の子の連鎖をハッシュし,異なった葉が次の層に変わらず昇進します.
...
条件の2つも成立しない場合, 同級記録 `missing_local_data`, 荷物を取り戻そうとする RBC または シンクロをブロックし, DA ステータスとテレメトリーのゲートです. DA 信号は最終性について助言します. コミット証明書からブロックがまだ完了し,それに対応するローカル・ユーティフルロードを追加します.離散した DA クォーラム証明書
...
Iroha 3 のコンセンサスプロフィールには, RBC がサポートする有用な負荷の拡散,マニフェスト・ガード,DA バンドル検証,および復元テレメトリが含まれます.同級型は,ブロックごとにコミットメントと証明開示の制限を `[sumeragi.da]` 明らかにします.`[sumeragi.advanced.da]` のタイムアウト 倍数 を 加えて クォーラム と 可用性 行動 に 関する. この 設定 は 1 つの ネットワーク プロフィール で validator 間で一致 し て いる.
...
現在の DA 問い合わせの名前には[クエリ参照](/ja/reference/queries.md#nexus-data-availability-and-packages) を使用し,ビルドで暴露されたローカル `[sumeragi.da]` ボタンのために [ ピア設定テンプレート](/ja/reference/peer-config/) を使用します.

## SoraFS {#sorafs}

SoraFS は,分散型コンテンツアドレッシングのストレージ・タブーです. バイトを決定的なブロック, CAR アーカイブにパッケージし,コンテンツルーツ,チャンキングプロフィール,ピンポリシー,およびガバナンス証明書を結びつける Norito マニフェストです.ストレージプロバイダは容量とコンテンツの利用性を宣伝し,ゲートウェイはコンテンツを配信する前にマニフェストや断片的なコミットメントを確認します.

典型的な SoraFS 用途は静的なアプリケーション資産,ドキュメントビルド,ゾーンバンドル,モデルまたはアーティファクト参照を含む.管理の証拠をまとめて Iroha データモデルの暴露 SoraFS ゲートウェイイベントと [`FindSorafsProviderOwner`](/ja/reference/queries.md#nexus-data-availability-and-packages) 提供者の所有権解決に関する問い合わせ
...
`/v1/sorafs/pin/register`がターゲットノードにルーティングされない場合, CLI は署名された `/transaction` 提出に戻り,端末パイプライン状態を待つことができます.
...
SoraDNS は, SORA サービスおよびコンテンツの決定的な命名層である.名称を正常化し, Iroha に resolver ディレクトリ更新をアンカーし,署名されたゾーンまたは resolver バンドルを SoraFS を介して配布する.解析器とゲートウェイは,発見メタデータを信頼する前に,解析器認証文書を検証します.
...
`/soradns/<alias>/...` フォールバックは公共の好ましい URL でありません.ツール設定,アプリマニフェスト,フロントエンド構成はバニティホストそのものを好むべきである.Taira でアリスがアクティブでない場合,ブラウザゲートウェイまたはフォールバックパスがアプリケーションルーティングを開始する前に `404` を返還するか, TLS を失敗させることができます.
...
- Soracloud 状態と雇用計画モデル FHE 管理管理管理のパラメータセット,実行方針,暗号文字コミットメント,クエリエンベルと開示要求を含む暗号テキストワークロード.

BFV 識別子経路は,プライバシーを守る登録に使用されます.クライアントが Torii 解析者に暗号化された識別子を提出できます.解析者はアクティブ識別子ポリシーの下で評価し, `OpaqueAccountId` を得出し,領収書を発行します.`ClaimIdentifier`はその後,その領収書を目標口座に付属した UAID に結合します.
...
`FheJobSpecV1::validate_for_execution` は,採用前に作業,実行方針,パラメータセットが一致していることを確認します. また操作特有の規則を強制します:少なくとも2つの入力が必要です,回転とブートストラップはちょうど1つの入力が必要であり,要求された深さ,回転数,ブットストラップ数,輸入数,暗号テキストクエリの結果は,素文列を返回してはならない.
...
公的メタデータ境界は, UAID 結合,不透明な識別子記録,マニストライフサイクル,ステートキー消化,暗号文字サイズ,暗号文字コミットメント,ポリシー名稱,パラメータセットバージョン,作業操作,出力状態鍵,FHE の秘密鍵は,この公開查询記録の外にある.
