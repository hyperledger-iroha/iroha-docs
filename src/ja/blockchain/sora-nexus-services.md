...
リレーインセンティブの摂取は 失敗終了です `incentives.enable` それは本当だ `incentives.trusted_verifier_ids` 少なくとも1本,最大64冊のカノニカル記事が含まれなければならない. IDs. ランタイムはリストを決定的な順序集合として保存し,リレー起動時に無効なリスト幾何学が拒絶されます.

各 `RelayBandwidthProof` は固定フレーム/アロケーション予算で解読され,完全なフレームを消費しなければならない.証明の検証アカウントは設定されたリストに存在し, `RelayBandwidthProof::verify_signature()` が成功しなければならない.リレーがロックまたは性能蓄積機を変更する前に.信頼されていない署名者または署名無効/偽造された証明は,測定に貢献し,インセンティブスナップショットを生成することはできません.
...
|方法と終点|目的|
| --- | --- |
|`GET /.well-known/sorafs/manifest`|カノニカルリクエストホストが選択したマニュメントを返します |
...
バイトを読む前に, Torii はローカル・マニフェストのカノニック・コーディング,セマンティック・コンストラクション,ダイジェスト,ルーツ CID を検証します. その後は,権威のあるローカル・プロバイダーのアイデンティティ,ガバナンス承認,および管理されたマニフェスの合致と削除チェックを要求します, CID,ゲートウェイ料金/禁止方針は,有効なクライアントアドレスを使用し,転送されたアドレスを設定した信頼性のあるプロキシを通じてのみ尊重します.欠けているポリシー,コンプライアンス,削除,アイデンティティ,または入学状態が閉鎖されます.

1 つの要求は,端から端まで公開ゲートウェイの許可を有し,プロセス全体の制限は 64 回の同時読み込みで,過剰な要請が `503 Service Unavailable` と `Retry-After: 1` を返却します.マニフェスト応答は 16 MiB に制限され,ファイルリストのデフォルト値は 50 件のエントリーに設定され,最大 500 を受け入れられ,完全なファイルまたは単行バイト範囲は 8 MiB に制限されます. CIDs,クエリ,ホスト,パス,および範囲ヘッダはカノニック単行値フォームを使用する必要があります.アクティブ HTML,スクリプト,SVG, XML, PDF,またはワームコンテンツは,構成された CID - による孤立した起源からのみ提供される (またはそこに転送される),共有のパス-ゲートウェイ起源が信頼されていないコンテンツを実行することを防ぐ.

### 適度化に関する課題 {#moderation-challenges}

SoraFS 緩和チャレンジ経済学は合意状態である.アクティブポリシーでは,管理投票資産とエスクローおよびスラッシングに使用されるガバナンスアカウントを命名します.すべての挑戦にはその資産の正確に150単位が必要です;それを増やすことでボンドがエスクローに移動します.ケースでは,重複した挑戦識別子,同じアカウントによる2回目の挑戦,またはバランスや挑戦カウンターを変更せずに再利用された証拠消化が拒否されます.

課題提出の締め切りと課題解決の締め切りは異なっています.ガバナンスは,未定の挑戦を受け入れるか否決する近い提案からちょうど24時間後に受け取ります.未定の課題をブロック投票が明らかにするのは,その決議期限を通じてのみです:

- 受け入れられた異議は,事件を停止し,全債金を返済する.
- 拒否された投訴により,事件が続行し,債券の25%をスラッシング受付者に送る (投票資産の精度に応じて下回 rounded),そして残りの金額を返済する.
- 解決されていない挑戦は 許容期間が過ぎて終了し,開封されず,完全な債券を返済します.

`ExpireSorafsModerationChallenge` は既に終了した訴訟の許可なく,無効です. 事件の最終解決はバックストップと同じ期限決済を行います.欠席する保管者は,資金を閉じ込めておくことはできません.または開示をブロックさせることもできません. 各決済は原子的なものです.返済や切断が失敗した場合,完全な決済は取り消されます.

モデレーションポリシーとケース記録は,最初のリリーススキーマを直接使用する.ノードは生成/状態初期化中に前切断の持続的なレイアウトを拒否します投票資産,保管口座,期限,または経済を旧状態から推移する代わりにそれらの固定装置を再現します.
