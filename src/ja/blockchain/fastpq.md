...
透明な数値転送は,指示がバランスを変異するときに構造化されたトランスクリプトを作成します.
...
Rust コードは,フィールド要素を`[0,p)` のカノニカル `u64` 値として表現する.
...
バックグラウンドプロバーレーンは,トランザクション入口またはコンセンサスゲートではなく,既に実行された状態上の証明生成経路である.
...
`compliance` 索に2つのメタデータ列が挿入され,一つはポリシー,もうひとつはターゲットデータ領域.
