...
また,コードベースには `IrohaSwift/IrohaSwift.podspec`. 宣言する `IrohaSwift` カプセル Swift 5.9とiOS 15で,podspecが引っ張ります Swift メインリポジトリからのソース;ネイティブブリッジは依然として存在し,トランザクションコーディングのためにリンクする必要があります.エド25519以外の署名,そして Connect暗号.
