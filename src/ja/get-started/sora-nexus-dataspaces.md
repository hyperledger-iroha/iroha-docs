...
SORA 3は, Iroha 3 と SORA Nexus で構築されたアプリ面の公開展開トラックです. まず Taira で構築し練習します. その後,同じクライアント形を Minamoto に移動する場合は別々のメインネットキーがあり,料金は実際の XOR であり,生産承認のみがあります.
...
| --------------------------- | ------------------------------------------------------------ | -------------------------------------------------- |
|ネットワーク状態を読み始めます |鍵のない質問 `/status`|鍵のない質問 `/status`|
...
5. 取引を提出する前に,ネットワークルート,チェーン,権限アカウント,指示概要,料金の資産,および予想される状態変更を報告するようエージェントに要求する.
...
原作 Iroha 取引については,最初に SDK または CLI でトランザクション包装を組み立てて署名し,その後,エージェントに `body_base64` としてコードされた法定署名したトランザクションバイトのみを与えます.代理人は,封筒を `iroha.transactions.submit_and_wait` で提出するか,または `iroha.transactions.submit` で提出し,アンケートを`iroha.transactions.wait` で提出することができます.

エージェントプロンプトにプライベートキーを貼ってはいけません.エージェントがトランザクションを作成する必要がある場合,ユーザのランタイム環境,キーチェイン,ハードウェアサインナー,または無視されたテストネット設定ファイルからの秘密をロードするローカルコードを指します.代理人は マークダウン,フィッチャー,ログ,コンマットに鍵を書き込むべきではない.
...
`--write-config`が提供されたとき,カナリーは署名したピンを送信し,確認を待て,実行時のサイン設定を書きます. Taira は公開テストネットであるため,排列の飽和感により,ポンプ自体は動作しても署名されたピンが失敗する可能性があります.`taira doctor` が飽和したキューを報告するか,カナリーが `PRTRY:NEXUS_FEE_ADMISSION_REJECTED` を返信する場合は,クライアントの設定エラーとして処理する前に待て,再試してください.
...
取得したアカウントを使用する ID どちらにしても Nexus API または CLI 命令は聖典的な説明を求めます ID, 例えば, Taira ポンプ `account_id`, バランスの取材,アカウントの厳格なフィールド,または別名結合. 対応するプライベートキーをクライアントの設定に保存しますそして同じ公共ネットワークを選択して `[account].profile = "taira"` または `[account].profile = "minamoto"`.
...
- 暗号化されたプライベートキーと同じファイルやバックアップバンドルの代わりに,パスワードマネージャーまたは分割保管プロセスでパスワードを保存する.
...
回答は現在, HTTP `202 Accepted`. 資産の定義 ID 上にあるのは Taira 公共 faucet によって資金提供された料金資産. faucet は返済時に申請を承認した `tx_hash_hex` そして `status: "QUEUED"`.
