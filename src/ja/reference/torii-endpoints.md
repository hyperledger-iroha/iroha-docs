...
アプリ面のレジュラー読書は,オプションの1つのカノニカルアカウント署名境界線を使用します.未署名されたリクエストは,アクティブパブリックデータスペックのみを受け取ります.有効な署名したリクエストでは,通話者の現在の UAID に結合したデータスペックとそのアカウントの `CanReadRestrictedDataspace` 許可によって指定された正確なデータスペックルートを追加します.`CanReadAllLedgerData`は,すべてのデータスペースに可視性を提供します.ただ単に `X-Iroha-Account` または不完全または誤った署名ヘッダセットを供給すると, `401 Unauthorized` を返却します;匿名可視性には戻らない.

同様の可視性オブジェクトは,アカウント,ドメイン,アセット定義,アセット, NFT, RWA,ホルダー,およびエクスプローラーを読みます.欠けているオブジェクトと呼び手の可視な経路の外にあるオブジェクトは意図的に区別できない.トランザクションに記録されたすべての経路足が可視である場合にのみ,コミットしたトランザクションと指示履歴が表示されます.したがって,参加者の1つの足が呼び手の範囲外にある場合でも隠されます. 欠落した,時代遅れまたは誤ったルーティングコンテキストはグローバルな読者にしか見えません.

Torii ユーザのフィルター,ページ化,カウント,またはプロジェクションの前にこの範囲を適用します SSE, WebSocket, 長期間のストリームは,レジャー許可変更と同じ権限を再評価しますアクセスが撤回された後,通用許可の故障により終了します.
...
ブロック,トランザクション,最新の取引,指示,および最新指示履歴カーサーは,追加的に約束されたスナップショット高度とブロックハッシュをピッチします.応答では `pagination.limit`, `pagination.snapshot_height`, `pagination.snapshot_hash`,`pagination.next_cursor`,および `pagination.has_more`が暴露されます.別の経路またはフィルターセットのためのカーソル,変更された可視性消化,またはノードがもはや検証できないスナップショットが閉鎖されます.ブロックする作業員が実行している間に,履歴スキャンは Torii のクエリ-アドミンス許可内に残ります.
...
ライブ `GET /v1/sumeragi/status/sse`コンセンサス診断ストリームは匿名データスペースフィードでもありません.接続のあらゆる試みで,完全なオペレーター署名ヘッダークォートートを要求します.クライアントは,正確なストリーム URI に対して新しい署名を生成し,自動輸送再試を経由してリダイレクトやサインされた試みを再再生しない.
...
全員 ISO 航路には新しいオペレーターの署名が必要. `pacs.008`, `pacs.009`, `sese.023`, または `colr.012` 提出,認証されたオペレーターは,アプリケーションヘッドで識別された参加者に属しなければならない. `From` 財政的アイデンティティ `To` アイデンティティは,他の構成された参加者の名前を指定し,選択したプロフィールが両方の当事者に対して許容されなければならない.持久的な入学記録は,出産者,相手方,入学参加者・オペレーターキー,およびオリジナルプロファイルと埋め込み署名ポリシー.
...
ISO レコードストアでは,スケーマのみが受け入れられます V3 記録と墓碑を再現する Torii 始動に失敗すると,持続したデータはそのスキーマと一致しない場合,明らかな不適合性エラーが発生します.富裕な記録は保存される恒久的な墓石はメッセージを保存します ID, バイトロードハッシュ,ビジネスメッセージ ID, そして UETR 完全な減倍のために TTL 富裕な記録の詳細が切断された後でも

Torii はライフサイクルのメッセージを署名または処理する前に再プレイエントリが持続する.未期限の再再生アイデンティティを決して排除しない.設定されたレコード容量には TTL によって保護されているエントリーのみが含まれている場合は,生命周期または会計状態を変異させずに再起動可能な `503 Service Unavailable` を送信します.
