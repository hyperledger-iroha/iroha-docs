---
translation_locale: pt
translation_source: /index.md
translation_source_hash: 491d30ca47ce82af5220641c9b508a015af61bccc6f3b6b0c86acd100b0a3e81
translation_status: machine-validated
translation_engine: nllb-200-ct2

layout: home

hero:
  name: Hyperledger Iroha 3
  text: "Documentação"
  tagline: "Plataforma de blockchain determinista para SORA Nexus, SDKs e fluxos de trabalho do operador"
  image:
    src: /icon.svg
    alt: "Hyperledger Iroha 3 logotipo"
  #actions:
  #- theme: alt
  #  text: View on GitHub
  #  link: https://github.com/hyperledger-iroha/iroha

features:
  - icon:
      dark: /start.svg
      light: /start-light.svg
    title: "Comece"
    details: "Construir o espaço de trabalho atual, lançar uma rede local e começar a usar o Iroha 3 CLI."
    link: /pt/get-started/
  - icon:
      dark: /build.svg
      light: /build-light.svg
    title: "Guia"
    details: "Encontre SDKs, melhores práticas, configuração, segurança e fluxos de trabalho do operador."
    link: /pt/guide/
  - icon:
      dark: /explained.svg
      light: /explained-light.svg
    title: "Arquitetura"
    details: "Compreender Torii, Sumeragi, Norito, IVM, e o Nexus Modelo do espaço de dados"
    link: /pt/blockchain/iroha-explained
  - icon:
      dark: /reference.svg
      light: /reference-light.svg
    title: "Referência"
    details: "Consulte as páginas de referência binárias, gênese, Torii e compatibilidade atuais."
    link: /pt/reference/
  # - title: Cookbook # (TBA)

footer: true
---

<section class="nexus-build" aria-labelledby="nexus-build-title">
<a class="nexus-build__panel" href="/pt/get-started/sora-nexus-dataspaces">
    <span class="nexus-buildcopy"> <span class="nexus-buildeyebrow">SORA Nexus</span> <span id="nexus-build-title" class="nexus-buildtitle">Construir sobre Iroha 3 / SORA Nexus</span> <span class="nexus-builddetails">Construir sobre SORA 3 com Taira rede de teste, Minamoto Exemplos de mainnet e copy-paste que podem tentar agora.</span> </span> <span class="nexus-build__action">Guia aberta <span aria-hidden="true">-&gt;</span></span>
</a>
</section>

<hr style="margin-top: 3rem;">
<p style="font-weight: 200; font-size: 0.875rem;">Hyperledger Iroha faz parte do <a href="https://www.lfdecentralizedtrust.org/projects/tag/ledger-technology" target="_blank">LF Decentralized Trust</a>. Saiba mais em <a href="https://iroha.tech/" target="_blank">iroha.tech</a>.</p>
