...
|`GET /v1/events/ws` |WebSocket |Tadbirlar oqimlariga obuna boʻling |
...
|`GET /v1/schema` |JSON |Maʼlumotlar modeli sxemasi fotosuratlari |
...
`/openapi` sxemada aks ettirilgan yo'nalishlar uchun asosiy ishlab chiqilgan shartnoma bo'lib, to'liq operatsion-sonda inventar emas. Joriy hujjatda `/livez` va `/readyz` qoldiriladi va uning `/health` tavsifi tayyorlik ko'rsatuvchisida orqaga borishi mumkin. Yo'nalish klientlarini jonli hujjatdan yarating, ammo faollik va tayyorlikni to'g'ridan-to'g'ri ishlaydigan nodga va qo'lga kiritilgan uskunalarga qarshi tasdiqlang. aniq yuza hali ham qurilish xususiyatlari va ishga tushirish vaqti konfiguratsiyasiga bog'liq. O'sha jonli hujjatni yuklash, JSON yo'nalishlarini sinovdan o'tkazish, curl so'rovlarini nusxa olish va joriy sxemadan mijoz kodini yaratish uchun [Torii API konsolini ](/uz/reference/torii-api-console.md) ishlating.
...
|`/v1/soracloud/`, `/v1/soradns/`, `/soradns/`, `/api/` |SoraCloud xizmat hayoti davri, xususiy hisoblash/model oqimlari, ommaviy kashfiyot va uyushtirilgan dasturlarni yo'naltirish  |
...
Demo repo-da avtomatik tekshiruvlar uchun quyidagilarni o'tkazing:
