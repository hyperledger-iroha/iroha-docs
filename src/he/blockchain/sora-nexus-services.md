...
### מסלול רכוב פרטי או זרימה {#private-fetch-or-streaming-route}
...
אייטאי הוא מסדרת האפליקציה SORA עבור הסדר בסגנון שוק שבו קונה ומוכר מתואמים תשלום מחוץ למשרשרת בעוד Iroha שולח את משמורת הנכסים במשרשרת. זה צריך להשתמש במשפחת ההוראות המקומית של אבטחה במקום חשבון אבטחה בבעלות החוזה עבור זרמים חדשים של שומרון נכסים מספרים.

הבטחון המקומי שומר על המשמורת בספריה הגדולה. המוכר פותח הצעה עם `OpenAssetEscrow`, הקונה מקבל ומדביק את התשלום מחוץ לשרשרת עם `AcceptAssetEscrow` ו `MarkEscrowPaymentSent`, והמכר משחרר עם `ReleaseAssetEscrow` או מבטל לפני שתשלם מסומן. אם קונה ומכר אינם מסכימים, כל צד יכול לפתוח מחלוקת ופתרון עם `CanResolveEscrowDispute` יכול לחלק את הסכום המנעול.
...
| `OpenAnonymousAssetEscrow`, `AcceptAnonymousAssetEscrow`, `MarkAnonymousEscrowPaymentSent`, `ReleaseAnonymousAssetEscrow`, `CancelAnonymousAssetEscrow`       |הצעות מוגנות כאשר תנועות המימון והסגירה נעשות על ידי קישור ראיות. |
...
נקודת הסיום הציבורית Taira היא שימושית עבור בדיקות בצד קריאה, אך אל תשתמשו בה לדוגמאות מוטציות אלא אם כן אתם מפעילים חשבון מורשה וכוונתם לשנות את מצב הקשר.
...
|`HttpService` |`Inrou` |חיים HTTP APIs, עבודה כבדה בקולקטור, שירותים בעלות תמיכה במזמון מקשן, SSE, זרמים בעזרת הדפדפן |

שטוח הבקרה הוא סמכותי. פקודות הפעלת, העדכון, ההפסקות, הקונפיגציה, הסודיות, המודל והמצב מסופקים באמצעות Torii וקוראים את מצב העולם המחויב; הם לא מסתמכים על מראה מקומי נפרד CLI. מסלול ציבורי מבוסס על קובץ מקדם ארוך ביותר, כך שאחד מארח רשום יכול לחלק את התנועה בין מסלולים HTTP ומסלולים דטרמיניסטיים API.
...
Inrou הוא זמן ההופעה הוסט HTTP המשמש על ידי Soracloud. קשר Iroha עם הפרויקטים של זמן ההופעות המשתוללים Soracloud הוסמכים למצב Soracloud לתוכנית מתאמה מקומית, מתחיל את העתקות השירות המארח המיועדות כשירותי מחזור. ומדווחים דפוקת תקופת הפעלה בחזרה למודל הרשמי.
...
- DA רשום מחויבויות, מדיניות הוכחה, פתיחות הוכחה, וכוונות קישור שמאפשרים את בייטים אלה להיות מתוכנן, בודק, ומקושר בחזרה למצב ההדף.

השתמש DA כאשר יישום או קו Nexus זקוק לאבטחה נראית למספרים שנתונים מחוץ לשערות נשארים ניתנים למשוך. דוגמאות נפוצות כוללות התחייבויות של עומס שימושי בקו עבור זרמי הסדר, כוונות סימנים SoraFS לתוכן פורסם, חבילות ראיות שעליהם לשמור כדי לאמת מאוחר יותר, ואנטיפקטים של יישום אשר מצבם הציבורי צריך להיות דיגסט ולא המטען הפועל המלא.
...
1. קאנוניקליז את המשאב הפועל הנשלח. Torii מקבל בקשה לנטול עם `(lane_id, epoch, sequence)`, בייטים של המטען הפועל, נתונים מט-הלחץ, גודל חתיכה, פרופיל למחוק, מדיניות שמירה וחתום של המגיש . הערך מפוצץ את עומס הקריאה של gzip, deflate או Zstandard בעת בקשה, ולאחר מכן מאשר כי אורך הביט הקנוני שווה `total_size`.
2. אישור קו המסלול והפרמטרים של חתיכות. הקו חייב להתקיים בקאטלוג קו Nexus. `chunk_size` חייב להיות בעל כוח שאינו אפס של שני, לפחות שני בייטים, ולא גדול מקסימום מותאם. הפרופיל למחוק חייב לכלול חלקי נתונים ושני חלקי שוויון לפחות. קטלוג המסלול בוחר את תוכנית ההוכחה, `merkle_sha256` או `kzg_bls12_381`.
...
5. הוסף התחייבויות למחוק. חתיכות מתקבוצות לשורות של `data_shards`. תאים חסרים בשורה הסופית הם אפס מכובד לחישוב השוויון. RS(16) השוויון יוצר שורות / חלקי השוויון הגלובליים; אופציונלית `row_parity_stripes` להוסיף שורת סגנון שורה משוויון ברחבי המתריכה. התחייבויות של חתיכות השוויון הן סימבולות BLAKE3 של סמלים קטנים `u16`.
...
כותרת הבלוק מאחסנת חשיש עבור DA תוכניות הוכחה, מחויבויות וכוונות קבעות. חבורת ההתחייבויות חשפה גם שורש מרקל אשר העלים הם האשיס של קנוניקה Norito-קודד `DaCommitmentRecord` הערכים. קשרים הורים מאפיינים את הקשר בין ילדים שמאל וימין; דף מוזר מופעל ללא שינוי לשכבה הבאה.
...
השתמשו ברשימת השאלות [](/he/reference/queries.md#nexus-data-availability-and-packages) עבור שמות השאלות הנוכחיים DA, ובנמגם ההסדרים של הדוגמאות [ ](/he/reference/peer-config/) עבור הכפתורים המקומיים `[sumeragi.da]` שנחשפו על ידי הבנייה שלך.

## SoraFS {#sorafs}

SoraFS הוא הרכב של אחסון מתועד לתוכן בלתי מרכזי. זה חבילה בייטים לחתיכות דטרמיסטיות, ארכיונים CAR, ומניסטים Norito שמחברים את שורשי התוכן, פרופילים של חתיכות, מדיניות פין ותוכנות השלטון. ספקי אחסון מפרסמים על קיבולת וזמינות התוכן, בעוד שערים בודקים את המניפסטים והתחייבויות של חתיכות לפני שישלמו תוכן .

טיפוסי SoraFS השימוש כולל נכסי יישומים סטטיים, מערכות תיעוד, קבוצות אזורים, דוגמאות או מקרי הזיכרון, וכבשים של ראיות לניהול. Iroha מודל נתונים חשוף SoraFS אירועים כניסה ו [`FindSorafsProviderOwner`](/he/reference/queries.md#nexus-data-availability-and-packages) בקשה לפתרון הבעלים של ספק.
...
### בדיקות הוכחה לתאוששות {#proof-of-retrievability-checks}
...
SoraDNS הוא שכבת ההגדרה של השמות עבור שירותי SORA ותוכן. היא משגרת שמות, מקושרת עדכונים לקובץ הגורמים ב Iroha, ומפיצחת חבילות אזורים או גבולות הגורמים באמצעות SoraFS. סולבורים וערוצים בודקים מסמכים של אישור הסולבורים לפני שהם מאמינים בתנתונים מטאטאפיים.
...
ההפסקות `/soradns/<alias>/...` אינן הציבורית המועדפת URL. הכלים, מוניסטים של אפליקציות וההסדר של הקצה הקדמי צריכים להעדיף את המארח השטויות עצמו. אם שם כינוי אינו פעיל ב Taira, שער הדפדפן או מסלול ההחזרה יכול לחזור `404` או להיכשל TLS לפני שתתחיל הנסיעה של היישום.
...
נתיב מזהה BFV משמש להירשם שמגן על הפרטיות. לקוח יכול לשלוח מזהה מוצפן למפתרת Torii. המפתרת מעריכה אותו תחת מדיניות זהות פעילה, מוצאת `OpaqueAccountId`, ומציאת קבלה. `ClaimIdentifier` מחבר אחר כך את הרישוי הזה ל- UAID המוסך לחשבון היעד.
...
`FheJobSpecV1::validate_for_execution` בודק כי המשרה, מדיניות ההוצאה והסגנום של הפרמטרים מסכימים לפני הכניסה. הוא גם מכיל חוקים ספציפיים לפעילות: הוספת ומضاعفة זקוקות לפחות לשתי הכניסות, סובלת ו-bootstrap זקוקות בדיוק להכניס אחד, ואת העומק הנדרש, ספירת הסיבוב, ספירת bootstrap, ספירת הכניסות; תוצאות בקשת סיפר טקסט לא צריכות להחזיר שורות של טקסט קל.
...
- לשימוש ציבורי Taira או Minamoto, תתחיל עם [תקשר לתאונות נתונים SORA Nexus ](/he/get-started/sora-nexus-dataspaces.md).
