...
RAM-LFE הוא ראשי תיבות של Random-Access Machine Laconic Function Evaluation. ב־Iroha זו שכבת פונקציות מוסתרות כללית לתוכניות שהמדיניות הציבורית שלהן נמצאת על השרשרת, אך לוגיקת ההערכה, הסודות או הקלט הגולמי שלהן אינם אמורים להיכתב למצב העולם. זרימות זיהוי של SORA Nexus משתמשות בה, למשל לחיפוש פרטי של טלפון או דוא״ל; אפשר לחשוף אותה גם כמסייעת כללית להפעלת תוכניות ב־Torii כאשר פרופיל ה־node מפעיל נתיבים הפונים ליישום.
...
במודל הנתונים, `RamLfeProgramPolicy` ו־`RamLfeExecutionReceipt` הם טיפוסים של RAM-LFE. פרמטרי BFV, מעטפות טקסט מוצפן ופרופיל התוכנית המוסתרת של RAM-FHE שייכים למנגנון הביצוע המוצפן שבו משתמשת המדיניות.
...
- המפתח הציבורי של ה־resolver עבור קבלות חתומות
...
- \(s\) הוא הסוד של הפותר, הנשמר מחוץ למצב העולם.
...
|פעולה על טקסט גלוי |פעולה על טקסט מוצפן |
| --- | --- |
|\(m+n\) |חיבור רכיבי הטקסט המוצפן. |
|\(m+\alpha\) |הוסף קבוע של טקסט פשוט בקנה מידה ל \(c_0\). |
|\(\alpha m\) |סולם את שני מרכיבי הטקסט הצפוני ב \(\alpha\). |
|\(mn\) |כפל הפולינומים של הטקסט המוצפן, שינוי קנה המידה ולאחר מכן relinearization. |

כפל הוא הפעולה היקרה. מכפלה של שני טקסטים מוצפנים בעלי שני רכיבים יוצרת באופן טבעי טקסט מוצפן בעל שלושה רכיבים, שמפוענח באמצעות \(1\), ‏\(s_k\) ו־\(s_k^2\). תהליך ה־relinearization משתמש במפתח הערכה שפורסם כדי לקפל את האיבר \(s_k^2\) בחזרה לטקסט מוצפן רגיל בעל שני רכיבים. כך גם חיבורים וכפלים מאוחרים משתמשים באותה צורה של טקסט מוצפן.
...
בתכנון RAM-LFE הזה, BFV מסתיר את קלט הלקוח מנתוני ספר החשבונות הציבורי ומצופים שרואים רק את העסקה או את payload הנתיב. אין פירוש הדבר שהשרשרת מריצה בעצמה תוכניות מוצפנות שרירותיות. זמן הריצה של Torii resolver עדיין מחזיק בחומר הסודי של BFV, מעריך את התוכנית הפרטית שהוגדרה, מצפין את הפלט המותר ומעיד על התוצאה. לאחר מכן ספר החשבונות מאמת את ה־receipt מול ה־policy commitment שבשרשרת ופותר metadata של public key או proof.
...
כל רכיב מוצפן כטקסט מוצפן סקלרי נפרד של BFV. מבנה זה הופך את הנרמול ואת אימות המעטפה למפורשים, מאפשר לארנקים לבנות בקשות מוצפנות מפרמטרים ציבוריים ומאפשר לפותר להפוך קלטים מוצפנים שקולים לתמליל קבלה קנוני ויציב.
...
התוצאה היא שוב טקסט מוצפן של BFV בעל שני רכיבים.
...
לכל אחד מ־32 הנתיבים, סביבת הריצה דוגמת \(r_j \in [0,t)\) ושומרת טקסט מוצפן של BFV שמצפין את \(r_j\). לאחר מכן התוכנית המוסתרת פועלת על אוגרים מוצפנים ועל זיכרון מוצפן:
...
מטען הנתונים של הקבלה חתומה הוא:
...
`ClaimIdentifier` מקבל את הקבלה רק כאשר החתימה או ההוכחה היא תקיימת, מטען הנתונים של ביצוע RAM-LFE המשולב מתאים למדיניות התוכנית המתועדת, ו`uaid` ו `account_id` הם החיבורים הנדרשים.

## זרימת הביצוע {#execution-flow}
...
|כביש |מטרה.|
| --- | --- |
|`GET /v1/ram-lfe/program-policies` |רשימה של מדיניות תכנית פעילה ובלתי פעילה RAM-LFE ומטא-נתונים על ביצוע ציבורי. |
|`POST /v1/ram-lfe/programs/{program_id}/execute` |להפעיל תוכנית אחת מתוך `input_hex` או `encrypted_input` ולהחזיר גיבובי פלט וקבלה ללא מצב.|
...
תמיד בדוק את מסמך `/openapi.json` של הערך היעד לפני בניית נגד המסלולים האלה. זמינות תלויה בניית הערך ובפרופיל הרשת.
...
- אל תכניסו מזהים גולמיים לכינויי חשבונות, למטא־נתונים של עסקאות, לאירועים או לשדות במצב העולם.
