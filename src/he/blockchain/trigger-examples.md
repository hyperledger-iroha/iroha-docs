...
- חשבון קנוני שנשלח על ידי המפתח של אליס.
- חשבון קנוני שליטה על ידי המפתח של משוגעת Hatter
- הגדרה של נכס צפויה להיות `tea` תחת `wonderland.universal`
- סולן של נכס זה שנמצא על ידי כל חשבון

המטרה היא לרשום מפעיל שמבחין בסלון תה של אליס ומגיש העברה מהחשבון של כובע משוגע כאשר אירוע הנתונים המתאימים מוציא.
...
הגדרה של נכס עדיין יש כתובת קנוניקה לא ברורה. שמור או שאל את הכתובת לאחר הרישום ולהשתמש בה בפעולה ההפעלה.
...
השתמש בפתוחים הטייפנים הנוכחיים של SDK עבור המטען הפועל של העסקה הסופית. הימנע מהקוד הקשיח טקסטלי ישן IDs בקוד ההדק; חישוב או חיפוש קאנוני IDs לפני בניית המוצא.
...
3. תפעילים שהפילטר שלהם מתאימים לאירועים האלה מיועדים.
4. ההשפעות המוצרות על ידי הגירוי מתמודדות בצינור ביצוע הבלוק מבלי לאפשר ביצוע בלתי מוגבל של גירוי גירוי.

אם מפוצץ משתמש `Repeats::Exactly(n)`, רשום פוצץ חדש כאשר ההספקה נגמרה ונדרש שוב את אותה התנהגות.
