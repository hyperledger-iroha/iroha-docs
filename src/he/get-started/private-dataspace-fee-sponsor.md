...
התחל מהקטלוג של מרחבי נתונים פרטיים ועבודת ההסלול המתוארת ב [התחברות למרחבי הנתונים של SORA Nexus](/he/get-started/sora-nexus-dataspaces.md#_8-provision-a-new-dataspace). קטנטן פונה למפעיל נראה כך:
...
תחילה להגדיר את הדומיין והליז SNS שחזיקים במרחב שמות נכסים. ליצור כוונה ללא סוד `AliasSetupPlanRequestV1` עבור `$BILLING_DOMAIN`, כולל המרחב נתוני `team` המספרי ID, הבעלים הקנוני, תקופת השכרת, ומגן התמחור הנוכחי:
...
הנפיקו או העבירו את הטוקן המקומי למשתמש במהלך האינטרנט:
...
השתמשו במספר טלפון וכתובת דואר אלקטרוני כביצועים של זיהוי פרטי, לא שם כינוי ציבורי. הזרימה המבוססת על FHE מומרת על זיהויים שבורים מחוץ לזיהוי חשבונות, מטא-נתונים של עסקאות, ומצב העולם:
...
7. השרשרת שומרת מזהה אטום וגיבוב קבלה, ולא את מספר הטלפון או כתובת הדואר האלקטרוני הגולמיים
...
- ערכי הטלפון והדוא״ל הגולמיים אינם מופיעים לעולם בכינויים, במטא־נתונים, ביומנים או במטעני עסקאות.
...
עבור: Taira התרגילים, להציל את עוזר faucet [קבל Testnet XOR על Taira](/he/get-started/sora-nexus-dataspaces.md#_4-get-testnet-xor-on-taira) כמו `taira_faucet_claim.py`, לאחר מכן לממן את הספונסר באמצעות faucet הציבורית במקום העברת כספית:
...
## 7. הענקת גישה של משתמש לנותן החסות {#_7-grant-a-user-access-to-the-sponsor}

נותן החסות חייב להעניק לכל משתמש הרשאה לחייב אותו בעמלות. ההרשאה הזו מונעת ממשתמשים לציין חשבונות חסות שרירותיים.
...
## 8. תוסף מטא-נתוניםנסור {#_8-attach-sponsor-metadata}
...
עבור SDKs, צרפו את אותו אובייקט מטא-נתונים של העסקה לעסקה החתומה. המשתמש חותם עליה במפתח שלו. נותן החסות אינו חותם על כל עסקת משתמש, מפני שהרשאת `CanUseFeeSponsor` שהוענקה קודם היא האישור.
...
השתמשו בכך כאשר המשתמשים אינם אמורים להחזיק XOR, אך מרחב הנתונים עדיין זקוק לעמלת יישום פנימית, להוצאת אשראי או לאסימון מכסה.
...
ביישום אמיתי, אל תגישו את התשלום באסימון המקומי כעסקת מאמץ מיטבי נפרדת. בנו עסקה חתומה אחת שמכילה גם את התשלום וגם את ההוראה העסקית, או חשפו נקודת כניסה לחוזה שגובה את האסימון המקומי לפני החלת הפעולה העסקית.
...
- מה קורה כאשר היתרה של המשתמש הוא נמוך מדי
- מה קורה כאשר המשקל של ספונסר XOR הוא נמוך מדי

::: warning

אל תשתמשו ב־`gas_asset_id` עבור תבנית "עמלת אסימון מקומי", אלא אם ברצונכם שגם נותן החסות יחויב בנכס הגז הזה. בזמן הריצה הנוכחי, `fee_sponsor` הופך את נותן החסות גם למשלם של חיובי נכס הגז המוגדרים בשרשרת העיבוד העיבוד. עבור עמלות משתמש באסימון מקומי, גבו את האסימון במפורש באמצעות העברה או כלל חוזה.
...
| `fee balance ... is insufficient` | השלימו את יתרת ה־XOR של נותן החסות. |
