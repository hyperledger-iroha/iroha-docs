...
|`GET /block/stream` |WebSocket |זרם של בלוקים מחויבים |
...
הודעות `pacs.009` צריכות לספק את המסר העסקי ID, ההגדרה של המסר ID, זמן היצירה, סכום הסדר בין-בנקאי, מטבע, תאריך הסדר, סוכן המנחה והמסמך BICs, וחובד וקרן IBANs. אם ההודעה כוללת `Purp`, הגשר מקבל כרגע מימון למטרות ערך בלבד: `Purp=SECU`.

נקודות הסיום של הגשת `pacs.008` ו `pacs.009` מקבלות מעטפות XML ISO או פורמט שדה שטוח המשמש בבדיקות גשר. תחומים אופציונליים `SplmtryData` יכולים לתפוס את הספר הגדול של המטרה Iroha, המקור ואת חשבון מטרה IDs או כתובות, והגדרת נכס ID. התגובה היא `202 Accepted` עם `message_id`, `transaction_hash`, `status`, `pacs002_code`, והקשר של ספריה/חשבון/יסוד שנפתרו.
...
- `JoinKaigi` ו `LeaveKaigi`: עדכן את רשימת השיחות. במצב פרטי, המשתתפים משתמשים בהתחייבות, וראיות רוסטר במקום לחשוף חשבון משתתף IDs ישירות.
...
שימוש `--room-policy public` לחדרים שהרילייים עשויים לחשוף ללא כרטיסי צופים, או `--room-policy authenticated` כאשר יצירות צריכות לדרוש אימות של הצופה. `--privacy-mode zk-roster-v1` רק לאחר הרשת יש את Kaigi רשימה ושימוש בדיקות מפתחות מותאמות; אחרת יחידות, העלים, רישומי השימוש הפרטי נכשלים במהלך אימות דטרמיניסטי.

### בדיקות עם דמו JavaScript {#testing-with-the-javascript-demo}
...
ראיית נקודת הסיום תלויה בהגדרות הטלמטריה.
