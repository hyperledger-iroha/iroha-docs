...
השתמש בדמו עם [`@iroha/iroha-js`](https://github.com/hyperledger-iroha/iroha/tree/main/javascript/iroha_js) ממחסנת המקור האחים Iroha . תלותה ב `file:` פותרת את הצ'אוקוט ישירות. אם הקשר המקומי משתנה, לבנות אותו מחדש תחת `iroha/javascript/iroha_js`; תיווך חבילות נקיים לא מכיל את חלל העבודה של Cargo הנדרש על ידי `npm run build:native`.
