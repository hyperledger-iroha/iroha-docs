...
- `id`: пайда болған каноникалық RWA сәйкестендіру белгісі: `<hash>$<domain>`
...
Желідегі пайдалы жүкті тығыз сақтаңыз. Үлкен заң құжаттарын, тексеру есептерін және аудит топтамаларын WSV, содан кейін асқазанды жазып қой. URI, SoraFS жол, немесе айқын сілтеме RWA метамәдени деректер.

## Идентификаторлар {#identifiers}

`RegisterRwa` шақырушы таңдалған адамды қабылдамайды `id`, және оны қабылдамайды `owner` Транзакция органы бастапқы `owned_by` есеп, және орындау уақыты `RwaId` мақсатты аймақта.
...
Өтініштер өздерінің бизнес сәйкестендіруін сақтау керек `primary_reference` немесе `metadata`, Содан кейін пайда болған `RwaId` бойынша `RwaEvent::Created`, `FindRwas`, `/v1/rwas`, немесе сауда-саттықтың міндеттемелерін орындағаннан кейін белгіленген зерттеуші бағыты.
...
|`RedeemRwa` |Қорытындылай келе, `redeem_enabled` және [PH0000000]|
...
Пайдалану [`FindRwas`](/kk/reference/queries.md#assets-nfts-and-rwas) тіркелген тізімге RWA Тікелей жаңартуды қажет ететін қолданбалар [`Rwa` деректер оқиғалары](/kk/blockchain/filters.md#data-event-filters) құрылған, меншік иесі өзгерген, бөлінген, біріктірілген, сатып алынған, тоңазытылған, тоңазатылмаған, ұсталған, босатылған, күшпен көшірілген, басқаруды өзгерткен және метамәлімет оқиғалары үшін.

Torii тізбектік режимдегі маршруттарды көрсетеді: `/v1/rwas` және `/v1/rwas/query`, Сонымен қатар зерттеуші бағыттары: `/v1/explorer/rwas` және `/v1/explorer/rwas/{rwa_id}` Жаратылған клиенттер тіршілік етуді [`/openapi`](/kk/reference/torii-endpoints.md#common-endpoints) түйіннің нақты жауап пішініне арналған құжат.
...
Тізімдеу RWA тіршілік ету арқылы анықталатын бағыттар Taira OpenAPI құжат:
...
### Алу RWA API Маршруттар {#discover-rwa-api-routes}
...
Егер тізім бос болса, түйін әлі де қолдауы мүмкін RWA басқалар арқылы нұсқаулар мен сұрау салулар Torii APIs, Бірақ ол еріктісін ашпайды JSON Жолдың отбасы.
...
Транзакция міндеттемелерін алғаннан кейін, құрылған тізім RWA IDs. Желілік-мемлекеттік бағыттар каноникалық IDs; іс-шараларды немесе эксплуатордың егжей-тегжейлі маршруттарын пайдалану, егер сіз ID қайту `primary_reference` немесе метадеректер:
...
Пайдалануы RWA ID Бұл мысалда `alice` иесі болып табылады және сонымен қатар басқарушы ретінде конфигурацияланады `hold_enabled`.
...
Фактураны RWA партиясы бойынша шот нөмірін `primary_reference` тіркелгеннен кейін, пайдаланған ID көшіру және өтеу үшін.
