...
- сандық актив - шотта ұсталатын күшеюлейтін баланс
...
- `id`: пайдаланған каноникалық RWA сәйкестендіруші, `<hash>$<domain>` ретінде көрсетілсін
- `owned_by`: партияны қазіргі уақытта иеленуші шот
...
- `controls`: бақылаушылардың есептері, бақылаушылардың рөлдері және рұқсат етілген бақылаушылардың операциялары
...
|`TransferRwa` |Кілікті басқа тіркелгіге көшіру. Толық аударым `owned_by` өзгеруі мүмкін. Жарым-жартылай аудару ID пайда болған бөлек бала партиясын жасайды. |
...
|`RedeemRwa` |Тұрақты түрде айналымнан мөлшерді алып тастау. Иесі немесе бақылаушы `redeem_enabled` дұрыс болғанда оны тапсыра алады. |
|`MergeRwas` |Ата-аналар партиясынан бір доменді және ерекшелікті біріктіріп, балалар партиясына айналдыру. |
|`ForceTransferRwa` |Кілікті басқарушының ағыны арқылы жылжыту. Конфигурацияланған басқарушы және `force_transfer_enabled` қажет. |
|`SetRwaControls` |Партиялық бақылау саясатын ауыстыру. Иесіні немесе бақылаушыны қажет етеді. |
...
Бақылаушы тіркелгілері мен рөлдері тек тиісті boolean байрақтарымен рұқсат етілген операцияларды ғана орындай алады. Қазіргі бақылау жүктемесінде контроллер сәйкестіктері мен операциялық байрақтар болады. Алу рұқсаттары тізімдері және оялған `transfers` ережелері осы пайдалы жүктемеден тыс.

## Сұрақтар, оқиғалар және APIs {#queries-events-and-apis}

Пайдалану [`FindRwas`](/kk/reference/queries.md#assets-nfts-and-rwas) тіркелген тізімге RWA Тікелей жаңартуды қажет ететін қолданбалар [`Rwa` деректер оқиғалары](/kk/blockchain/filters.md#data-event-filters) құрылған, меншік иелерін өзгерткен, бөлінген, біріктірілген, сатып алынған, тоңазытылған, тоңазатылмаған, өткізілген, босатылған, күшпен көшірілген, бақылауы өзгертілген және метамәдени оқиғалар.
...
Желіден тыс процес сәтті аяқталғаннан кейін `ReleaseRwa` тапсырылады:
...
`RedeemRwa` тізбектен тыс активтер жеткізілгеннен кейін, жетілгеннен кейін, зейнеткерлікке шыққаннан кейін немесе айналымдан шығарылғаннан кейін тапсырылады. Бұл тапсырылған мөлшерді партиядан тұрақты түрде алып тастайды. Партияда `redeem_enabled` болуы тиіс. Қол қоюшы меншік иесі немесе бақылаушы болуы керек.
...
Жіберу `FreezeRwa` егер тізбектен тыс тексеру әдеттегі меншік иесінің операциясын бөгделейді. Қолтаңбалаушы бақылаушы болуы тиіс. `freeze_enabled`.
...
Тексеру өткеннен кейін `UnfreezeRwa` ұсынылсын:
...
Ұсынылған соманы тізбектен тыс есеп айырысудан кейін қайтару:
...
Көміртек кредиттерін айналымнан алып тастау үшін `RedeemRwa` тапсырылады. Тіркеуден тыс сертификатты немесе тіркеу куәлігін метамәдени деректерде сақтау:
