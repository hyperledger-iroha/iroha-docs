...
|Құпия оқиға | Үлгілік кітапша оқиғасы `ConfidentialEvent::Shielded`, `Transferred`, немесе `Unshielded`.                              |
...
3. Активті ZK-мен қабілетті `RegisterZkAsset`, немесе саясаттың ауысуын жүзеге асыру `TransparentOnly` үшін `Convertible` немесе `ShieldedOnly`.
...
Деталдық депозит үшін ISI мінез-құлық және мысалдар, қараңыз [Жергiлiктi активтердi басқару](/kk/blockchain/escrow.md#anonymous-escrow).
