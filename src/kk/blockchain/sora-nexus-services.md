...
Қолжетімділік түйін құрылымы мен желі профиліне байланысты. [`/openapi`](/kk/reference/torii-endpoints.md#app-and-sora-route-families) пайдалану арқылы мақсатты түйіндеген қосымша-API бағыттарын табуға болады. Қоғамдық жергiлiктi SoraFS CID және танымал бағыттар осы шығарылған құжаттың сыртында орнатылады, сондықтан орналасуды тексеру кезінде сол бағыттарды тікелей тексеріңіз.
...
|SoraNet |Шектілер, эстафеталық трафик, VPN, қосылу сессиялары және стриминг бағыттары үшін жекешелік және көлік төсемі. |`/v1/connect/`, `/v1/vpn/`, SoraNet бағыттың метамәдени деректері |
...
|Атай |Қолданба деңгейіндегі фиаттық және активтерді есеп айырысу коридоры жеке бухгалтерлік кітапша емес, жергілікті депозиттік жазбалар арқылы қамтамасыз етіледі.| `OpenAssetEscrow`, `FindAssetEscrow*`, `EscrowEventFilter`, Kotodama `escrow_*` ғимараттар |
...
4. Soracloud маршруттары `/api/auth` және `/api/v1/user` детерминистік IVM басқарушыларға.
...
| ----------------- | --------------------- | ------------------------------------------------- |
| `/`               |SoraFS статикалық мазмұн |Қайталануға болатын мазмұнның тамыр және шлюз алдын ала сақтау |
...
|`/api/v1/user*` |Soracloud IVM |Басқаруға сезімтал мемлекеттердің мутациялары |
...
5. Міндетті профиль нақты дәлелдеме қажет болған жағдайда DA шерті мақсаты немесе қолжетімділік міндеттемелерін жазу.
...
1. Клиент атауды немесе манифесті шешеді.
...
|`OpenEscrowDispute`, `ResolveEscrowDispute`, `OpenAnonymousEscrowDispute`, `ResolveAnonymousEscrowDispute` |Дауылға түсу және сот стилі бойынша шешім қабылдау. |
...
Осы беттегі мысалдарды қолданғаннан бұрын, бағытталған түйінде бағыт отбасының бар екенін растаңыз:
...
Егер `/openapi.json` профильден көрінбесе, `/openapi` сынап көріңіз. Шараның нақты қолжетімділігі құрылыстың ерекшеліктеріне және желі конфигурациясына байланысты. Құжатта қоғамдық жергілікті SoraFS CID және белгілі маршруттар саналмаған; осы соңғы нүктелерді төменде сипатталғандай тікелей тексеріңіз.

### Taira Тек оқуға арналған темекі тексеректері {#taira-read-only-smoke-checks}

Қоғамдық Taira аяқталу нүктесі оқу жағындағы тексерулер үшін пайдалы, бірақ егер сіз рұқсат етілген тіркелгімен айналысасыз және қоғамдық тестілеу желісінің жай-күйін өзгертуге ниеттенбесеңіз, оны мутациялық мысалдар үшін қолданбаңыз.
...
Taira OpenAPI жол картасында көрсетілген емес, орналасуға арналған арнайы басқарушы ұшақ маршруттарын анықтауы мүмкін. `/openapi` құрамында орналасқан бағыттар үшін жасалған келісімшарт ретінде қараңыз, содан кейін оларды қолжетімділікпен құжаттағаннан бұрын тікелей орналасуға байланысты және қоғамдық жергілікті SoraFS бағыттарды растаңыз
...
|Атқарушы ұшағы |Уақыты |Пайдалану үшін .|
| ---------------------- | ------- | -------------------------------------------------------------------------------------------- |
...
Қауымдастық SoraFS алып келу CLI жергілікті прокси манифесті және спильді шығара алады SoraNet браузердің кеңейтулері үшін маршрут метамәдени деректері немесе SDK Адаптерлер, оркестр. JSON анықтауы тиіс `local_proxy` қосылған `"emit_browser_manifest": true`, және CLI орнатылуы тиіс `local-quic-proxy` Қолдау. Taira, рұқсат етілген провайдерлердің каталогын қоғамдық тест-нет тамырларында тексеру; одан кейін осы провайдер үшін берілген қорғалған провайдер туппасын толтыру:
...
### Релелік ынталандыру тексеруші тізімі {#relay-incentive-verifier-roster}

Егер барлық қажетті тексерулер сәтті болмаған жағдайда, релейлік ынталандыруды қабылдау дәлелдемелерді бас тартады. `incentives.enable` шындық, `incentives.trusted_verifier_ids` кем дегенде бір каноникалық есепті қамтуы тиіс ID. Тізім ешқашан 64 кіруден аспауы керек, тіпті ынталандырулар рұқсат етілмеген кезде де. Runtime оны детерминистік реттелген жиынтық ретінде сақтайды және эстафетаны іске қосу кезінде жарамсыз рестр геометриясын қабылдамайды.

Әрбір `RelayBandwidthProofV1` тұрақты кадр / бөлу бюджеті бойынша декодталады және ол толық кадрды қажет етеді. дәлелдеушінің тексеруші есебі конфигурацияланған тізімде болуы керек, ал `RelayBandwidthProofV1::verify_signature()` релейдің бұғаттаудан немесе оның өнімділік акумуляторын өзгертуден бұрын сәтті болуы тиіс. Эстафета сенімсіз қолтаңбалаушыны немесе қолтаңбаның жарамсыздығы / бұзылған дәлелдісін елемейді. Мұндай дәлел өлшеуді қосуға болмайды және ынталандыру кескінін шығара алмайды.
...
|Этап |Жазбалар|
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
|Мақсат |Билет, манифестикалық сілтеме, псевдоним, жолақ/эпоха/тізбелік сілтеме, сақтау саясаты немесе репликациялық мақсат. |
...
DA пайдалы жүктемені қолтаңбаланған, қайта ойнаудан қорғалған, блок-индекстелетін міндеттемеге айналдырады. маңызды алгоритмдер детерминистік, сондықтан растаушылар және шлюздер бірдей байттардан бірдей дигестерді қайта есептеуі мүмкін.

1. Torii `(lane_id, epoch, sequence)`, пайдалы жүктеме байттары, қысылу метамәліметрі, бөлшектердің мөлшері, өшіру профилімен сіңірілетін сұрауды қабылдайды. gzip, deflate немесе Zstandard пайдалы жүктемелерін талап еткен кезде түйін сымдайды, содан кейін каноникалық байт ұзындығының `total_size` тең екенін тексереді.
...
4. Жол жүріп өту саясаты міндеттемелерді қабылдайды.
5. Қатысу жапырағынан бауырлас жолды бөле отырып, берілген тамыр қайта құрылады.
6. Қайта құрылған тамыры топтық тамырға тең.

Бұл белгілі бір блок жүктемесіне нақты қолжетімділік міндеттемесінің енгізілгенін дәлелдейді; ол барлық көшірмелердің қазіргі уақытта онлайн екендігін дәлелдемейді. Тікелей алу қабілеті SoraFS провайдерлер арқылы жеке тексеріледі, PDP/PoTR тексерулері немесе профильге байланысты қолжетімділік дәлелдемесі.
...
- RBC блок хэш, биіктік, көрініс және пайдалы жүктемеге сәйкес келетін пайдалы жүкті қалпына келтірді.
...
Әдеттегі SoraFS пайдалануы статикалық қолданба активтерін, құжаттамалық құрылыстарды, аймақ топтамаларын, модель немесе артефакт сілтемелерін және басқару дәлелдемелері топтамаларын қамтиды. Iroha деректер моделі SoraFS шлюз оқиғаларын және провайдердің меншігін шешу үшін [`FindSorafsProviderOwner`](/kk/reference/queries.md#nexus-data-availability-and-packages) сұрау салуын көрсетеді.

### Taira Сынақ желісі профилі {#taira-testnet-profile}

Taira - каноникалық қоғамдық SoraFS тестілеу желісі. Оның тексерілген растаушының профилі `fc56984b-2be7-431d-840e-21514d1883f0` тізбекті және тізбектік ажыратушы `369` тізбегін пайдаланады. оның жарияланған SoraFS параметрлері мыналар:

- желі ID: `hash:82531CE8EAE8BFF6BEECA4698BFD13A3BC8BEC5F0EE0D23D428C97FC17AB0F3B#3E94`
- gateway base URL: `https://taira.sora.org`
- қапшығы Torii URLs: `https://taira-validator-1.sora.org` арқылы `https://taira-validator-4.sora.org`
- Алу қабілеті: `torii_gateway`, `chunk_range_fetch` және `potr_mldsa`
- оқшауланған мазмұндағы шығу тегі: `https://{cid}.sorafs.taira.sora.org/{path}`
- мемлекеттік пин саясаты: рұқсатсыз және ақы төленген, `require_council_signatures = false`
...
Taira валидаторлары SoraFS сақтау, жөндеу және қоқыс жинауды рұқсат еткен. Олардың конфигурацияланған қуаты валидаторының бір бөлігі болып қалады Диск-бюджетті тексеру; бұл растаушының сақтау провайдері екенін білдірмейді. Тестілеуден бұрын ағымдағы конфигурацияланған шлюз мен пин бағытын оқу үшін `GET /v1/sorafs/storage/peers?limit=4` қолданыңыз.

`sorafs.sora.org` CID жұрнағы Taira емес, тікелей эфир/өндірістік профилі болып табылады. Оны Taira манифестерінде, шығу тегі тексерістерінде немесе браузерлік сынақтарда орналастыруға болмайды. Өндірістік орналасулар өздерінің желілік сәйкестігін, басқару кілттерін, провайдерлерді қабылдау материалдарын, түйіршік соңғы нүктелерін және қуаттылық/жөндеу саясатын пайдалануы тиіс; Taira куәліктерін немесе түпкілікті нүктелердің болжамдарын ешқашан өндірістік конфигурацияға көшірмелеуге болмайды.

### Қоғамдық жергілікті CID және сайтқа кіру жолдары {#public-local-cid-and-site-gateways}

SoraFS рұқсат етілген Torii түйіннің әрқайсысы API қосымшасы орнатылмай тұрғанда да, осы анонимді қоғамдық маршруттарды орнатуда:

|Әдіс және мақсат |Мақсаты |
| ---------------------------------- | -------------------------------------------------------------------- |
|`GET /.well-known/sorafs/manifest` |Каноникалық сұраныс қожайыны таңдаған манифестті қайтарып беру |
|`GET /v1/sorafs/cid/{cid}` |Бір CID үшін шектелген жергілікті манифесті метамәдени деректері мен файл жазуларын қайтару. |
|`GET /sorafs/cid/{cid}` |Бір жергілікті мазмұнды мекендейтін сайтқа түбір құжатын қызмет көрсету |
|`GET /sorafs/cid/{cid}/{*path}` |CID төменгі бір қалыпты жолды немесе шектелген байт диапазонын көрсету.|

Бұл бағыттар ешқашан `x-sorafs-stream-token` немесе `x-sorafs-token-id` қабылдамайды. Екеуінің де болуы жаман сұраныс болып табылады. Қоғамдық оқу қабілеті; кеш қатесі қашықтықтан провайдердің гидратациялануына рұқсат бермейді. қорғалған провайдер CAR және бөлшектік бағыттар жеке аутентификацияланған протокол беттерінде болады.

Байттарды оқудан бұрын, Torii жергілікті манифесттің каноникалық кодтауын, семантикалық шектеулерді, дигесті және тамырды растайды CID. Содан кейін ол заңды жергілікті провайдердің жеке басын, басқаруды тануды және манифестке сәйкес келуді талап етеді. CID, және провайдер. Gateway ставкасы / тыйым салу саясаты клиенттің нақты мекенжайын пайдаланады, жіберілген адрестерді тек конфигурацияланған сенімді өкiлдер арқылы ғана құрметтеуге болады. Егер саясатты, сәйкестікті, жеке басын немесе қабылдауды жоғалтқан болса, Torii өтінішті бас тартады.

Бір өтiнiштiң iшiнен аяғына дейiнгi мемлекеттiк шлюзге рұқсаты бар; процес бойынша шек 64 бiр мезгiлде оқылады. қайтаруды талап ететін артық өтініштер `503 Service Unavailable` және `Retry-After: 1`. Жауаптар 16 MiB, файл тізімі әдетті түрде 50 жазуға және ең көп 500-ге дейін қайтарылады, ал толық файл немесе бір байт диапазоны 8 MiB. Сұрау салуды талдау құрылымы мен жөнелтуіне байланысты. `app_api` build дикодтандырылған қолтаңбаланбайтын 32-битті қабылдайды `limit`, басқа сұраныс кілттерін назардан тыс қалдырып , соңғысын қайталайды `limit` жеңіске жетіп, мәнді `1..=500`. Құрылымы жоқ ерекшеліктері аз `app_api` тек қана бір ғана каноникалық `limit=1..500` белгісіз, қайталанатын, пайыздық кодталған немесе каноникалық емес нысандарды қабылдамайды. `limit=<1..500>` үйлесімдер арқылы тасымалданатын мінез-құлқы үшін жұп. CIDs, хосттар, жолдар және ауқымдастық атаулары екеуінде де қаноникалық және бір мәнді болып табылады. HTML, CSS, JavaScript, SVG, XML, PDF, немесе Wassm мазмұны тек конфигурацияланған CID- бөлініп алынған оқшауланған шығу тегі (немесе оған қайта бағытталады), ортақ жол-кешек шығу тегі сенімсіз мазмұнды орындаудан сақтайды.

### Құрылыс жасап, тапсыру {#pack-build-and-submit}

Келесі мутация мысалы Taira `NetworkId`, PIN аяқталу нүктесін, репликация қабатын және басқару саясатын пайдаланады. Қаржыландырылған тест-нет тіркелгісі мен біржола иеленушіге арналған кілті файлын қолданыңыз. Taira кеңес қолтаңбаларысыз рұқсатсыз шикізаттарды қабылдайды, бірақ әлі күнге дейін реттелетін алымды алады.
...
`manifest submit` `/v1/sorafs/pin/register` талап етеді. Егер мақсатты түйін оны бағыттамаса, команда сәтсіздікке ұшырайды; бірінші релиздегі CLI жалпы `/transaction` аяқтық нүктеге қайтып кірмейді.

### Тексеру және әкелу {#verify-and-fetch}

Қорғалған алу туплесі провайдерге тән. оның провайдерін ID және жарнамаланған базасын URL Taira провайдерлер каталогынан алыңыз, сондай-ақ осы арқылы шлюз кілті мен ағын белгісін алыңыз провайдердің қабылдау ағыны. Бұл мәндер валидатор-сақтау параметрлері емес. тіркелген Taira валидаторлар сақтауды өшіріп тастаған, сондықтан URL валидаторының шертісін провайдермен алмастыруға болмайды URL.
...
Операторлар тексеру, экспорттау және қалпына келтіру мүмкіндігін дәлелдейтін нәтижелерді мәлімдеуі мүмкін. Шабақтың дәлелді құбыр желісі бойынша қиындықтар жоспарланады; CLI олардың нәтижелерін көрсетеді.
...
### Қабылдау нысандары {#host-forms}

|Нысан |Мысалы:|Мақсаты |
| --- | --- | --- |
|Мағыналы шығу тегі |`https://<fqdn>/<path>` |Манифестер мен хабарламаларда жазылған Canonical app URL |
|Taira браузерлік шлюз |`https://<fqdn>.mon.taira.sora.net/<path>` |Белсенді атау үшін жалпы браузер шлюзі |
...
- Soracloud мемлекеттік және лауазымдық схемалар үлгісі FHE басқарумен басқарылатын параметрлер жиынтығымен шифрлы мәтін жұмыс жүктемелері, орындалу саясаттары, шифрлы текст міндеттемелері, сұраныс конверттері және ашылу өтініштері.
...
Space Directory UAID-ге қосу мүмкіндіктерін көрсетеді. `AssetPermissionManifest` -да UAID, деректер кеңістігі, белсенділеу және ерікті аяқталу кезеңі аталады, сондай-ақ дерек кеңістігіне, бағдарламаға, әдіске, активке және AMX рөліне байланысты рұқсат ету / бас тарту жазулары тапсырылады. Бағалау - жеңімпаздарды бас тарту: бірінші сәйкестікті бас тарту сұрау салуды бас тартады, әйтпесе соңғы сәйкестікті рұқсат ету кандидаты кез-келген мөлшердің шегіне қарсы тексеріледі. Бұл манифесттерді жариялау, тоқтату және қайтарып алу `CanPublishSpaceDirectoryManifest` арқылы қорғалады.
...
- Torii түйінінде `/openapi` пайда болған қызмет отбасыларын растаңыз және тікелей қоғамдық жергілікті SoraFS CID және белгілі бағыттарды тексеріңіз.
...
- Қоғамдық тестілеу желісін пайдалану үшін Taira профилін пайдаланып, [мен бастаңыз SORA Nexus деректер қорына қосылыңыз](/kk/get-started/sora-nexus-dataspaces.md).
...
- [Қосылған commit](https://github.com/hyperledger-iroha/iroha/blob/0010c5a70039eac101a4846499ba9ceaf43eb65c/configs/soranexus/taira/config.toml)-да Canonical Taira validator конфигурациясы
