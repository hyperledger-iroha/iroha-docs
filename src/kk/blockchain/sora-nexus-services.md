...
Нейттивл ескроу бухгалтерлік кітапшада күтім жасайды. Сатушы `OpenAssetEscrow` дегенмен ұсыныс ашады, сатып алушы `AcceptAssetEscrow` және `MarkEscrowPaymentSent` дегендермен тізбектен тыс төлемді қабылдайды және белгілейді, ал сатушы төлем белгіленмесінен бұрын `ReleaseAssetEscrow` дегендерімен босатады немесе аттап қалады. Егер сатып алушы мен сатушы келіспесе, кез-келген тарап дауды ашуы мүмкін және `CanResolveEscrowDispute` арқылы шешімін шығарушы бекітілген соманы бөлуге болады.
...
|`HttpService` |`Inrou` |Тікелей HTTP APIs, коллекторлық ауыр жұмыс, кешпен қамтамасыз етілген қызметтер, SSE, браузер көмегімен ағымдар |

Басқару деңгейі авторитетті. Жоспарлану, жаңарту, қайта оралу, конфигурациялау, құпия, модель және жай-күй командалары Torii арқылы тапсырылады және берілген әлемдік жағдайды оқиды; олар бөлек CLI - жергілікті айнаға сүйенбейді. Қоғамдық бағыт-бағдар ең ұзақ префикске негізделеді, сондықтан бір тіркелген қоректенуші трафикті хост HTTP маршруттары мен детерминистік API маршруттары арасында бөлуге болады.
...
Inrou - қонақ үйі HTTP пайдалану уақыты Soracloud. Қалған Iroha кіріктірілген түйін Soracloud іске қосылған жобалар қабылданды Soracloud жергiлiктi материяландыру жоспарына енгiзiледi, жүктелген хост-сервис репликаларын loopback қызметтер ретiнде бастайды; және репликалық орындалу уақытын қайталау туралы есептерді сенімгерлік үлгіге енгізеді.
...
1. Пайдалы жүктемені WSV шегінен тыс құрастыру немесе қабылдау, мысалы, SoraFS CAR файлы немесе Nexus жолағындағы пайдалы жүк.
...
2. Кезекті және бөлшек параметрлерін бекіту. Кезекті Nexus жол каталогында болуы керек. `chunk_size` нөлдік емес қуаттылығы екі, кем дегенде екі байт және конфигурацияланған максималдан аспауы тиіс. Өшіру профилі деректер бөліктерін және кем дегенде екі паритетті бөліктерді қамтуы керек. Жол картасының каталогы `merkle_sha256` немесе `kzg_bls12_381` дәлелді схеманы таңдайды.
...
5. Өшіру міндеттемелерін қосу. Қисықтар `data_shards` жолақтарына топтастырылады. Ақырғы жолақта жоқ ұяшықтар паритетті есептеу үшін нөлге толтырады. RS(16) паритет қатар / жаһандық паритетті бөліктерді жасайды; ерікті `row_parity_stripes` матрица бойынша баған стилідегі жолақтың паритетін қосыңыз . Паралық бөлшек міндеттемелері - BLAKE3 кішкентай эндиандық `u16` символдардың өшіруі.
...
8. Қол қойылған артефакттерді шығару. Torii PDP міндеттемесін есептеп шығарады, `DaIngestReceipt` қол қояды, `DaCommitmentRecord` құрастырады және манифестке арналған скрул артефакттерін жазады, PDP міндеттеме, міндеттеме жазбасы, міндеттемелер кестесі, пин ниеті, квитанция файлы және квитанцияның журналы. Қабылдау курсоры `(lane_id, epoch)` бойынша біртекті түрде алға жылжып отырады.
...
Блок тақырыбы DA дәлелдеу саясаттары, міндеттемелері және пин ниеттері үшін хештарды сақтайды. мүшелік дәлелдемелері үшін міндеттемелер топтамасы сондай-ақ жапырақтары каноникалық Norito кодталған `DaCommitmentRecord` мәндерінің хештары болып табылатын Меркл тамырын ашады. Ата-ана түйіндері сол және оң балалардың қосылымын шешеді; таңғажайып жапырақ келесі қабатқа өзгермегендей көтеріледі.
...
SoraFS - орталықтандырылмаған мазмұнды мекендейтін сақтау туындысы. Ол байттарды детерминистік бөліктерге, CAR мұрағаттарға және Norito мазмұн тамырларын, бөлшектіру профильдерін, пин саясаттарын және басқару аттестацияларын байланыстыратын манифестерді жинақтап тұрады. Сақтау провайдерлері өнімділік пен мазмұнның қол жетімділігін жарнамалайды, ал шлюздер контентті ұсынудан бұрын манифесттер мен бөлшекті міндеттемелерді тексереді.
...
SoraDNS - SORA қызметтерінің және мазмұнының детерминистикалық атау қабаты. Ол атауларды қалыпқа келтіреді, ресейлеуші каталогының жаңартуларын Iroha-ға бекітеді және қол қойылған аймақтарды немесе ресейлеуш топтамаларын SoraFS арқылы таратады. Резолюторлар мен шлюздер табыстың метамәдени деректеріне сенуден бұрын резолютордың куәландыру құжаттарын тексереді.
...
`/soradns/<alias>/...` ауқымы жалпыға танымал емес URL. Құралдар, қолданбалар манифесттері және фронт-энд конфигурациясы пайдасыздық хостингтің өзіне артықшылығын білдіреді. Егер Taira желісінде аты-жөн белсенді болмаса, браузердің шлюзі немесе кері қайту жолы қолданбаларды бағыттау басталғанға дейін `404` немесе TLS қатесін қайтара алады.
...
BFV сәйкестендіру жолы құпиялылықты сақтау үшін қолданылады. Клиент шифрланған идентификаторды Torii режушіне тапсыра алады. Результер оны белсенді сәйкестендіру саясаты бойынша бағалайды, `OpaqueAccountId` алып шығарады және квитанция береді. `ClaimIdentifier` одан кейін осы квитанцияны мақсатты шотқа қоса берілген UAID квитанцияға байлайды.
...
- Қоғамдық Taira немесе Minamoto пайдалану үшін [мен бастаңыз қосылымы SORA Nexus деректер қорына](/kk/get-started/sora-nexus-dataspaces.md).
