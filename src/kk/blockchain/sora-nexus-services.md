...
Релелік ынталандыруды жеуге рұқсат етілмейді. `incentives.enable` шындық, `incentives.trusted_verifier_ids` кем дегенде бір және ең көп дегенде 64 каноникалық есепті қамтуға тиіс. IDs. Runtime тізімді детерминистік реттелген жиынтық ретінде сақтайды, ал эстафетаны іске қосу кезінде жарамсыз тізім геометриясы қабылданбайды.

Әрбір `RelayBandwidthProof` белгіленген кадр/алу бюджеті бойынша декодталады және ол толық кадрды қажет етеді. дәлелдеушінің тексеруші есебі конфигурацияланған тізімде болуы керек, ал `RelayBandwidthProof::verify_signature()` сәтті болуы тиіс, релейдің орындалу акумуляторын бұғаттаудан немесе өзгертуден бұрын. Сенімсіз қолтаңбалаушы немесе қолтаңбаның жарамсыз/жасалған дәлелі осыған байланысты өлшеуге қатыспайды және ынталандыру кескінін жасай алмайды.
...
SoraFS рұқсат етілген Torii түйіннің әрқайсысы API қосымшасы орнатылмай тұрғанда да, осы анонимді қоғамдық бағыттарды орнатуда:
...
|`GET /sorafs/cid/{cid}/{*path}` |CID төменгі бір қалыпты жолды немесе бір шектелген байт диапазонын көрсету|
...
Байттарды оқудан бұрын, Torii жергілікті манифесттің каноникалық кодтауын, семантикалық шектеулерді, дигесті және тамырды растайды CID. Содан кейін ол заңды жергілікті жеткізушінің жеке басын, басқаруды қажет етеді қабылдау, сондай-ақ манифесттің сәйкестігін және алынып тасталуын тексеру; CID, және провайдер. Gateway ставкасы / тыйым салу саясаты клиенттің нақты мекенжайын пайдаланады, жіберілген адрестерді тек конфигурацияланған сенімді өкiлдер арқылы ғана құрметтеуге болады. Жоғалған саясат, сәйкестік, алып тастау, жеке басын анықтау немесе қабылдау мемлекетінің жабылуы мүмкін емес.

Бір өтiнiштiң iшiнен аяғына дейiнгi мемлекеттiк шлюзге рұқсаты бар; процес бойынша шек 64 бiр мезгiлде оқылады. қайтаруды талап ететін артық өтініштер `503 Service Unavailable` және `Retry-After: 1`. Жауаптар 16 MiB, файл тізімдері әдетті түрде 50 жазуға және ең көп 500 қабылдау, ал толық файл немесе бір байт диапазоны 8 MiB. CIDs, Сұрау салулар, қоректендірушілер, жолдар және ауқымдастық атаулары өздерінің қаноникалық бір мәнді нысандарын қолдануы тиіс. HTML, сценарий, SVG, XML, PDF, немесе Wassm мазмұны тек конфигурацияланған CID- бөлініп алынған оқшауланған шығу тегі (немесе оған қайта бағытталады), ортақ жол-кешек шығу тегі сенімсіз мазмұнды орындаудан сақтайды.

### Өмірлендірудің қиындықтары {#moderation-challenges}

SoraFS moderation challenge экономикасы консенсус мемлекеті болып табылады. белсенді саясат басқарушылық дауыс беру активтері мен кепілге және қысқарту үшін пайдаланылатын басқарушылық шоттар аталады. Әрбір күрес дәл осы активтің 150 бірлігін қажет етеді; оны көтеру атомдық түрде облигацияны кепілге көшіреді. Бұл жағдайда қосарланған қарсылық идентификаторы, екінші қарсылық туралы есеп немесе қайтадан пайдаланылған дәлелдемелерді теңгерімдер мен қарсылық есептегіштерді өзгертпей тастау керек.

Тәуекелді беру мерзімі мен тәуекелді шешу мерзімі әртүрлі. Әкiмшiлiк әкiмдiктiң қабылдауы немесе бас тартуға жақын тұрған қарсылықтарды ұсынғаннан кейiн дәл 24 сағаттан кейін алады. Күтуде тұрған қарсылықтар туралы дауыс беру блогы осы қаулының мерзiмi арқылы ғана:

- қабылданған қарсылық іс-әрекетті тоқтатады және кепілдіктің толық сомасын қайтарады;
- қабылданбаған қарсылық істі жалғастыруға мүмкіндік береді, облигацияның 25% -ын қысқарту алушыға жібереді (ауыстық активтің дәлдігіне байланысты дөңгелектелген), ал қалған бөлігін қайтарады; және
- шешімін таппаған дау кешіктірілу мерзімі өткеннен кейін аяқталады, ашылмайды және толық кепілдікті қайтарады.

`ExpireSorafsModerationChallenge` бұрын-соңды аяқталған дауға рұқсатсыз және күшсіз. Осылайша, жоқтың қасы ұстаушы қаражатты бұғаттап тастай алмайды немесе ашуды бұғаттауды сақтай алмайды. Әрбір есеп айырысу атомдық: егер қандай да бір қайтару немесе кесу аяғы сәтсіздікке ұшыраса, толық есеп айырысу кері қайтарады.

Модерациялау саясаты мен жағдайды жазулар бірінші релиз схемасын тікелей қолданады. Нодтар генезис/мемлекетті бастау кезінде алдын ала кесілген сақталған орналасуларды бас тартады немесе шұғыл кескінді қалпына келтіру; дауыс беретін активті, күтім шоттарын, мерзімдерді немесе экономиканы мұралық жағдайдан шығарудың орнына, сол құрылғыларды қайта құру.
