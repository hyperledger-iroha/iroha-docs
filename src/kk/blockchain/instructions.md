...
Біз туралы сөйлескенде [қалай Iroha жұмыс істейді](/kk/blockchain/iroha-explained), Біз мұны айттык Iroha Әлемдік мемлекетті өзгертудің бірден-бір жолы - арнайы нұсқаулар. Бізде қандай арнайы нұсқаулар бар? Егер сіз осы оқу-әдістемедегі тілдік нұсқауларды оқысаңыз, Сіз бірнеше нұсқаулықты көрдіңіз: `Register<Account>` және `Mint<Numeric>`.
...
Кейбір нұсқаулықтарда мақсатты анықтау қажет. Мысалы, егер сіз активтерді аударатын болсаңыз, оны қай шотқа аударып жатқаныңызды әрдайым көрсетуіңіз керек. Басқа жағынан, сіз бір нәрсені тіркеуде болсаңыз, сізге тек өзіңіз тіркегіңіз келетін зат керек.

|Нұсқаулық |Нысандар |Мақсаты |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | -------------------- |
| [EnsureAlias](#ensurealias) |Әдеттегі домен, деректер кеңістігінің атаулары және есептік жазбалардың атауларын орнату|                      |
...
| [SetKeyValue/RemoveKeyValue](#setkeyvalue-removekeyvalue) |[ метадеректері бар объектілер](./metadata.md): домендер, шоттар, активтер анықтамасы, NFTs, RWAs, триггерлер |                      |
...
Қоғамдық Taira тест-нетін нысанаға алғанда, Taira клиент конфигурациясын пайдаланыңыз. Төлем төлейтін мысалдарды орындаудан бұрын, кранның көмекшісін [ден сақтаңыз. Testnet XOR Taira](/kk/get-started/sora-nexus-dataspaces.md#_4-get-testnet-xor-on-taira) бойынша `taira_faucet_claim.py` ретінде алыңыз, содан кейін краннан testnet XOR талап етіңіз:
...
`EnsureAlias` - домендер мен олардың SNS лизингін құру үшін әдеттегі бірінші релиз жолы. Ол нақты дерек кеңістігін, иесін, жалға беру мерзімі мен цитатаны бекітеді, содан кейін барлық қажетті күйді атомдық түрде жасайды немесе жөндейді. Аутентификацияланған `POST /v1/aliases/setup/plan` аяқтық нүктесін немесе сәйкес келетін CLI жұмыс тетігін пайдалану:
...
Мақсат пен жоспар құпия емес, бірақ қадам белгілерін қолдану және конфигурацияланған шотпен әдеттегі операцияны тапсыру. Жоспар өзінің тізбектеріне, билігіне, тіршілік-мемлекеттік бекітпесіне және мерзіміне байланысты; оны ешқашан басқа желіде қайта қолданбаңыз.

## (Бас) Тіркеу {#un-register}

Тіркеу және тіркеуден шығару - бұл блокчейндегі жаңа субъектіге ID беру үшін қолданылатын нұсқаулар.

Тіркелуі мүмкін барлық нәрсе `Registrable` және `Identifiable` болып табылады, бірақ `Identifiable` дегеннің бәрі `Registrable` емес. Көптеген нәрселер тікелей тіркеледі, бірақ кейбір жағдайларда блокчейндегі бейнелеуде айтарлықтай көп деректер бар. Қауіпсіздік және өнімділік себептерінен біз мұндай деректер құрылымдары үшін құрылысшыларды қолданамыз (мысалы, `NewAccount`), ал теңгерімдік тіркеуде иелікті дәлелдейтін арнайы нұсқаулық бар.

Сіз шоттарды, активтердің анықтамаларын, NFTs, теңгерімдерді, рөлдерді және триггерлерді тіркей аласыз. Доменді орнату `EnsureAlias` қолданады; шикі `Register::Domain` пайдалы жүктеме генезис/bootstrap үшін арналған. Теңгерлік тіркеу `RegisterPeerWithPop` пайдаланады, ол теңгерім кілтіне ие болу дәлелі бар. Еліміздің [ атау конвенцияларын](/kk/reference/naming.md) тексеріп, субъектінің есімдеріне қойылатын шектеулер туралы білуге болады.

RWA партиялары арнайы `RegisterRwa` нұсқау арқылы құрылады. Ағымдағы кодта `UnregisterRwa` нұсқаулығы жоқ; бейнеленген мөлшерді шығару үшін `RedeemRwa` қолданыңыз.

::: info

[ генезистік блогыңызды ](/kk/guide/configure/genesis.md) `genesis.json` -да қалай орнатуға шешім қабылдағаныңызға байланысты (атап айтқанда, рұқсат белгілерін тіркеуді қосасыз ба немесе жоқ па), шотты тіркеу процесі өте әртүрлі болуы мүмкін. Жалпы алғанда, біз оны былай қорытындылаймыз:
...
Блокчейн объектілерін тіркеу үшін тілдік-белгілі нұсқаулықты қолдану:

|Тіл |Кітапшы |
| --------------------- | ------------------------------------------------------------------------------------------------------- |
|CLI | Пайдаланыңыз [Iroha CLI](/kk/get-started/operate-iroha-via-cli.md) Домендерді құру және шоттар мен активтерді тіркеу. |
...
Тіркеу және тіркеуден шығару триггерлері. Триггердің тіркелуіне жинақталған IVM байткоды немесе сериалдық нұсқаулық тізімі қажет. Бұл мысал `Log` нұсқаулығын CLI арқылы жасайды және оны триггерді тіркеуге өткізеді:
...
Mint блокчейн активтері үшін тілге тән нұсқаулықты қолданыңыз:
...
Трансферлер шоттар арасындағы иелікті немесе құнды жылжытады. Жалпы трансферттік нұсқалар домендерді, активтердің анықтамаларын, сандық активтерді қамтиды және NFTs. RWA көлемді қозғалысы арнаулы `TransferRwa` және `ForceTransferRwa` көрсетілген нұсқаулар [Реалдық дүниедегі активтер](/kk/blockchain/rwas.md).
...
Бұл нұсқаулықтар нысанды жаңарту [ метамәдени деректер](/kk/blockchain/metadata.md). Метамәдени деректерді енгізу немесе ауыстыру үшін `SetKeyValue` және бірін өшіру үшін `RemoveKeyValue` пайдалану.
...
CLI триггерлерді тіркей алады және тікелей триггерлік орындалу оқиғаларына жазылуы мүмкін. Ол `execute trigger` командасын келтірмейді, сондықтан қолма-қол `ExecuteTrigger` нұсқауларын тапсыру үшін, SDK немесе орындаушы құралымен сериалдалған `InstructionBox` туындысын өндіреді және нәтижесінде пайда болған JSON массивін `ledger transaction stdin` арқылы өткізеді:
