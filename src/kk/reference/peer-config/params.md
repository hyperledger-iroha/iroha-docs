...
Консенсусты растаушылар пайдалануы тиіс BLS-Әдеттегі теңгермелік кілттер. [`trusted_peers_pop`](#param-trusted-peers-pop) кіру.

<param-table env="TRUSTED_PEERS">
<template #type>

Бауырластар жиектерінің тізімі. `PUBLIC_KEY@ADDRESS` кезіндегі P2P мекенжайы белгілі; бос `PUBLIC_KEY` сондай-ақ қабылданады және жасөспірімдердің мекенжайын әңгімеден анықтауға мүмкіндік береді.
...
Қолтаңбаланған генезис блогының пайдалы жүктемесіне файл жолы `kagami genesis sign`. Жаратылған профильдер әдетте осыны Norito `.nrt` файл.
...
Стринг, бiр немесе бiрнеше сызықпен бөлiнген директивтерден тұрады. Әрбiр директиваға сәйкес келетiн ең жоғары сөздiк деңгейi болуы мүмкiн, ол (мысалы, таңдап алады) соған сәйкес аралықтар мен оқиғаларды жүзеге асырады. Iroha кем эксклюзивті деңгейлерді қарастырады (мысалы: `trace` немесе `info`(мысалы, аудармалы деңгейлер) `error` немесе `warn`).
...
Қосымша мәліметтер үшін қараңыз: [`tracing-subscriber` құжаттама](https://docs.rs/tracing-subscriber/latest/tracing_subscriber/filter/struct.EnvFilter.html).
...
`logger.filter` бірге жұмыс істейді [`logger.level`](#param-logger-level) Бір-біріне бір-бірінен бас тартпайды.
...
- `json`: Өнімдер жаңа сызықпен шектеледі JSON Бұл құрылымдалған бүркектерді тұтыну жүйелерімен өндіріске арналған JSON Анализ және қарау құралдары арқылы. JSON шығыс адам үшін оқуға жарамды емес.

Толығырақ және үлгілік нәтижелер үшін қараңыз. [`tracing-subscriber` құжаттама](https://docs.rs/tracing-subscriber/latest/tracing_subscriber/fmt/format/index.html).
...
- `readonly`: Ұқсас `read_write` бірақ Iroha ешқандай фотосуреттер туғызбайды.
