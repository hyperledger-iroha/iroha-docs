...
# Қосылған Kaigi а) JavaScript Қолданба {#embed-kaigi-in-a-javascript-app}

Kaigi Қолданбаға қолма-қол ақшамен қамтамасыз етілген аудио/видео кездесулер құруға мүмкіндік береді, олардың өмір циклі Iroha. Браузер әлі де медиамен айналысады WebRTC, кезінде Torii және Kaigi нұсқаулықтар тұрақты кездесу жазбасын, шифрланған сигнал беру метамәліметтерін, жеке тізімді қолдауды және пайдалану оқиғаларын ұсынады.
...
- Node.js 20+ егер JavaScript демо немесе түпнұсқа `@iroha/iroha-js` тікелей байланыстыру
...
Демоны пайдалану [`@iroha/iroha-js`](https://github.com/hyperledger-iroha/iroha/tree/main/javascript/iroha_js) бауырластан Iroha Бастапқы қоры. `file:` егер түпкілікті байланысы өзгерсе, оны қайта құруға `iroha/javascript/iroha_js`; таза пакеттiк каталогта жүктiң жұмыс кеңiстiгi жоқ `npm run build:native`.
...
Бұл командалар растайды TAIRA тірі және бұл Kaigi телеметриясы қол жетімді. Олар Kaigi Транзакциялар. `CreateKaigi` немесе `JoinKaigi` сынама қажеттіліктері қаржыландырылады TAIRA есеп айырысу және демо көпірінен немесе басқа да қолма-қол ақшамен қамтамасыз етілген көпірден өту.
...
|Iroha көпір| қол қою, `CreateKaigi`, `JoinKaigi`, `EndKaigi`, сигналды сайлау |
...
Демонстрациялық қолданбада бұл көпір әдістерін `@iroha/iroha-js`, жергілікті қолтаңбалау, шифрланған Kaigi метамәдени деректер және Torii шақырулар.

## Көмекшілерді шақырыңыз {#invite-helpers}

Пайдалану Torii- үйлесімді шақыру IDs және `domain.dataspace:meeting` формасы. Демода `kaigi.universal:<call-name>` құрылған кездесулер үшін.
...
Қауымдастық UI түтін сынамасы `/kaigi` Транзакцияға қолтаңба қою, камера, микрофон, және WebRTC рұқсаттар орындалу уақытына қарай әртүрлі болады.

Егер сіз TAIRA және шақыруға арналған бағытты қайтару `404`, ең алдымен , қосқан қапшықтың сәтті тапсырылғандығын растаңыз . `CreateKaigi`. Реле денсаулығының соңғы нүктелері белгілі бір шақырудан бұрын қол жетімді болуы мүмкін.
...
- Жер беті `KaigiRosterSummary`, `KaigiUsageSummary`, және `KaigiRelayHealthUpdated` операторыңыздың панельіндегі оқиғалар.
