...
ایک Kotodama V1 معاہدے کی جانچ پڑتال اور مرتب کریں، مقامی طور پر اس کے عوامی داخلے پوائنٹ کو انجام دیں، تصدیق شدہ IVM آرٹیفیکٹ کو تعینات کریں، تعینات کردہ داخلہ پوائنٹ کی نقالی کریں، اور اسے واضح طور پر اقتباس شدہ حکام کی طرف سے ادا کردہ فیس کے ساتھ جمع کروائیں.

## لازمی شرائط {#prerequisites}

- Iroha ذریعہ چیک آؤٹ `bc7114ed1c7f265a156d2100ff09e851cc95702c`، Rust، اور کارگو پر کمیٹی.
...
### 2۔ آثار قدیمہ کی جانچ پڑتال، تعمیر اور تصدیق {#_2-check-build-and-verify-the-artifact}
...
پہلا بلڈ آرٹیفیکٹ اور تصدیق شدہ سائیڈ کارس شائع کرتا ہے۔ دوسرا صرف پڑھنے کے `--verify` موڈ میں چلتا ہے اور ناکام ہوجاتا ہے اگر کوئی موجودہ آؤٹ پٹ موجودہ ماخذ سے بالکل مماثل نہیں ہوتا ہے۔ `.to` فائل اور اس کی منشور کو ایک جائزہ لینے والے بلڈ آؤٹ پٹس کے طور پر علاج کریں۔

### بائٹ کوڈ کو مقامی طور پر چلائیں {#_3-run-the-bytecode-locally}

`compute` ایک عوامی `kotoage` انٹری پوائنٹ ہے۔ اسے `debug-call` کے ساتھ چلائیں ، جو کسی لین دین کی پیش کش یا ادائیگی کیے بغیر مقامی فکسچر کے خلاف کام کرتا ہے۔
...
مددگار بائٹ کوڈ کے ٹکڑے اپ لوڈ کرتا ہے ، دستخط شدہ منشور رجسٹر کرتا ہے ، اور ایک `CommitContractDeployment` آپریشن پیش کرتا ہے۔ یہ ہر ٹرانزیکشن کی فیس کوٹیشن دیتا ہے اور کسی ایسے کوٹیشن سے انکار کرتا ہے جو منتخب کردہ ادائیگی کنندہ یا گیس بانڈ کو تبدیل کرے۔
...
خالی `charge_limits` درخواست ایک کاپی شدہ اثاثہ شناختی نہیں ہے: معاون دستخط کرنے سے پہلے عین مطابق لائیو کوٹیشن قبول کرتا ہے۔ واپسی چارج اثاثے کو موجودہ نل کے جواب کے ساتھ موازنہ کریں۔ معاہدہ کالز میں ورثہ `gas_asset_id` میٹا ڈیٹا شامل نہ کریں.

### 5۔ تعینات انٹری پوائنٹ کی نقالی کریں اور کال کریں۔ {#_5-simulate-and-call-the-deployed-entrypoint}

تخروپن Torii پر عوامی داخلے پوائنٹ کو جمع کرانے کے بغیر چلاتا ہے۔ مندرجہ ذیل کال ایک لین دین ہے اور اس وجہ سے اتھارٹی فیس ادا کرنے والا صریح طور پر منتخب کرتا ہے۔ دونوں احکامات 1,500,000 گیس کی حد کو پابند کرتے ہیں۔
...
عرفی کو حل کریں ، واپس آنے والے کوڈ ہیش کے ذریعہ آن لائن منشور حاصل کریں ، اور ایک ہی عوامی داخلہ پوائنٹ کو کینونیکل ایڈریس کے ذریعہ نقالی بنائیں:
...
- `CanRegisterSmartContractCode` ناکامیوں کے لئے Taira آپریٹر کی گرانٹ یا مقامی نیٹ ورک پر جینس / بوٹسٹریپ تبدیلی کی ضرورت ہوتی ہے۔ ایک عام اکاؤنٹ اس حقیقت کے بعد خود سے یہ اجازت نہیں دے سکتا۔
...
- ایک manifest یا ABI عدم مطابقت کا مطلب ہے کہ بائٹ کوڈ ، manifest اور node runtime ایک ہی artifact کی وضاحت نہیں کرتے ہیں۔ `--verify` کے ساتھ pinned commit پر دوبارہ تعمیر کریں۔
...
- صرف نقطہ نظر کے اندراج کی غلطی کا مطلب ہے کہ `compute` کو غلط کمانڈ فیملی سے روٹ کیا گیا تھا۔ یہ نمونہ `kotoage` کا اعلان کرتا ہے۔ لہذا کال سمیلیشن یا جمع کرانے کا استعمال کریں۔
- معاہدہ کالز میں مثبت ٹائپڈ گیس کی حد کی ضرورت ہوتی ہے۔ اعلی سطح کے ورثہ گیس یا فیس اثاثے کے میٹا ڈیٹا کو مسترد کیا جاتا ہے۔

## ماخذ اور متعلقہ دستاویزات {#source-and-related-docs}

- [Kotodama V1 کمانڈ پر عملدرآمد پنڈل کمیٹ](https://github.com/hyperledger-iroha/iroha/blob/bc7114ed1c7f265a156d2100ff09e851cc95702c/crates/ivm/src/bin/koto.rs)
- [ٹپل ریٹرن ماخذ کا نمونہ پنڈ commit](https://github.com/hyperledger-iroha/iroha/blob/bc7114ed1c7f265a156d2100ff09e851cc95702c/crates/kotodama_lang/src/samples/tuple_return_demo.ko) پر
- [پنڈل کمیٹ پر مقامی تعیناتی معاون](https://github.com/hyperledger-iroha/iroha/blob/bc7114ed1c7f265a156d2100ff09e851cc95702c/crates/iroha_cli/src/bin/ivm_contract_deploy.rs)
- [کنٹریکٹ انٹیگریشن ٹیسٹ پر پنڈ commit](https://github.com/hyperledger-iroha/iroha/blob/bc7114ed1c7f265a156d2100ff09e851cc95702c/integration_tests/tests/contracts.rs)
