...
- \(s\) باہر دنیا کی ریاست میں رکھا خفیہ حل کرنے والا ہو.
...
بیرونی طور پر فراہم کردہ خفیہ کردہ ان پٹ کے ل the ، ریزولور شناخت کنندہ لفافے کو ڈسکرپٹ کرتا ہے اور اسے انجام دینے سے پہلے اس تعیناتی لفافے میں دوبارہ خفیہ کرتا ہے۔ یہ کینونائزیشن سیمنٹک equal برابر BFV ciphertexts میں وصولی ہیشوں کو مستحکم رکھتی ہے۔
...
`IdentifierResolutionReceipt` ایک اعلی سطح کے مفید بوجھ پر دستخط کرتا ہے:
...
`ClaimIdentifier` رسید کو صرف اس صورت میں قبول کرتا ہے جب دستخط یا ثبوت درست ہو، ایمبیڈڈ RAM-LFE عملدرآمد کا فائدہ مند بوجھ ریفرنس کردہ پروگرام کی پالیسی سے ملتا ہے، اور `uaid` اور `account_id` لازمی طور پر دعوی کیا جاتا ہے.

## پھانسی کا بہاؤ {#execution-flow}
...
- اکاؤنٹ کا UAID
- کینونیکل `account_id`
- عام RAM-LFE عملدرآمد کا فائدہ مند بوجھ
...
|`POST /v1/ram-lfe/programs/{program_id}/execute` |`input_hex` یا `encrypted_input` سے ایک پروگرام کو چلائیں اور آؤٹ پٹ ہیشز کے علاوہ ایک غیر ملکی رسید واپس کریں۔ |
...
ان راستوں کے مقابلے میں تعمیر کرنے سے پہلے ہدف نوڈ کی `/openapi` یا `/openapi.json` دستاویز کو ہمیشہ چیک کریں۔ دستیابی نوڈ کی تعمیر اور نیٹ ورک پروفائل پر منحصر ہے۔
...
- اکاؤنٹ کے عرفی ناموں، ٹرانزیکشن میٹا ڈیٹا، واقعات یا دنیا کی ریاست کے شعبوں میں خام شناخت کنندہ نہ ڈالیں۔
