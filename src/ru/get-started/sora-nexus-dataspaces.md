...
- Требуйте четких человеческих инструкций перед живыми мутациями сети тестирования. Для заранее подписанных конвертов сделок используйте `iroha.transactions.submit_and_wait`, чтобы агент подождал результата, а не только подавал.
...
Финансируйте счет Taira через трубку, прежде чем запустить оплачиваемые счета. Прямой поток крана в [Получить тестнет XOR на Taira](#_4-get-testnet-xor-on-taira).
...
- Сохранить частные ключи в зашифрованном менеджере паролей, аппаратно поддерживаемом клавиатуре или специальном сервисе подписи. Не используйте ключи для управления источником или не оставляйте производственные ключи в истории оболочки, журналах, чатах, билетах или незашифрованных резервных копиях.
- Используйте уникальную пароль с высокой энтропией для каждого хранилища или подписи производства. Сохранить пароли в менеджере паролей или разделенном процессе хранения, не в том же файле или резервном пакете, что и зашифрованный частный ключ.
...
- `iroha3d --sora --config <config.toml> --trace-config` передает конфигурацию решенного узла.
