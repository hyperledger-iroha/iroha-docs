...
Сборник Python SDK в рабочем пространстве вверх по течению `iroha-python`. Первый Iroha 3
выпускных целей текущего Torii и Norito Пин версии упаковки
или пересмотр источника, используемый вашей интеграцией, чтобы SDK и узел остается на
одинаковый пересмотр формата провода.

Нижеприведенные примеры, предназначенные только для чтения, были проверены против общественности. Taira на
`https://taira.sora.org`. Мутационные примеры - шаблоны транзакций: они
требуют реальных Taira власть, частный ключ, газовые метаданные и любой оператор
токены, требуемые целевым маршрутом до их подачи.

Используйте примеры в этом порядке:

| Стадия | Соревноваться с общественностью Taira? | Что тебе нужно |
| --- | --- | --- |
| Звонки клиентов для чтения только | Да , это так . | Python пакет плюс доступ к сети |
| Местные строители подписей и инструкций | Никаких сетевых звонков до `submit()` | Местное расширение и ваш ключевой материал |
| Мутация транзакций и звонки на услуги | Только на собственном счете . | Счет органа, частный ключ, цепочка ID, метаданные о сборах, баланс сборов и токены маршрута |
| Соедините кодеки кадров, крипто и GPU помощники | Только местные | Начальное расширение; GPU Помощники также нуждаются в CUDA- способный обратный контент |

## Установка {#install}

Название метаданных пакета `iroha-python`. Не предполагайте, что не подкрепленный PyPI
устанавливать совпадает с живым Taira установить колесо или источник кассы, что
был построен из того же пересмотра вверх ваших целей интеграции:

```bash
python -m pip install /path/to/iroha_python-*.whl
```

Если ваш проект напрямую потребляет рабочее пространство вверх потока, установить Python
зависимости и создать нативную расширение перед запуском примеров, которые используют
`Instruction`, `TransactionDraft`, подпись, крипто, SoraFS местные помощники, GPU
Используйте команду создать из потока
`python/iroha_python/README.md`, затем проверять, что нагрузка на внутренний экспорт:
...
Если `create_torii_client` импорт, но `Instruction` или
`generate_ed25519_keypair` Неудачи, чистые Python пакет доступен, но
Но это не так.

## Быстрый старт {#quickstart}

Начнём с общественного, только для чтения Taira конечные точки:
...
Используйте эту настройку для мутирующих шаблонов.
Taira авторитет, частный ключ, токен и актив/счет IDs от вашего развертывания
до подачи.

`authority` является счетом, подписывающим транзакцию. `private_key` должно соответствовать
этот счет, `CHAIN_ID` должны соответствовать целевой сети, и `TX_METADATA` Должен
включают в себя поля сборов, ожидаемые сетью.
намеренно недействительны, поэтому они не представляются случайно.
...
`Instruction.*` Призывы только строить инструкции полезные нагрузки. `submit()` Это
точка, в которой SDK подписывает сделку, отправляет ее Torii, и ждет
статус.

## Сборы и газ {#fees-and-gas}

Запись транзакций требует метаданных по сборам и баланса активов по финансируемым сборам Taira,
актива по счетам финансируется государственным краном, а метаданные сделки должны быть
включает в себя `gas_asset_id`. На Minamoto, сборы оплачиваются реальными XOR и активы
ID происходит из конфигурации этой сети.

Метаданные по счетам относятся к сделке, а не к отдельным инструкциям.
`submit()` помощник над прикреплениями `TX_METADATA` на каждую транзакцию, которую она создает:
...
Прежде чем отправить письмо, убедитесь, что на счете власти достаточно денег.
Точный кранок и актив. ID являются специфическими для сети; Taira
форма:
...
Насос возвращает бетон . `asset_id` Для проверки баланса.
`gas_asset_id` поле метаданных использует определение актива сбора ID.

Сохранять метаданные заявки отдельно от метаданных сборов путем объединения картографий
...
Если вы пропустите метаданные сборов, используйте неправильный актив сборов или подпишитесь без финансирования
Счет, реальная сеть должна отклонить транзакцию даже если инструкция
полезная нагрузка в противном случае действует.

## Taira- Проверенные звонки только для чтения {#taira-checked-read-only-calls}

Эти вызовы успешно вернулись против общественности Taira:
...
Маршруты, такие как `/v1/status`, общественный инвентарь сверстников, Sumeragi RBC выборка проб, узлы
администратор снимки , и приложение Connect регистрация администрации не были публично
доступно в Taira Во время проверки. `request_json("GET", "/status")` для
полезная нагрузка статуса публичного узла на Taira.

## Инструкция строителя {#instruction-builders}

Сборник SDK выявляет типовые конструкторы для наиболее распространенных семейных инструкций и
JSON шлюз для вариантов, не являющихся первоклассными Python Методы пока.
Следующие фрагменты являются мутирующими шаблонами транзакций и не были
представленной общественности Taira без подписанного счета.

Предпочтительнее, когда они существуют: они нормализуются Python значения и неисправности
использовать недействительные формы `Instruction.from_json` только тогда, когда вам нужен
вариант инструкции, который не имеет Python Помощник еще.

| Инструкционная семья | Python поверхность |
| --- | --- |
...
| Отчуждение регистрации | `unregister_trigger`; использование `Instruction.from_json` для других вариантов |
...
| RWA жизненный период | `merge_rwas`, `redeem_rwa`, `freeze_rwa`, `unfreeze_rwa`, `hold_rwa`, `release_rwa` |
| ExecuteTrigger | `execute_trigger` |
| Расширения репо/заселения | `repo_initiate`, `repo_unwind`, `repo_margin_call`, `settlement_dvp`, `settlement_pvp` |
| Закрытие коренных активов | `open_asset_lock`, `drawdown_asset_lock`, `cancel_asset_lock`, `expire_asset_lock`, плюс клиент `*_and_wait` помощники |
| Грант/Ревок, SetParameter, Регистрационные, пользовательские, модернизированные и менее распространенные варианты реестра/нерегистрации | `Instruction.from_json` или `TransactionBuilder.add_instruction_json` с каноническим `InstructionBox` JSON |

Для условных платежей в виде поручительства см.
[Осуществление сбережений на собственные активы](/ru/blockchain/escrow.md#python-asset-locks). Python
в настоящее время подвергает воздействию первоклассные помощники для генеральных блокировки активов; рыночные и
анонимные помощники не первоклассники Python Методы пока.

### Создать домены, затем зарегистрировать счета и активы {#set-up-domains-then-register-accounts-and-assets}

Обычное создание домена проходит через декларирующий псевдоним планировщика так что SNS
аренду, возможности владельца, охрана котировок и состояние домена проверяются вместе.
Создайте без секретов `AliasSetupPlanRequestV1` намерение с вашей SDK или
сервис бортового обслуживания, затем использовать `iroha app alias setup plan` и
`iroha app alias setup apply`. Не подчиняйтесь `Instruction.register_domain`
из транзакции приложения; этот конструктор остается для генезиса/bootstrap
инструменты.

После того, как план настройки домена обязуется, зарегистрируйте объекты, принадлежащие домену.
сеть, например: Taira, использовать присвоенное вам доменное и учетное пространство.
...
`mintable` принимает `Infinitely`, `Once`, `Not`, или `Limited(n)` принятые значения
по модели данных. `scale` для неограниченного количественного актива.

### Минетные, сжигаемые и перечисленные активы {#mint-burn-and-transfer-assets}

Эти звонки используют существующий актив ID. Сначала зарегистрируйте определение актива, затем
строить бетонный актив ID на счет, который владеет активом.
...
Передача собственности изменяет, кто контролирует домен, определение активов или NFT.
Используйте действующего владельца в качестве органа по сделке.
...
Значения метаданных должны быть: JSON- в серии. `TransactionDraft`, в)
власть `TransactionConfig` становится целевым учетным записью по умолчанию.
...
Проект помощника высокого уровня по умолчанию ориентирован на орган по сделкам:
...
RWA помощники используют JSON-сериализируемые полезные нагрузки для метаданных, специфических для активов,
происхождение и политика контроллера. `register_rwa` не принимает `id` или
`owner`: время запуска генерирует `RwaId`, и орган по осуществлению сделки
становится первоначальным владельцем.
...
После того, как регистрационная сделка обязуется использовать `FindRwas`, `/v1/rwas`, в) RWA
событие или маршрут исследователя, установленный для обнаружения генерируемого ID:
...
Последующие операции используют генерируемые `hash$domain` ID:
...
Полное перечисление может меняться `owned_by` на существующей партии.
Слияния создают генерируемые детские лоты.

### Стрельцы {#triggers}

Используйте помощники регистрации запуска , когда исполняемый инструмент является другим инструкцией
последовательность:
...
Torii также выявляет REST вспомогательные устройства для учета запасов:
...
Инвентарные вызовы триггера читаются или проверяются только.
Исполнение, повторение изменений и нерегистрация - это мутирующие операции.

### Инструкции по перевозке и расчету {#repo-and-settlement-instructions}

Репо и помощники двустороннего урегулирования добавляют специальные инструкции по домену
варианты без ручной работы Norito полезные грузы:
...
### JSON Убежище {#json-escape-hatch}

Когда Python Помощник пока не доступен, подать каноническую модель данных
`InstructionBox` JSON в `Instruction.from_json` или непосредственно
`TransactionBuilder.add_instruction_json`. Это рекомендуемый путь для
`Grant`, `Revoke`, `SetParameter`, `Log`, `Custom`, `Upgrade`, сопоставимость/роль/NFT
регистрация, а также недействительные варианты незарегистрации до тех пор, пока эти помощники
Написано.
...
Для генерируемых или непрозрачных инструкций, перемещение обратно JSON перед хранением
устройства:
...
Использование `TransactionDraft` для приложений, которые создают несколько инструкций до
Проект позволяет сохранить настройки уровня транзакции, такие как: `ttl_ms`,
`nonce`, и метаданные в одном месте, а затем подпишите один раз:
...
Экспортировать детерминистический манифест для рассмотрения, аудита или передачи кошелька:
...
Присоединяйте доказательство конфиденциальности полосы перед подписью, когда это требуется для целевой полосы:
...
Типовые помощники запроса возвращают классы данных вместо сырых JSON Словарь.
Это самый простой способ начать, потому что SDK просматривает страницы и общие
для вас записывающие поля:
...
Используйте общие помощники при запросе Torii конечный пункт еще не имеет напечатанного
упаковка:
...
Помощники учетной записи требуют идентификатора счета, принятого SDK Я ...
Нормализатор. I105 счета IDs или псевдоним на цепи; если блок
исследующий или сырой конечный пункт возвращает ID что SDK отклоняет, разрешают его на
канонический отчет ID прежде, чем вызвать этих помощников:
...
Декодирование помощников потокового потока JSON по умолчанию. `with_metadata=True`
когда вам нужно SSE Имя события, идентификатор, намек на повторную попытку и сырая полезная загрузка.
с `EventCursor` Чтобы сохранить последний идентификатор события.
события, так что запустить их против узла, где соответствующий поток событий
включенные и активные.
...
Сборник SDK раскрывает местные помощники для подписи для каждого составленного алгоритма подписи
Они не призывают к помощи. Taira, но они требуют
местное расширение:
...
Использование `supported_crypto_algorithms()` чтобы увидеть, что поддерживает ваше колесо.
общие помощники используют канонические алгоритмные ярлыки и работают для Ed25519,
secp256k1, ML-DSA, GOST, BLS, и SM2 когда эти алгоритмы составляются в:
...
### Китайский SM Криптовалюта {#chinese-sm-cryptography}

Сборник Python SDK выявляет как генерические SM2 помощников и SM2-специфический удобство
Используйте объявление о возможностях узла, чтобы выбрать SM2 различающее
идентификатор, ожидаемый целевой сетью:
...
`crypto.sm.enabled` сообщает вам, принимает ли узел SM- семейные алгоритмы в
В этом же объявлении включена SM политика хэша и ускорение
Статус, который полезен при принятии решения о том, позволить SM2-специфические потоки:
...
Общественность Taira выявленные SM объявление о возможностях во время проверки, но SM подпись
Его рекламные алгоритмы подписания были `ed25519`,
`secp256k1`, и `bls_normal`, Так что не поддавайтесь SM2- подписанные сделки к этой
развертывание, если только полезная нагрузка возможностей не меняется.

### GOST и послеквантовые ключи {#gost-and-post-quantum-keys}

Используйте общий крипто API для GOST R 34.10-2012 параметровые наборы и ML-DSA
(`ml-dsa`1) после квантовых подписей.
проверка и экспорт мультихаш:
...
Ворота GOST и послеквантовых потоков на рекламных алгоритмах подписания узла.
Используйте полезную нагрузку сырой возможности для имен алгоритмов, совместимых вперед:
...
Если узел не рекламирует необходимый алгоритм, используйте ключ только для локальных
Не предоставляйте транзакции, подписанные с этим алгоритмом
Во время общественного обсуждения Taira проверка, GOST и ML-DSA были доступны как SDK
крипто-помощи в верхнем потоке Python Библиотека, но не были объявлены
узел для подписания транзакции.

## Создание клиентов с конфигурацией {#config-aware-client-creation}

Использование `resolve_torii_client_config` когда приложение читает настройки узлов
из файла, но все же требует переменных по условиям среды или испытаний:
...
## Готовность кагемуши {#kagemusha-readiness}

Сборник Python SDK может запросить текущее JSON маршрут готовности через свой общий
Torii Помощник:
...
Python не раскрывает загруженные Kagemusha дополнения или выкупа архивных конструкторов.
Используйте напечатанный Swift или JVM кошелек для построения канонического V4 архивы, тогда
представить и опросить их через поддерживаемую Kagemusha Torii Клиент.

## Подпись {#subscriptions}

Подписные помощники мутируют звонки службы , унаследованные от общих Torii
клиент, используемый `iroha_python.ToriiClient`. Использование IDs и активов, которые существуют на
сеть, которую вы нацелились.
...
Создать и анализировать соединение URIs, и прочитать статус общественного соединения , выявленный
Taira:
...
Фрейм-кодеки, выводы ключей сессии и создание сеанса требуют родной
расширение и включенный маршрут сессии Connect:
...
Эти звонки для чтения только успешно возвращаются против общественности Taira:
...
Помощники обновления запуска принимают форму манифеста , используемую в обновлении запуска
API. Это действия оператора, так что используйте их только против узла, где ваш
разрешены счета и токены:
...
Эти помощники доступны, когда целевой узел раскрывает соответствующие
Nexus/SORA Оценить пустые списки как действительный ответ: публичный Taira Мая
имеют включенный маршрут без данных для манифеста образца; или UAID.
...
Использование `NoritoRpcClient` когда у вас уже есть Norito Байты и необходимо позвонить
двойной Torii Пример требует подписанного конверта из предыдущего
шаблон сделки:
...
CUDA Помощники возвращаются `None` когда обратный конец не доступен, поэтому приложения
может вернуться к масштабным реализациям:
...
Сборник Python SDK уже включает в себя помощников для:

- Torii поток подачи, состояния, запроса и администратора
- Типовые конструкторы инструкций для общего пользования ISI и доменно-специфические расширения
- проект сделки, манифест, подпись и подписанный конверт сделки
  рабочие процессы
- потоковые события, фильтры и курсоры для перезагрузки
- доступ к генерическому Kagemusha готовности и Torii подписные помощники;
  строители пополнения и выкупа не подвергаются риску
- адрес аккаунта, помощники для подписания всех алгоритмов, многофамильные обратные поездки, SM2,
  GOST, ML-DSA, BLS, и конфиденциальная обработка ключей
- Подключение URIs, сессии, кадры, помощники шифрования и администратор реестра
- управление, модернизация рабочего времени, Sumeragi, Node-admin, SoraFS, UAID, и Kaigi
  закладки конечных точек, где узел раскрывает эти особенности
...
Эти файлы являются источником правды для Python поверхность в закрепленной
пересмотр рабочего пространства.
