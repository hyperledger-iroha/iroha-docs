...
Примеры используют TypeScript и написаны так, чтобы они могли работать в Electron, браузере с безопасным бэк-эндем или веб-приложении с расширением кошелька.
...
Ведущий создает предложение, хранит его через `CreateKaigi`, Гость получает зашифрованное предложение, создает ответ. и сообщения, которые отвечают с `JoinKaigi`.
...
Автоматическая сигнализация зависит от живого кошелька, маршрутов Kaigi -способных Torii и генерации доказательств в частном режиме.
...
Если вы тестируете на TAIRA и маршрут специального звонка возвращает `404`, сначала подтвердите, что кошелек-хозяин успешно поступил `CreateKaigi`.
