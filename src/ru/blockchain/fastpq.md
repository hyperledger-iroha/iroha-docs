...
В данном разделе описывается арифметика, реализуемая текущим Rust проверщиком и верификатором.
...
FRI обязуется выполнять оценку состава AIR. Для каждого раунда `l` транскрипты пробуют задачу `beta_l`.
...
Прежде чем создать след, проверка FastPQ канонизирует порядок партии по ключу перехода, рангу операции и порядку вставки.
...
Трейс содержит колонки-селекторы для передачи, монетки, сжигания, предоставления ролей, отзыва ролей, набора метаданных и рядов поиска разрешений.
...
В заявке `compliance` вводятся две строки метаданных: одна для политики и другая для целевых данных.
...
Канонический каталог параметров показывает два набора параметров. `fastpq-lane-balanced`.
