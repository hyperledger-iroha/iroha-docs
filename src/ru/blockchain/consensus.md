...
Все сети Iroha 3 используют доступность данных и надежные каналы вещания.
...
Вместо того, чтобы каждый валидатор отправлял каждый голос каждому другому валидатору, Sumeragi может выбрать одного или нескольких коллекторов на высоту. Настройки эффективных коллекторов раскрываются через `GET /v1/sumeragi/collectors`; изображение CLI по `ops sumeragi telemetry` сообщает о текущем количестве коллекторов.

Сверстники наблюдателей могут синхронизировать обязательные блоки, но они не предлагают, голосуют, не собирают голосов или не рассчитывают на кворум комитета.
...
В очереди сохраняется это решение маршрутизации с хэшем транзакции, чтобы более поздние этапы не должны делать его снова.
