...
|`TransferRwa` |Перемещение количества на другой счет. Полный перевод может изменить `owned_by`. Частичный перевод создает отдельную детскую партию с генерируемой ID. |
...
|`RedeemRwa` |Постоянно вычитать количество из обращения. Владелец или контролер может представить его, когда `redeem_enabled` является верным. |
...
Используйте метаданные для компактных фактов, которые помогут приложениям определить и проверить партию:
...
Контроллерские учетные записи и роли могут выполнять только операции, которые разрешаются соответствующими булевыми флагами. Нынешняя контрольная нагрузка содержит идентификации контроллера и эксплуатационные флаги. Передачи разрешенных списков и заложенные правила `transfers` не входят в эту полезную нагрузку.
...
Ожидается пустой `items` выпуск, когда пока не зарегистрированы публичные партии. Регистрация, перевод, хранение, замораживание и выкуп - это подписанные сделки.
...
Подача `ReleaseRwa` после успешного выполнения процесса вне цепи:
...
Составление `RedeemRwa` после того, как представленный актив вне цепочки доставляется, потребляется, выводится на пенсию или иным образом удаляется из обращения. Это постоянно вычитает представленное количество из партии. `redeem_enabled`. Подписчик должен быть владельцем или контролером.
...
Подача `FreezeRwa`, когда внецепочного обзора должно блокировать обычные операции владельцев. Подписьчик должен быть контролером. В партии должна быть `freeze_enabled`.
...
Сдача `UnfreezeRwa` после прохождения пересмотра:
...
Представьте `RedeemRwa`, чтобы удалить заявленные углеродные кредиты из обращения. Сохраняйте сертификат вне цепи или доказательство регистрации в метаданных:
