...
Aitai - это коридор приложений SORA для расчетов в стиле рынка, где покупатель и продавец координируют платеж вне цепочки, а Iroha контролирует хранение активов на цепочке. Он должен использовать родную семью инструктажей на хранение, а не контрактный счет на хранение для новых потоков конфиденциальности с цифровыми активами.

Native escrow сохраняет хранение в регистре. Продавец открывает предложение с `OpenAssetEscrow`, покупатель принимает и отмечает внецепочной оплату с `AcceptAssetEscrow` и `MarkEscrowPaymentSent`, а продавец выпускает с `ReleaseAssetEscrow` или отменяет ее до того, как оплата будет отмечена. Если покупатель и продавец не согласны, любая из сторон может открыть спор, а разрешение с `CanResolveEscrowDispute` может разделить запертую сумму.
...
| `OpenAnonymousAssetEscrow`, `AcceptAnonymousAssetEscrow`, `MarkAnonymousEscrowPaymentSent`, `ReleaseAnonymousAssetEscrow`, `CancelAnonymousAssetEscrow`       |Защищенные предложения, в которых движения по финансированию и закрытию осуществляются с помощью доказательных приложений. |
...
Для общественности Taira или Minamoto использование, относиться к внецепочке платежной железной дороге и любому рабочему процессу поддержки или судебного разбирательства как к политике применения. Iroha записывает состояние содержания, события жизненного цикла, хэши доказательств и окончательное движение активов; не проверяет фиатные расчеты самостоятельно.
...
| ---------------------- | ------- | -------------------------------------------------------------------------------------------- |
|`DeterministicService` |`Ivm` |Автор, состояние хранилища, сертифицированные чтения, заказанные операторы почтовых ящиков, мутации, чувствительные к управлению |
|`HttpService` |`Inrou` |Прямой HTTP APIs, коллекторная работа, услуги с поддержкой кеша, SSE, потоки с помощью браузера |
...
Инру - хозяин . HTTP время выполнения, используемое Soracloud. Сборник Iroha узел с встроенным Soracloud приняты проекты по запуску Soracloud сообщение в местный план материализации, запускает присвоенные копии хостинг-сервиса в качестве сервисов loopback, и сообщает реплика состояния запуска обратно в авторитетную модель.
...
В приведенном ниже примере показана форма двух манифестов.
...
1. Создать или получать полезную нагрузку за пределами WSV, например файла SoraFS CAR или полезной нагрузки в полосе Nexus.
...
1. Канонизировать представленную полезную нагрузку. Torii принимает запрос приема с помощью `(lane_id, epoch, sequence)`, байтов полезной нагрузки, метаданных сжатия, размера кусочка, профиля удаления, политики хранения и подписи подавшего заявление. Уз разжимает gzip, deflate или Zstandard полезные нагрузки при запросе, а затем проверяет, что каноническая длина байта равна `total_size`.
2. Подтвердить параметры полосы и части. полоса должна присутствовать в каталоге полос Nexus. `chunk_size` должна иметь не нулевую мощность двух, по меньшей мере двух байтов и не превышать установленный максимум. Профиль удаления должен включать фрагменты данных и по крайней мере два фрагмента паритета. В каталоге полос выбирается схема проверки, либо `merkle_sha256`, либо `kzg_bls12_381`.
...
8. Выпускать подписанные артефакты. Torii вычисляет обязательство PDP, подписывает `DaIngestReceipt`, создает `DaCommitmentRecord` и пишет артефакт-спутник для манифеста, PDP обязательства, записи о обязательствах, расписания обязательств, намерения PIN, файла квитанции и дневника квитанций. Курсор квитанции продвигается монотонно на `(lane_id, epoch)`.
...
В заголовке блоков хранятся хэши для политики доказательства DA, обязательств и целей пин. Для доказательств членства пакет обязательств также раскрывает корень Меркла, листья которого являются хэшами канонических значений `DaCommitmentRecord`, закодированных в Norito. Родительские узлы соединяют левую и правую клетку; необычный лист продвигается неизменным на следующий слой.
...
Если ни одно из этих условий не соответствует требованиям, регистрационная группа `missing_local_data`, продолжает пытаться восстановить полезную нагрузку через RBC или блок-синхронизацию. и сообщает портал DA по состоянию и телеметрии. В текущей реализации эти сигналы DA являются рекомендательными для окончательности: блок все еще завершается из сертификата обязательства плюс соответствующая местная полезная нагрузка, не из отдельного сертификата кворума DA.
...
Iroha 3 Консенсусные профили включают: RBC- поддерживаемое распространение полезной нагрузки, защитные манифесты, DA Бандлевая валидация и телеметрия восстановления. `[sumeragi.da]` лимиты на обязательства и открытия для доказательств в блоке, плюс `[sumeragi.advanced.da]` Усовершенствуйте эти настройки по validators в одной сети Профиль.
...
SoraFS - это децентрализованная ткань хранения адресованного контента. Он упаковывает байты в детерминистические куски, архивы CAR и манифесты Norito, которые связывают корни контента, профили разбивки, политики пин и аттестации управления. Поставщики хранилищ рекламируют способность и доступность контента, в то время как шлюзы проверяют манифесты и частичные обязательства перед предоставлением контента.
...
SoraDNS - это детерминистический слой наименования для услуг и контента SORA. Он нормализует имена, закрепляет обновления каталога решителей в Iroha и распространяет подписанные зоны или пакеты решений через SoraFS. Решатели и шлюзы проверяют документы аттестации решателя до того, как доверять метаданным обнаружения.
...
В сети, где пока не опубликован каталог решений, `/v1/soradns/directory/latest` может возвращать `404`, даже если маршрут включен.
...
Маршрут идентификатора BFV используется для сохранения конфиденциальности. Клиент может представить зашифрованный идентификатор решителю Torii. Решатель оценивает его в соответствии с политикой активного идентификатора, получает `OpaqueAccountId` и выдает квитанцию . `ClaimIdentifier` затем связывает этот квитанция с UAID, прикрепленной к целевому счету.
...
`FheJobSpecV1::validate_for_execution` проверяет, согласуются ли задачи, политика выполнения и набор параметров до ввода. Он также применяет специфические для операции правила: добавление и умножение требуют как минимум двух входов, вращение и загрузка требуют точно одного входа, а также требуемая глубина, количество вращений, количество загрузок, количество входов; Результаты запроса по шифру текста не должны возвращать строки простого текста.
