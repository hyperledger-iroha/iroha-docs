...
Поглощение стимулов эстафеты закрывается. `incentives.enable` Это правда. `incentives.trusted_verifier_ids` должно содержать не менее одного и не более 64 канонических сведений. IDs. Время выполнения хранит список как детерминистический упорядоченный набор, а недействительная геометрия списка отклоняется во время запуска эстафеты.

Каждый `RelayBandwidthProof` декодируется в соответствии с фиксированными рамками/бюджетом распределения и должен включать всю рамку. Счет проверщика доказательства должен присутствовать в настроенном списке, а `RelayBandwidthProof::verify_signature()` должен быть успешным, прежде чем эстафета блокируется или изменяется его аккумулятор производительности. Недоверенный подписитель или подпись-недействительный/ошибленный доказательство, следовательно, не способствует измерению и не может производить снимок стимула.
...
Прежде чем читать байты, Torii подтверждает каноническое кодирование локального манифеста, семантические ограничения, дигест и корень CID. Затем требуется авторитетная идентификация местного поставщика, признание управления, а также регулируемые проверки соответствия и снятия конфиденциальности для манифеста CID. и провайдера. Политика тарифа / запрета на вход использует действительный адрес клиента, уважая направленные адреса только через конфигурированные доверенные представители. Лишняя политика, соответствие, снятие, идентификация или состояние приема не закрывается.

Один запрос имеет разрешение на общественный портал от конца к концу; ограничение по всему процессу составляет 64 одновременные чтения, с возвращением излишних запросов `503 Service Unavailable` и `Retry-After: 1`. Явные ответы ограничены на 16 MiB, Списки файлов по умолчанию составляют 50 записей и принимают не более 500, а полный файл или диапазон одного байта ограничиваются 8 MiB. CIDs, запросы, хосты, пути и заголовки диапазона должны использовать их канонические формы с однозначным значением. HTML, сценарий, SVG, XML, PDF, или контент Wasm подается только из конфигурированного CID- полученное изолированное происхождение (или перенаправленное туда), предотвращая реализацию недостоверного контента совместным путем-воротным источником.

### Проблемы с умеренностью {#moderation-challenges}

SoraFS Экономика сдерживания вызовов - это состояние консенсуса. Активная политика называет актив управленческого голосования и учетные записи управления, используемые для депонирования и резки. Дело отклоняет двойной идентификатор вызовов, второй вызов с той же учетной записью или повторно использованный анализ доказательств без изменения балансов или счетчиков вызовов.

Срок подачи вызовов и срок решения вызовов разные. Управление получает именно через 24 часа после подачи заявления о принятии или отклонении предъявленного вызова. Ожидающиеся вызовы блокируют голосование только в течение указанного срока резолюции:

- принятый вызов прекращает дело и возвращает полную сумму облигации;
- отклоненный вызов позволяет продолжить дело, направляет 25% облигации получателю скрата (закругляется вниз по точности активы с правом голоса) и возвращает оставшуюся часть; а также
- неразрешенный вызов истекает по истечении срока действия, не открывается и возвращается полная облигация.

`ExpireSorafsModerationChallenge` не имеет разрешения и не в состоянии ответить на вызов, который уже истек. Окончание дела выполняет то же расчеты по истечении срока действия, что и резервный режим. так что отсутствующий хранитель не может оставить деньги запертыми или хранить раскрытия заблокированными. Каждое урегулирование является атомным: если возмещение или решетка не удается, полная урегулировка отменяется.

Политика модерации и записи случаев используют схему первого выпуска непосредственно. или восстановление мгновенных снимков; регенерировать эти устройства вместо вывода голосового актива, счетов по хранению, сроков, или экономики из состояния наследия.
