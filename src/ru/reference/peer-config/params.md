...
Повторная атака - это попытка передать действительную транзакцию в другую сеть, чем она была предназначена. Поскольку `chain` является частью подписанной полезной нагрузки транзакции, сделка, подписанная для одной цепи, отклоняется коллегами, использующими другую цепь ID.
...
Стринг состоит из одной или нескольких директив, разделенных с помощью кома.Каждая директива может иметь соответствующий максимальный уровень вербильности, который позволяет (например, выбирает для) совпадающие промежутки и события. Iroha считает менее эксклюзивные уровни (например, `trace` или `info`) более сложными, чем более исключительные уровни (такие как `error` или `warn`).
...
::: info Совместное использование с [`logger.level`](#param-logger-level)
...
- `full`: По умолчанию форматирующий устройство. Это выпускает читаемые человеком, однострочные журналы для каждого события, которое происходит, с текущим контекстом протяженности отображается перед форматным представлением события.
- `compact`: Вариант форматировщика по умолчанию, оптимизированный для коротких длин строк. Поле из текущего контекста интервала приложены к полям форматного события, а названия интервала не отображаются; уровень верботности сокращается до одного символа.
- `pretty`: Выпускает чрезмерно красивые, многолинейные журналы, оптимизированные для человеческой читаемости. дебюгирование или для приложений командной строки, где автоматизированный анализ и компактное хранение журналов не являются приоритетом по сравнению с читаемостью и визуальной привлекательностью.
...
Kura режим инициализации. `strict` - это нормальный и по умолчанию режим: он проверяет каноническую историю, артефакты восстановления, вспомогательные индексы и учет хранения до того, как узел становится активным.

`fast` является режимом экстренной службы для восстановления видимости эксплуатации, когда Это требует хранения, предварительно инициированного через `strict` и текущее поколение мгновенных снимков, содержащие ровно пять артефактов: `snapshot.data`, `snapshot.sha256`, `snapshot.sig`, `snapshot.fast.norito`, и `snapshot.merkle.json`. подпись оператора, разделенная на домены, связывает объявленный перевод полезной нагрузки и маркированный манифест; манифест связывает длину полезной нагрузки, идентичность цепочки/сети, высоту терминала/шесты; SCCP политический хэш, и присутствие стартовой линии. быстро отвергает стартовую родословная и требует того же точного маркера/счета/конкурса границы от длительной Kura. Узлы первого выпуска принимают именно эти пять артефактов и отвергают любой другой набор артефакта или файлового имени.

Быстрый инвентарь этих пяти имен и метаданных связывает полезную нагрузку и файлы Merkle, но не читает, хэширует, анализирует или декодирует их содержание. Он создает минимальный мир/Nexus из подписанного манифеста, отображает точный Kura хэш-префикс только для чтения, а затем оставляет снайп World, блок-хэш массив. история транзакций, производные индексы и журналы долгосрочного восстановления не открыты. Merkle, канонические и семантические аудиты мгновенных снимков, исторический блок/окончательность/SCCP согласование, восстановление активной высоты Sumeragi, журналы слияния и запросов, манифест полос/источники соответствия, Архивы SoraFS, аккаунтирование рекурсивного хранения и дополнительные сервисные согласователи, поддерживаемые Kura, остаются отложенными. Kura сам отклоняет запуск автора и долгосрочные мутации; трубопроводы и очереди настойчивости FASTPQ сразу же отказываются от работы, вместо того чтобы сохранять или кодировать ее. Kura читается APIs также отключает поведение ремонта и синхронизации долговечности: временные боковые автомобили не продвигаются, отсутствующие артефакты полосы не публикуются, а барьеры для прогресса не синхронизируются. Sumeragi и сплетни о транзакциях не запускаются. Torii показывает только здоровье, жизнеспособность, готовность, сверстники и операции конфигурации; API-версия, статус, показатели и все обычные маршруты состояния/истории остаются недоступными.

Используйте `fast` только для инцидента. После того, как служба стабилизируется, остановить узел, восстановить `strict` и перезапустить так, чтобы каждая отложенная проверка и восстановление индекса работали до возобновления производства. Быстрый режим не требует отложенного журнала слияния и не создает, не ремонтирует, не сокращает или не импортирует каноническое хранилище; неопубликованные суфиксы и ожидающие этапы вспомогательного восстановления игнорируются без прочтения или мутации, а затем оставляются для строгой восстановления. Импортированный с помощью хэша линейка снимков остается недоступным. Отсутствующий или недействительный текущий снимок сразу же проваливается; быстро никогда не возвращается к пустому миру или восстановлению исторической воспроизведения.
...
- `fast`: ограниченный аварийный старт с производственным карантином до строгой перезагрузки.
...
## Nexus Атомное частное урегулирование {#nexus-atomic-private-settlement}

`[nexus.atomic_private_settlement]` регулирует отдельный путь `AtomicPrivateSettlementV1`. Он отключен по умолчанию. Настройка `enabled = true` также требует `activation_height`; вход все равно не закрывается, если не активированы возможности на цепочке, срок уведомления, фиксированный профиль доказательства и управление фондом / аудитом.

Основные границы: `max_participants`, `max_expiry_blocks`, `audit_timeout_blocks`, `prepare_timeout_blocks`, `commit_timeout_blocks`, `max_proof_bytes`, `max_capsule_bytes`, `max_carrier_bytes`, `sidecar_retention_blocks`, `sidecar_max_records`, и `sidecar_max_total_bytes`. `capsule_padding_classes_bytes` должны быть строго растущим подмножеством V1 занятия по накладке. `permitted_policy_versions` принимает только V1.

`max_capsule_bytes` измеряет канонический Norito байты полного `PrivateSettlementAuditCapsuleV1`, в том числе AAD, нонс, шифровый текст, векторная рамка и каждый аудитор завернутый...DEK Каждый включенный класс заполнения должен соответствовать консервативному обложке всей капсулы для использования на минимум `default_min_auditor_approvals` Это утверждение также является регулируемым этапом: Torii отклоняет вновь принятую политику с более низким `min_approvals` ценность и отклоняет какую-либо фактическую капсулу, превышающую пределы канонических байтов.

Эти настройки не имеют обхода активации переменной среды производства. Для полного примера конфигурации и эксплуатационных требований см. [Run Atomic Private Cross-Dataspace Settlement](/ru/get-started/atomic-private-settlement) . Путь не квалифицирован для производства до прохождения документальных внешних выпускных портов.

## Снимок {#snapshot}

Этот модуль отвечает за чтение и написание мгновенных снимков [Взгляд на мир](/ru/blockchain/world#world-state-view-wsv).

Снежные снимки сохраняют сериализированный контрольный пункт World State View, чтобы один коллега мог перезагрузить без повторного воспроизведения каждого блока из Kura. Kura остается долговечной историей блоков и источником истины для воспроизведения; снежные фотографии являются ускоренным путем. При запуске Iroha проверяет метаданные снимка с конфигурированной цепочкой и хранящимися блоками, прежде чем решать, загрузить снимок или вернуться к воспроизведению.
